...
Description
  State table for "Place Arc by Center and Radius" command

Notes


 */
command_string  GRMPlcArcCntRad,CO_ni_place_arc_radcnt,GRPArCnR
synonym         GRMPlcArcCntRad,CO_ni_place_arc_radcnt,EMPArCnR
class           COnpt
...
state start
   execute SSactivate ()
   message_key     GRC_M_PlcArcCntRad
...
state have_third_event
   execute SSactivate ()
   prompt_key      GR_P_EntSweepAng
   dynamics        DYrbarccntrad
   filter          get_event
   on EX_DATA.GRst_ANGLE_TO     or
      EX_DATA.GRst_ANGLE_FROM   or
      EX_DATA.GRst_TANGENT_TO   or

...
Description
  State table for "Place Equally Spaced Points along Element" command.
  Command Key = GRPEqPnAlE

Notes


 */
command_string  GRMPlcEqSpcPtsEle,CO_ni_place_equal_pts,GRPEqPnAlE
class           COnpt
...
state start
   message_key     GRC_M_PlcEqSpcPtsEle
   prompt_key      GR_P_EntNumPtsPlc
   filter          get_event
   construct_class "GR3dpoint"
   on SCALAR < 3  
        do status_key GR_E_NumLT3
                                                     state .
   on SCALAR        
   ...
   on EX_DATA
        ...
        do isopen
           on SUCCESS
                do erase_hilite
                do get_construct_env_from_dpb
                do locate_two_and_construct            state start

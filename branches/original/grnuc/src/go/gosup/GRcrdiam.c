...
extern    IGRboolean    GRcrdiam_rtr();
extern    IGRboolean    GRprerout();
extern    IGRboolean    GRccschoice();

IGRboolean GRcrdiam(msg,events,num_events,cr)
...

{
  ...
  *msg = MSSUCC;
  ...
  max_val = 0;					/* max number of vael for cr */
  ...
  if( *num_events <= GRMAX_CR_EVENTS )		/* valid number of events?   */
      ...
      if( GRprerout(msg,events,num_events,&max_id,&max_val,rtevents) )
	{
	  ...
	  GRccschoice(msg,&level,rtevents,num_events,&view,&ccw,GRcrdiam_rtr,
		      cr);
        }
    }
  else

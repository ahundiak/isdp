...
#define va_start(__list, __parmN) (__list = (char *) \
 ( \
  (__builtin_alignof(__parmN) == 8) ? \
   (((int)&__parmN + sizeof(__parmN) + 8 - 1) & -8 ) : \
   (((int)&__parmN + ((sizeof(__parmN) > 4)?sizeof(__parmN):4) + 4 - 1) & -4) \
  ) \
 )
...
#define va_arg(__list, __mode) ((__mode *)(__list = (char *) \
    ( \
      (__builtin_alignof(__mode) == 8) ? \
       (((int)__list + sizeof(__mode) + 8 - 1) & -8 ) : \
     (((int)__list + ((sizeof(__mode) > 4)?sizeof(__mode):4) + 4 - 1) & -4) \
    ) \
    )) [-1]

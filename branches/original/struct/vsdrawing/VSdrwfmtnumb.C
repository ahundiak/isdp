...
/*----------------------------------------------------------------------------*/
int VSformatNumber( format, d, output, size, errFile )
	...
	*tmp = '\0' ;
	...
	if( endstr == endfmt ) {
		...
		if( endstr == endfmt ) {
			...
			fprintf( errFile, "\n" ) ;
			VSdrwPrintf( "!!! Bad decimal part for number : '%s'\n",
					format ) ;
		}
	} else if( errFile ) {
		fprintf( errFile, "\n" ) ;
		VSdrwPrintf( "!!! Bad format for number : '%s'\n", format ) ;
	}
	if( *tmp ) {
		int	truncated ;

		for( p = tmp ; ' ' == *p ; p++ ) ;
		VSstrncpy( output, p, size, &truncated ) ;
		if(    delimiter

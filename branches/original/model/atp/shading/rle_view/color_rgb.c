#ifndef X11
/*  color_rgb:  entry points for init_rgb and index_rgb.
 *
 *    init_rgb:  initializes the rgb-to-vlt-index lookup table and downloads
 *      an appropriate corresponding vlt, given the number of bit planes.
 *
 *    index_rgb:  returns a vlt index, given x- & y-coordinates for a pixel
...
static struct vlt_slot fixed_vlt[14]; /* space to hold original fixed VLT for GX/GZ machines */
...
#define BLUM 114	/*   used to determine best match for menu colors */
    ...
    int  i, j, k, red, grn, blu,		/* loop indices */
         ...
         num_shds;	/* no. of "base" shades for particular no. of planes */
    ...
    int  slot_num,		/* index for current slot number */
         ...
         delta_red, delta_grn, delta_blu,    /* red, green, and blue deltas */
         dist,		/* "distance squared" between 2 colors */
         best_match,		/* index for "closest" color that matches */
    ...
    if (num_planes < 8)
      {
        menu[ 0] = 0x000000;    menu[ 1] = 0x00ba00;
        menu[13] = 0xbbbbbb;    menu[14] = 0x888888;    menu[15] = 0xddeeee;

        m_priority[ 0] = 14;    m_priority[ 1] = 15;    m_priority[ 2] = 13;
        m_priority[ 3] =  0;    m_priority[ 4] =  1;
      }
    else
     {
	Readvlt( vs_num, fixed_vlt, 14, 1 );
        menu[ 0] = 0x000000;    menu[ 1] = 0xeeeeee;    menu[ 2] = 0x888888;
        menu[ 3] = 0x000000;    menu[ 4] = 0x00ffff;    menu[ 5] = 0xaaaaaa;
        menu[ 6] = 0x555555;    menu[ 7] = 0xffffff;    menu[ 8] = 0x0000ff;
        menu[ 9] = 0x00ff00;    menu[10] = 0xff0000;    menu[11] = 0xbbffff;
        menu[12] = 0x0000dd;    menu[13] = 0x00bb00;    menu[14] = 0xcc0000;

        m_priority[ 0] =  6;    m_priority[ 1] =  2;    m_priority[ 2] = 11;
        m_priority[ 3] = 13;    m_priority[ 4] =  1;    m_priority[ 5] = 12;
        m_priority[ 6] =  5;    m_priority[ 7] = 14;    m_priority[ 8] =  0;
        m_priority[ 9] =  7;    m_priority[10] =  3;    m_priority[11] =  4;
        m_priority[12] =  8;    m_priority[13] =  9;    m_priority[14] = 10;
      }
    ...
    if (num_planes < 3)	/* not enough planes */
      ...
      {
	base_shade[1] = 0xba;	num_shds = 3;
      }
    else if (num_planes < 8)    /* num_planes = 6 (or 7) */
      {
	base_shade[1] = 0x9b;	num_shds = 4;
	base_shade[2] = 0xd4;
      }
    else if (num_planes == 8)
      {
	base_shade[1] = 0x7b;	num_shds = 6;
	base_shade[2] = 0xa8;
	base_shade[3] = 0xca;
	base_shade[4] = 0xe6;
      }
      ...
      {
	base_shade[1] = 0x69;	num_shds = 8;
	base_shade[2] = 0x90;   num_planes = 9;
	base_shade[3] = 0xad;
	base_shade[4] = 0xc6;
	base_shade[5] = 0xdb;
	base_shade[6] = 0xee;
    ...
    for (i = 0; i < 128; i++)
	  ...
	  i,rgb_tbl[i],i+128,rgb_tbl[i+128],i+256,rgb_tbl[i+256],i+384,
	  rgb_tbl[i+384]);
    printf ("\nvlt\n");
    printf ("---\n");

/*****************************************************************/
/* FIproto_pub.h -						 */
/*	This is the I/Forms ( FORMS SN01014 ) ANSI-C declaration */
/*	header file, for the forms and menu functions.		 */
/*****************************************************************/
...
#ifndef FI_PROTO_PUB_H
#define FI_PROTO_PUB_H 1
...
extern "C" {
	...
	/*** include "tools.h" if it hasn't already been included ***/
#ifndef	KEYBOARD_EVENT
#include "tools.h"
#endif

	/*** include "FI.h" if it hasn't already been included ***/
#ifndef FI_H_INCLUDED
#include "FI.h"
#endif

	/*** include "FIdyn.h" if it hasn't already been included ***/
#ifndef FI_DYN_H
#include "FIdyn.h"
#endif
...
/******************************************************************/
/* NOTES:							  */
/*  1.  01/14/92 --- I/Forms version 2.0.2.31 --		  */
/*	Some of the function declarations below have been	  */
/*	changed so that the "float" arguments are now "double".	  */
/*	This is because the I/Forms source code actually	  */
/*	declares them as "float", but since the I/Forms code is	  */
/*	non-ANSI C ( and will not be ANSI-ized before its	  */
/*	obsolescense ), the compiler promotes "floats" to	  */
/*	"doubles".  This causes problems for ANSI-C applications  */
/*	calling these functions with "floats", because ANSI-C	  */
/*	doesn't promote them to doubles.  Therefore, ANSI-C	  */
/*	applications passing a "float" to these functions would	  */
/*	encounter a memory fault.  So, to fix this, these	  */
/*	declarations in this file ( but not in the real source	  */
/*	code ) have been changed from "float" to "double".	  */
/*	This shouldn't adversely affect anyone.  Those		  */
/*	applications using non-ANSI C don't include this file	  */
/*	anyway, and ANSI-C applications should now be able to	  */
/*	pass a "double" safely to the non-ANSI C I/Forms library. */
/*	( which is really expecting a "double", even though it's  */
/*	actually declared as a "float" ).			  */
/*	[ How's that for "double" talk ?  Or should that be	  */
/*	"float" talk ... EEEK!  I can't take it anymore.... ]	  */
/******************************************************************/
...
extern int (* FI_map_character)	(  char * character );

extern int (* FI_move_index) (  char * string, int index,
...
extern int FI_message_box  ( int screen, int xpos, int ypos,
			     ...
			     char *da_message,	 int message_color,
...
extern int FI_msg_box ( char *title, int title_color, char *da_message,
			int message_color );

extern int FI_msg_box2 ( char *title, int title_color, char *da_message,
			int message_color );
...
extern int FIf_get_user_pointer ( Form form, void *user_pointer );
...
extern int FIf_new ( int label, char *form_name,
		     void (*notification_routine)(), Form *form );
...
extern int FIf_set_button_intercept_routine ( Form, 
               int ((*routine)( Form, int, int, int, int, int )));
...
extern int FIf_set_notification_routine ( Form form, void (*routine)() );
...
extern int FIf_set_user_pointer ( Form form, void *user_pointer );
...
/*** as per NOTE # 1 above -- JAJ:01/14/92			***/
extern int FIg_set_increment ( Form form, int label, double da_increment );
...
extern int FImcf_get_attr ( Form form, int label, int *attr_mask );
...
extern int FImcf_set_attr ( Form form, int label, int attr_mask );

...
IGRint EFdypt6_6f8v( pt1, pt2, pt3, pt4, pt5, display )
 IGRpoint            pt1, pt2, pt3, pt4, pt5;
 struct IGRdisplay   *display;

{
 struct       pt6_struct    header;
 ...
 /**MOVE THE INPUT POINTS TO THE POLYLINE POINT BUFFER**/
  OM_BLOCK_MOVE(pt2,pt_buffer,size_pnt);   
  OM_BLOCK_MOVE(pt3,&pt_buffer[3][0],size_pnt);    
  OM_BLOCK_MOVE(pt4,&pt_buffer[8][0],size_pnt);
  OM_BLOCK_MOVE(pt5,&pt_buffer[5][0],size_pnt);
 ...
 /*  INVOKE DYNAMICS  */
 dp$dynamics( dyn_fun = EFdyrbpt6_6f8v, information = &header );
...
/*
NAME    EFdyrbpt6_6f8v -->  function called by dp$dynamics to put new
                            stuff in
DESCRIPTION
...
IGRint EFdyrbpt6_6f8v( header, cursor_pt, matrix, objects, num_objects,
 ...
 IGRchar               **out_dummy1, **out_dummy2, **out_dummy3;
{
 ...
 ptr->points[6] = cursor_pt->x + header->h_vec[1][0];
 ptr->points[7] = cursor_pt->y + header->h_vec[1][1];
 ptr->points[8] = cursor_pt->z + header->h_vec[1][2];
 ...
 ptr->points[6] = cursor_pt->x - header->w_vec[0] + header->h_vec[0][0];
 ptr->points[7] = cursor_pt->y - header->w_vec[1] + header->h_vec[0][1];
 ptr->points[8] = cursor_pt->z - header->w_vec[2] + header->h_vec[0][2];

 ptr->points[9] = cursor_pt->x + header->h_vec[1][0];
 ptr->points[10] = cursor_pt->y + header->h_vec[1][1];
 ptr->points[11] = cursor_pt->z + header->h_vec[1][2];

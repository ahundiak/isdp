...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  <xsl:template match="/">
     <table name="Pin Jig Panel Report" description="Pin Jig Panel Report">
        <xsl:for-each select="SMS_PRS/SMS_PRS_UNIT/SMS_PINJIGS/SMS_PINJIG">
           <layout>
	      <column title="Panel Pin Name" column_no="1"/>
	      ...
	      <column title="Nearest Pin"    column_no="5"/>
           ...
           <xsl:for-each select="SMS_PANEL_CORNER_DATA/CORNER_DATA" order-by="NAME">
	      <row>

...
DB_dump_rtree(root_page_p, levels_p)
  ...
  {
  R_PAGE_2D_INT *cur_page_p, *child_page_p;
  int i, j;
  char *status;
  OM_S_OBJID tobj;
  OM_S_RIP rip;
  ...
  printf("\nTABLE\n");
    ...
    for (i = 0; i < cur_page_p->Header.Key_count; i += 5)
      ...
      for (j = i; j < i + 5 && j < cur_page_p->Header.Key_count; j++)
        {
        printf("xmin:\t%d", cur_page_p->Key[j].xmin);
        if (j < i + 4) printf("\t");
        }
      printf("\n");
      for (j = i; j < i + 5 && j < cur_page_p->Header.Key_count; j++)
        {
        printf("ymin:\t%d", cur_page_p->Key[j].ymin);
        if (j < i + 4) printf("\t");
        }
      printf("\n");
      for (j = i; j < i + 5 && j < cur_page_p->Header.Key_count; j++)
        {
        printf("xmax:\t%d", cur_page_p->Key[j].xmax);
        if (j < i + 4) printf("\t");
        }
      printf("\n");
      for (j = i; j < i + 5 && j < cur_page_p->Header.Key_count; j++)
        {
        printf("ymax:\t%d", cur_page_p->Key[j].ymax);
        if (j < i + 4) printf("\t");
        }
      printf("\n");
      for (j = i; j < i + 5 && j < cur_page_p->Header.Key_count; j++)

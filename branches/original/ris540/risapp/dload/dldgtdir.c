	...
	rissqlca	*dbcap)			/*  For errors */
{
	...
	char	*config_file = RISCONFIG_FILE;
#if defined(unix)
	char	buffer[512];
	...
	if (RISdld_debug)
	{
		RISdld_output_debug("RISdld_get_risdir()\n");
		RISdld_output_debug("\tprodname:<%s>\n", STRING(prodname));
		RISdld_output_debug("\tvers_maj:%d vers_min:%d vers_rel:%d mode:%d\n",
			version_maj, version_min, version_rel, mode);
		RISdld_output_debug("risdir:0x%x development:0x%x dbcap:0x%x\n",
	...
	*/
	else if (risdir_file && ((fd = fopen(risdir_file,"r")) != NULL))
	{
		...
		if (!cp)
		{
			DLD_DBG(("RISdld_get_risdir:exiting fgets error reading %s\n",
					 risdir_file));
	...
	*/
	else
	{
		...
		{
			syserr(errno, dbcap);
			DLD_DBG(("RISdld_get_risdir:exiting CANT OPEN INGRCONFIG\n"));
	...
	*/
   	else
	{
		...
		DLD_DBG(("RISdld_get_risdir:Opening Reg. key <%s>\n", szKey));
		...
		if (dwNumSubKeys)
		{
			DLD_DBG(("RISdld_get_risdir:found %d Subkeys\n", dwNumSubKeys));
  	...
  	else
	{
		...
		*/
		...
		if (strcmp(prod_dir,"")==0)
		{
			...
			DLD_DBG(("Getting the version value\n"));
...
extern void RISdld_generate_path(
	char *ris_objects,
	char *ris_dir,
	int	 development)
{
	if( development )
	{
		sprintf(ris_objects, "%s%c%s%c%s%c%s%c%s", ris_dir, PATH_CHAR, "risapp", 

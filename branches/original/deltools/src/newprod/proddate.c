	...
	if (pptr = (struct prodnode *)calloc(1, sizeof(struct prodnode))) {
		 ...
		 */
 		if ((srcmedia == NETCDROM || srcmedia == TCPIP ||
		     srcmedia == DIRECTORY || srcmedia == CDROM ||
 ...
 */
void	get_prod_status(pptr, header)
struct prodnode *pptr, *header;
{
    register int	status;
    register struct prodnode *lpptr;
    struct prodnode *tpptr = pptr;
    int		firstpass = 1;
    int		match = 0;
    int		i;
    char	*c;
	...
	if (status < 0) {
	    ...
	    if (makenode && (strcmp(lpptr->prodnumber, pptr->prodnumber) ||
		(lpptr->arch != pptr->arch)))
	      continue;

	    match = 1;

	    if (!firstpass) {
		tpptr = (struct prodnode *) malloc(sizeof (struct prodnode));
		memcpy(tpptr, pptr, sizeof(struct prodnode));
	    }
	    strcpy(tpptr->loadeddir, lpptr->loadeddir);
	    strcpy(tpptr->loadedvers, lpptr->loadedvers);
	}
	else if (status > 0) {
	    /*
	...
	if (makenode) {
	    if (tpptr->loadeddir[0])
		sprintf(file, "%s/%s", tpptr->loadeddir, PRODDEF);
	    else {
		sprintf(file, "%s/%s", basedir, tpptr->prodname);
		/*
	    ...
	    if (tpptr->loadeddir[0])
		sprintf(file, "%s/%s", tpptr->loadeddir, PRODDEF);
	    else if (tpptr->unixdir[0] == '/')
		sprintf(file, "%s/%s", tpptr->unixdir, PRODDEF);
	    else
		sprintf(file, "%s/%s/%s", basedir, tpptr->unixdir, PRODDEF);
	}

	tpptr->pstat &= 0xff00;		/* clear out lower 4 bits */
 ...
 */
	    status = compare_dates(IDdate, pptr->date);
	    if (status == 0)
		tpptr->pstat |= UP_TO_DATE;
	    else if (status > 0)
		tpptr->pstat |= WORKSTATION_NEWER;
	    else
		tpptr->pstat |= NEEDS_UPDATING;
	}
	else if (status > 0)
	    tpptr->pstat |= WORKSTATION_NEWER;
	else
...
insert_node:
	if (match) {
	    if (!firstpass) {
		ins_prod_node(tpptr, header, 1);
		ins_class(tpptr, &classheader);
	    }
	    else
		firstpass = 0;
	}
 ...
 * SS01191#ACC#Apogee C Compiler#05.00.01.27#07-NOV-1990#/usr/ingr/comp/acc#
 */

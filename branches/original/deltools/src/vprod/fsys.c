 ...
 * in the parameter "ldirname".
 */
int	makedir(ldirname, mode, owner, group)
char	*ldirname;
int	mode;
uid_t	owner;
gid_t	group;
{
	...
	if (mkdirp(ldirname, mode) == -1) {
	    if (errno != EEXIST) {
		PRINT "ERROR: Could not create directory %s: %s", ldirname, sys_errlist[errno]);
		LPRINT "ERROR: Could not create directory %s: %s\n", ldirname, sys_errlist[errno]);
		if (verbose)
			PRINT ".\n");
		return(-1);
	    }
...
#if defined(SYSV) || defined(SunOS_4) || defined(OSF)
/*

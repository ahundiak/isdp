/* $Id: GRprocsetadp.C,v 1.1.1.1 2001/01/04 21:07:28 cvs Exp $  */
 ...
 *
 * Revision History:
 *      $Log: GRprocsetadp.C,v $
 *      Revision 1.1.1.1  2001/01/04 21:07:28  cvs
 *      Initial import to CVS
 *
 * Revision 1.1  1998/09/29  17:13:28  pinnacle
 * Created: emsfixes/grnuc/GRprocsetadp.C by vsrivast for vds
 *
 *
 * History:
 ...
 *       dhm     	09/19/91        Updated to invoke the CO define linestyl					e instead of the linestyle subform.
 ...
 *       mrm     	09/20/93    	call GRupdate_adp with NULL form pointer 					to find all bar menus
 *       Vinit		09/28/98    	TR 179800998. Converted the Set Active D					rawing Parameters form to permanent wind					ow to make the SmartSketch form active w					hile the main form exists.
***************************************************************************/ 
/* Name
        GRprocess_setadp
...
History
	...
	AIC     07/10/91    Commented out FI error messages.
        ...
        mrm     09/20/93    call GRupdate_adp with NULL form pointer to find
                            all bar menus
	Vinit	09/28/98    TR 179800998. Converted the Set Active Drawing Param			    eters form to permanent window to make the SmartSket                            ch form active while the main form exists.
*/
...
Form form_ptr;   /* pointer to the form whose gadget was updated */
{
    ...
    switch (form_label)
    {
        case SET_ADP_FORM_LABEL:
		    ...
		    /* The command may not be terminated if the form is 
		       a permanent window. Hence the subsequent changes
		       */
		    /*** Changes for TR# 179800998 ****/
		    /** Portion commented because if the form is converted 
			to a permanent window as was needed for this TR
			then on exit the stack doesn't clear up. To take care 
			of this for all cases the code in the body of the for 
			loop is executed.
			**/
		/**
...
*/
		  ...
		  /** Change for TR #179800998 - Vinit **/
		  /** The form has been converted to a permanent window
		      to make the Smartsketch form active while the 
		      Set Active Drawing Parms form is displayed **/

		FIf_cvrt_to_perm_win( setadp_form_ptr );

...
/*----------------------------------------------------------------------------*/
int VD_get_gs_content( msg, gs, keepOSes, OScount, keepClasses, filter, args,
		      p_count, p_content, p_notCount, p_notKeep )

/*
 * This function is interfaced by macro vd_$get_gs_content.
 */
...
struct GRobj_env	**p_content,
	...
	for( u = v = w = 0 ; u < total ; u++ ) {
		 ...
		 */
		if( keepIt && CLchk ) {
			keepIt = vd_$is_object_of_classes(
						objid		= elmObjid,
						osnum		= elmOsnum,

/* $Id: SMCmdDw.sl,v 1.1.1.1 2001/01/04 21:07:34 cvs Exp $  */

/* --------------------------------------------------------------------
 * I/VDS
 *
 * File:	vds/smdw/cmd / SMCmdDw.sl
 *
 * Description:
 *
 * Dependencies:
...
/****************************************************************************
	...
	Author:		AD Zuurhout 
	Type:		Slic File
	Process by:	$EXNUC/bin/slic SMCmdDw.sl
	Output:		SMCmdDw.S  and SMCmdDwi.I
	makemake file	SMcoppl.m -> makefile -> ../lib/SMDwght.o

	Description:

	  Slic file to produce methods, necessary for the user interface
	  of the dead weigth values for the Space Management Product.

	History:
	ADZ 	01-09-1992	Creation date
	ADZ	25-09-1992	Set working directory as default, even if
				no default filename is given.
				Error in case SMAN_DW_FILENAME of form_not.
...
command_string	SMC_M_CrDdWg, 0, SMLdDdWt
class		SMCmdDw
...
/*
  ...
  creation of the "Create (Modify) Dead Weight Command"
...
instance
{
  ...
  IGRchar  Workdir[MAX_CHAR_LINE];	/* Name of location where the 
					   Dead Weight information will be
...
#define	SMAN_DW_FILENAME	13
#define SMAN_DW_WORKDIR		15
#define SMAN_DW_OVERRIDE	17
...
action wakeup
{
  ...
  FIg_set_text( me->forms[0].form_ptr, SMAN_DW_FILENAME, me->Filename );
  FIg_set_text( me->forms[0].form_ptr, SMAN_DW_WORKDIR,  me->Workdir );
  FIg_set_state(me->forms[0].form_ptr, SMAN_DW_OVERRIDE, me->Override );

  /* display the form */
  if ( me->forms[0].form_ptr )
    FIf_display( me->forms[0].form_ptr );
  else{
    ex$message( msgnumb = SM_E_FrmNtFnd, type = "%s", var = `"SMdw"` );
    return( OM_E_ABORT );
  }
}
...
struct	GRid	dir;
  ...
  me->Filename[0] = '\0';
  strcpy( me->Workdir , "deadweight" );
  me->Override    = 0;
  f_ptr = NULL;
  vd$openfile(	file_name	= "DeadWeight",
		product_name	= "Vds",
		sub_path	= "config",
		cur_dir		= TRUE,
		p_file		= &f_ptr );
  if( f_ptr != NULL ){
    ...
    ex$message( msgnumb = VD_S_NoDefFile, type="%s", var = `"DeadWeight"` );
  }
...
struct	GRid	dir;
  ...
  switch ( gadget_label )
  {
	...
	FIg_set_state( form_ptr, SMAN_DW_OVERRIDE, me->Override );

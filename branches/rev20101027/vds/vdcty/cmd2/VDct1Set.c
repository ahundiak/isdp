/* $Id: VDct1Set.c,v 1.1 2001/01/14 16:01:21 art Exp $  */
 ...
 * Really should be in the tree routines
 */
#if 0
...
static IGRstat activateNode(TGRid *nodeID)
{
  ...
  VDctxGetTree(NULL,nodeID,&treeID);
  ...
  // For the description
  VDctyGetSetTypeInfo(NULL,NULL,&treeID,&setTypeInfo);

  VDfrmSetgText(form,VDCT_FORM_SET_G_TREE_DESC,setTypeInfo.desc);

  // Adjust the doit gadget here
  VDct1CmdResetGadgetList(&treeID,form,VDCT_FORM_SET_G_DOIT,1);
...
static IGRstat activateForm()
{
...
#if 0
  FIg_set_erase_color(form,VDCT_FORM_SET_G_DOIT,FI_RED);
  FIg_set_color      (form,VDCT_FORM_SET_G_DOIT,FI_GREEN);
  FIg_set_erase_color(form,VDCT_FORM_SET_G_TREE_NAME,FI_RED);
  FIg_set_color      (form,VDCT_FORM_SET_G_TREE_NAME,FI_GREEN);
  printf("Got here\n");
#endif
  ...
  // Tree interface
  row = 0;
  for(i = 0; tp = VDctyGetNthTP_ForApp(0,i); i++) {

    VDctyGetSetTypeInfo(tp,NULL,NULL,&setTypeInfo);
...
IGRstat VDct1CmdInitSetCmdInfo(TVDct1CmdInfo *a_cmdInfo)
{
  ...
  //FIg_set_erase_color(formInfo->form,VDCT_FORM_SET_G_DOIT,FI_RED);
  //FIg_set_color      (formInfo->form,VDCT_FORM_SET_G_DOIT,FI_GREEN);

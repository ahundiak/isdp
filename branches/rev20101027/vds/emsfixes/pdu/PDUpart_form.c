 ...
 * I/VDS
 *
 * File: vds/emsfixes/pdu/parts/PDUpart_form.c
 *
 * Description:
 *
 * Dependencies:
 ...
 *       JTS TR MP6350.
 *
 *       Revision 1.6  2002/05/03 16:54:49  jdsauby
 *       JTS TR 6323
 *
 *       Revision 1.5  2002/01/07 18:32:08  jdsauby
 *       JTSMP CR 5865
 *
 *       Revision 1.4  2001/10/29 17:16:28  jdsauby
 *       JTS CR 5552 - jds
 *
 *       Revision 1.3  2001/10/16 15:35:38  jdsauby
 *       JTS MP CR 5536, plus misc  JDS 10/16/01
 *
 *       Revision 1.2  2001/09/07 18:31:02  jdsauby
 *       Final Checkin for AIM / PDU integration  - jds
 *
 *       Revision 1.1  2001/08/15 19:26:01  jdsauby
...
static char RevCm[2];
static char CM_MdcRev[10];
static int  IsMDCRev = 0;
static int  NEW_MODE_FLAG = 0;
static int  SIM_MODE_FLAG = 0;
...
extern int PDMGetAttrsfromSirTable();
extern int SrCMCopySim();
...
void PDUpart_operations_init()
{
   ...
   end_move_to_state_btn_flag = FALSE;
     ...
     // default is on, it should be off for this, SAUBY JTSMP CR 5925
  ...
  Form    fp;            /* Pointer to the form     */
{
    ...
    int  		t_pos = 0;
    int  		t_select = 0;
    static char   *t_text = NULL_STRING;
    char          *t_value_text = NULL;
    char          *t_attr_text = NULL;
    char 		t_LpdDesc[50];
    ...
    switch ( g_label )
    {
	  ...
	  else if (state == 1)
	  {
			  ...
			  (strcmp(partnum, en_itemname) == 0))
		  {
			  ...
			  FIfld_set_text(fp,SIMILAR_MODE_MCF,row,2,filename,FALSE);
	  ...
	  /* added code Pullabhotla 25/08/2000 */	 
	  /* INSERT Catalog, Part, Rev into SIR_PDM_TABLE  */
	  /* for NEW PART or COPY SIMILAR CASE */
	  /* move all this to PDUadd_part.c  SAUBY */
#if 0
	  if(NEW_MODE_FLAG == 1)
	  {
	      w_flow[0] = '\0';
	      status = CMget_wflow_name( refresh->rev_catalog, 
		      refresh->rev_partid,
		      refresh->rev_revision, w_flow);
	      //__DBGpr_str( " w_flow ", w_flow);
	      if(!strcasecmp(w_flow, "Y"))
		  status = PDMInsertSirTable(RevCat, RevPart, RevRev);
	  ...
	  if(IsMDCRev)
	  {
	      ...
	      /*  Updates MDCREV, CTXREV, CTXNAME AND CTXTYPE IN SIR_PDM_TABLE */

	      status = SirVDUpdCMAttrs( RevCat, RevPart, RevRev, "mdc_rev", CM_MdcRev);

	      CtxRev[0] = '\0';
	      CtxName[0] = '\0';
	      CtxType[0] = '\0';

	      status = PDMGetAttrsfromSirTable(refresh->rev_catalog,
		      refresh->rev_partid, refresh->rev_revision, CtxRev, CtxName, CtxType);
	      status = SirVDUpdCMAttrs( RevCat, RevPart, RevRev, "ctx_rev", CtxRev);
	      status = SirVDUpdCMAttrs( RevCat, RevPart, RevRev, "ctx_name", CtxName);
	      status = SirVDUpdCMAttrs( RevCat, RevPart, RevRev, "ctx_type", CtxType);

	      if(status) _pdm_debug("could not update CM attrs");
	  }
	  /* end of move stuff to PDUadd_part.c SAUBY */
#endif
	  ...
	  if (PDU_command != PDC_M_CREATE_TOP_DOWN)
	  {
		 ...
		 strings[0] = "PDCpCPr";
	      ...
	      /* added code Pullabhotla 25/08/2000 */
	      /*  NEW PART CASE */
	      /* Move from this form, to PDUadd_part.c */
#if 0
	      if (VDaimIsAimLoaded()) {
		  if(new_mode)
		  {
		  ...
		  if ( similar_mode )
		  {
		      _pdm_debug("in similar_mode");

		      // TLS - Fix for TR#4976
		      //Get the lpd value from the form 
		      //and pass it in to the SrCMCopySimilar function
		      ...
		      FIfld_get_num_rows(fp,ATTRIBUTE_VALUE_MCF, &n_rows);
		      for ( t_row = 0; t_row < n_rows; t_row++)
		      {
			  _pdm_debug("getting attribute name", 0);
			  //__DBGpr_com("getting attribute name");
			  FIfld_get_text_length(fp,ATTRIBUTE_VALUE_MCF, t_row,0,&t_length);
			  t_text = (char *) malloc (sizeof (char) * t_length + 1);
			  memset(t_text, NULL, sizeof (char) * t_length + 1);
			  FIfld_get_text(fp, ATTRIBUTE_VALUE_MCF, t_row, 0, t_length,
				  (unsigned char *)t_text, &t_select, &t_pos);
			  PDUfill_in_string(&t_attr_text, t_text);
			  _pdm_debug("t_attr_text = <%s>", t_attr_text);
			  //__DBGpr_str("t_attr_text", t_attr_text);

			  if(   (strncmp(t_attr_text, "lpd",3) == 0) ||
				  (strncmp(t_attr_text, "LPD",3) == 0) ||
				  (strncmp(t_attr_text, "Lpd",3) == 0) )
			  {
			      _pdm_debug("getting value text", 0);
			      //__DBGpr_com("getting value text");
			      FIfld_get_text_length(fp,ATTRIBUTE_VALUE_MCF,t_row,t_col,&t_length);
			      t_text = (char *) malloc (sizeof (char) * t_length + 1);
			      memset(t_text, NULL, sizeof (char) * t_length + 1);
			      FIfld_get_text(fp, ATTRIBUTE_VALUE_MCF, t_row, t_col, t_length,
				      (unsigned char *)t_text, &t_select, &t_pos);
			      PDUfill_in_string(&t_value_text, t_text);
			      if (t_value_text == NULL)
				  PDUfill_in_string1(&t_value_text, "");
			      _pdm_debug("t_value_text = <%s>", t_value_text);
			      //__DBGpr_str("t_value_text", t_value_text);

			      // TLS - create a string of lpd values
			      if(strcmp(t_value_text,"Y") == 0)
			      {

				  t_number  = t_number + 1;
				  sprintf(t_LpdtempDesc,"%d",t_j+17);

				  if(t_number>1)
				  {
				      strcat(t_LpdDesc, ",");
				  }
				  strcat(t_LpdDesc, t_LpdtempDesc);
				  if(!strcmp(t_LpdtempDesc, "28"))
				      strcat(t_LpdDesc, "AF");
		      ...
		      /*	cm_mdc_rev[0] = '\0';
				status = SrCMCopySim(cm_mdc_rev, t_LpdDesc);
				strcpy(CM_MdcRev, cm_mdc_rev);
				if ( status )
				{
				_pdm_debug( "Could not Revise M D C.");
				break;
				}
		       */	
		      strcpy(RevCat, refresh->new_catalog);
	      ...
	      /* End of stuff moved to PDUadd_part.c */
#endif
	  ...
	  // Good idea, we did it.  JDS, TLS  09/04/01
	  // Moved it out, SAUBY
#if 0
	  if ( (status == PDM_S_SUCCESS) && VDaimIsAimLoaded() )
	  {
	      if ( similar_mode )
	      {
	  ...
	  if (status == PDM_S_SUCCESS)
	  {
	      FIg_get_state(PDU_form_id,SIMMODE_COPY_TGL,&state);
	      if (state == 1) {
		  // SAUBY, JTSMP CR 5925, want to setup filename info here
		  // to suit the new default of copy/checkin
		  if ((strcmp(ep_catalog, "") == 0) ||
			  (strcmp(en_itemname, "") == 0) ||
			  (strcmp(ep_rev, "") == 0))
		  {
	  ...
	  //JDS, make sure these get set back
	  PDU_simmode_copy = FALSE;
	  PDU_simmode_checkin = FALSE;
	  ...
	  // default is on, it should be off for this, SAUBY JTSMP CR 5925
	  FIg_set_state_on(forms.part_operations_form_id,SIMMODE_COPY_TGL);
	  ...
	  //SAUBY, JTSMP CR 5925.  Want default of these to be on
	  //FIg_erase(PDU_form_id,SIMMODE_CHECKIN_TGL);
	  //FIg_set_state_off(forms.part_operations_form_id,SIMMODE_COPY_TGL);
	  ...
	  //JTSMP CR 5925, SAUBY need these set to true
	  //PDU_simmode_copy = FALSE;
	  //PDU_simmode_checkin = FALSE;
	  PDU_simmode_copy = TRUE;
	  PDU_simmode_checkin = TRUE;
...
PDU_list_cat = FALSE;
PDU_add_files = FALSE;
//SAUBY, JTSMP CR 5925, need these set to TRUE
PDU_simmode_copy = FALSE;
PDU_simmode_checkin = FALSE;
//PDU_simmode_copy = TRUE;
//PDU_simmode_checkin = TRUE;

...
#define OPP	"$(EXNUC)\\bin\\opp"
#define OMCPP	"$(EXNUC)\\bin\\omcpp"
#define CC	"cl"
#define OCFILES	"$(EXNUC)\\bin\\ocfiles"
...
{
#if defined(UNIX)
         if( !strcmp(path, "/usr/include") 
#if defined(SUNOS4)
		|| !strcmp(path, "/usr/5include")
#if defined(__STDC__)
		|| !strcmp(path, "/usr/sunc/lang/SC1.0/ansi_include")
#endif
#endif
         )
         	return 1;
	...
	while (dir = getNextEnvIncludeDir())
		if (!strcmp(path, dir))
			found = 1;	/* need to continue loop to reset for next call */
	return found;
#else
#error Unknown OS
#endif
...
read_special(pnode, infile, endstring)
struct dtree_node **pnode;
FILE              *infile;
char		  *endstring;
{
...
char  *cf;
{
   ...
   if (is_absolute_path(fn))
   {
      ...
      {
         fprintf (stderr,"1 Cannot find file <%s> included by <%s>\n", fn, cf);
         ++Errors;
      }
      return (0);
   }
   else
   {
      if (mode == 'S')
      {
	    ...
	    if( !generate_filename( p->fname, fn )  &&  is_file(file_name) )
         ...
         {
           fprintf (stderr,"2 Cannot find file <%s> included by <%s>\n", fn, cf);
           ++Errors;
         }
         return (0);
      }
      else
      {
	    ...
	    /* printf("file_name %s\n",file_name); */
         ...
         if (!ignore || !find_name(ignore, fn))
         {
           fprintf (stderr,"3 Cannot find file <%s> included by <%s>, Mode %c\n", fn, cf, mode);
           ++Errors;
         }
         return (0);
      }
...
FILE *infile;
{
   ...
   stat = fread_infile (inline, infile);
   while (1)
   {
      if (stat == 0) break;

      if      (!strcmp ("INCLUDE", inline))
      {
      ...
      else if (!strncmp ("SOURCE", inline, 6))
      {
         src_group = get_src_group(inline+6, "SOURCE");
      ...
      else if (!strcmp ("DEPENDENCIES", inline))
      {
      ...
      else if (!strncmp ("LINKLIB", inline, 7))
      {
         *mk = (struct keyword_node *) calloc(1, KEYWORDSIZE);
         (*mk)->keycode = 6;
         new_keyword = 1;
         ...
         ci = &(get_src_group(inline+7, "LINKLIB"))->linklib;
      }
      else if (!strcmp ("COPT", inline))
      {
         *mk = (struct keyword_node *) calloc(1, KEYWORDSIZE);
         (*mk)->keycode = 7;
         new_keyword = 1;
      ...
      else if (!strcmp ("OPPFLAGS", inline))
      {
         *mk = (struct keyword_node *) calloc(1, KEYWORDSIZE);
         (*mk)->keycode = 8;
         new_keyword = 1;
      ...
      else if (!strncmp ("LIB", inline, 3))
      {
         ...
         *mk = (struct keyword_node *) calloc(1, KEYWORDSIZE);
         ...
         ci = &(get_src_group(inline+3, "LIB"))->libname;
#elif defined(NT)
...
#error Unknown OS
      ...
      else if (!strncmp ("EXEC", inline, 4))
      {
         *mk = (struct keyword_node *) calloc(1, KEYWORDSIZE);
         ...
         ci = &(get_src_group(inline+4, "EXEC"))->execname;
      }
      else if (!strncmp ("ARCHIVE", inline, 7))
      {
         *mk = (struct keyword_node *) calloc(1, KEYWORDSIZE);
         (*mk)->keycode = 16;
         new_keyword = 1;
         ...
         ci = &(get_src_group(inline+7, "ARCHIVE"))->archive;
         has_archive = 1;
      }
      else if (!strcmp ("NOSUFFIXES", inline))
      {
         nosuffixes = 1;
         new_keyword = 0;
         convert_filename = 0;
      ...
      else if (!strcmp ("SUFFIXES", inline))
      {
         int   c;
         while(isspace(c = fgetc(infile))) {}
         ungetc(c, infile);
      ...
      else if (!strcmp ("POST_SPECIAL",inline))
      {
         int   c;
         while(isspace(c = fgetc(infile))) {}
         ungetc(c, infile);
      ...
      else if (!strncmp ("DEPLIB", inline, 6))
      {
         *mk = (struct keyword_node *) calloc(1, KEYWORDSIZE);
         (*mk)->keycode = 23;
         new_keyword = 1;
         convert_filename = 1;
         is_executable_name = 0;
         ci = &(get_src_group(inline+6, "DEPLIB"))->deplib;
      }
      else if (!strcmp ("CCFILTER", inline))
      {
         int  c;
         while(isspace(c = fgetc(infile))) {}
         ungetc(c, infile);
      ...
      else if (!strcmp ("VERBOSE_RULES", inline))
      {
         verbose_rules = 1;
         new_keyword = 0;
         convert_filename = 0;
      ...
      else if (ci)
      {
         ...
         if (*ci)
            while ((*ci)->next)
               ci = &((*ci)->next);
         if (convert_filename)
         	convert_to_native_filename(inline, is_executable_name);

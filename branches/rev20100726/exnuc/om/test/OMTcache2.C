...
strcpy(chan_n, "tst1.chan1");

#ifdef DEBUG
printf("\n*** Construct tst objects ***\n");
#endif

sts=om$construct(classname="tst0", osnum=OM_Gw_current_OS,
		p_objid=&tst_objid[0]);
...
for(i=1; i<MAXTST; i++)
{
    ...
    sts=om$construct(classname="tst1", osnum=OM_Gw_current_OS,
			p_objid=&tst_objid[i]
			);
...
#ifdef DEBUG
om$get_classid(classname="tst1",p_classid=&classid);
printf("\nconstructed class: tst1  classid: %d\n", classid);
#endif

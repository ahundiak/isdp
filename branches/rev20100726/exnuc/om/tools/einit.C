 ...
 * Set of defines
 */
#define OMVAL_OBJSIZE_INVALID 	0x1

/*
 * A few of my comments : - Srikanth
 * 
 * Beware of GETOBJSIZE macro. It is not similar to OM_GETOBJSIZE.
 * It returns the size + 8 (Note the + 8). Hence, even if we pass a size
 * of 0, this chap cooly retuns 8. Hence, OM programmers, be warned.
 */
...
/* OM validation stuff - Srikanth */
unsigned int omval_errno = 0;
unsigned int omval_errsts = 0;
FILE         *errfp = NULL;
char         errfile[1024];
...
unsigned int   oid;
{
   ...
   unsigned long                 begin_fp = 0, remember = 0;
   int                           offset = 0;
   OM_S_OBJID                    temp_objid = 0;
...
/*
printf("Srik: OSCO oid, fp : %d %d\n", pOSCO->oid, objTable[pOSCO->oid].fp);
*/
   if (!err_logo)
   {
      err_logo = 1;
...
/*
printf("Srik: component %d 's classid, offset is %d %d\n",ii, component_list[ii].classid, component_list[ii].offset);
printf("Srik: OSCO's oid is %d\n", oscobyid[component_list[ii].classid]->oid);
*/
...
{
begin_fp = objTable[oid].fp + 8 + component_list[ii].offset +
                chan_info[j].offset;
/* printf("oid, begin_fp : %u %u\n", oid, begin_fp); */
remember = ftell(osfp);
fseek(osfp, begin_fp, 0);
if(fread(&offset, 1, 4, osfp) != 4)
   printf("Error while reading channel offset\n");
#ifdef BIG_ENDIAN
         swapint(&offset, &tempint);
         *pp_chan = (OMuint)tempint;
#endif
/* printf("value of offset : %d\n", offset); */
begin_fp += offset;
/* printf("Channel begin : %u\n", begin_fp); */
}
...

{
   ...
   if ( (type & CD_restricted) && ((type & CD_1_1) || (type & CD_m_1)) )
      goto jmp1;
/*
      printf("Srik: type(%d), flags(%d) for oid %d\n",type, p_chan->flags, oid);
*/
      if ( (p_chan->flags & 0x47e) != type )
      {
         printf("Om Val : type(%d) does'nt match flags(%d) for oid %d\n",
            type, p_chan->flags, oid);
         fprintf(errfp,"Om Val : type(%d) does'nt match flags(%d) for oid %d\n",
            type, p_chan->flags, oid);
         printf("Zeroing out channel offset\n");
         fprintf(errfp, "Zeroing out channel offset\n");
         fseek(osfp, begin_fp - offset, 0);
         temp_store = 0;
         if (fwrite(&temp_store, 4, 1, osfp) != 1)
            printf("Error correcting channel offset\n");

         continue;
      }

      if ( (type & CD_1_1) && ((type & 0x1e) != 2) )
         srik_error = 1;
      if ( (type & CD_m_1) && ((type & 0x1e) != 4) )
         srik_error = 1;
      if ( (type & CD_1_m) && ((type & 0x1e) != 8) )
         srik_error = 1;
      if ( (type & CD_m_m) && ((type & 0x1e) != 0x10) )
         srik_error = 1;

      if (srik_error)
      {
         printf("flags(%d) incorrect for channel with oid %d (type=%d)\n", 
          p_chan->flags, oid, type);
	 fprintf(errfp, "flags(%d) incorrect for channel with oid %d (type=%d)\n",p_chan->flags, oid, type);
         ...
         {
            printf("Error correcting channel offset\n");
            fprintf(errfp, "Error correcting channel offset\n");
         }
...
printf("   Restricted\n");
            ...
            if ((type & CD_1_1) || (type & CD_m_1)) {   /* res1chan */
               count = 1;
               if ( (((int) (*pp_chan)) >> 1) == NULL_OBJID)
                  continue;
               objid = *pp_chan >> 1;
               if ( (objid > highOid) || !objTable[objid].classid )
               {
...
#endif
/*
 * The oid is encoded in the offset field itself (and hence no offset is
 * actually necessary.  If this object is screwed, then store a 
 * null objid there.
 */

 fprintf(errfp, "Offset %u. Restricted channel %d of component %d at offset %u contains invalid target id %u. Object id: %d, Class id: %d\n", 
          objTable[oid].fp, j, ii, ((objTable[oid].fp+8)
          + component_list[ii].offset +  chan_info[j].offset), objid, oid, 
          fil_classid);
 printf("Correcting objid\n");
 fprintf(errfp, "Correcting objid\n");
 temp_objid = NULL_OBJID << 1;
#ifdef BIG_ENDIAN
     swapint(&temp_objid, &tempint);
     temp_objid = tempint;
#endif
/*
 * We have already incremented begin_fp by offset.  But in this case, the 
 * offset field itself contains the object id.  Hence backo !
 */
begin_fp -= offset; 
fseek(osfp, begin_fp, 0);
if (fwrite(&temp_objid, 1, 4, osfp) != 4)
{
   printf("Error in writing null objid\n");
   fprintf(errfp, "Error in writing null objid\n");
                  continue;
}
               }
               p_oid = &objid;
            ...
            }
{
   ...
   if ( (type & CD_1_1) || (type & CD_m_1) )
   {
      if (count != 1)
      {
         printf("OMval: Count in channel header not = 1 for a singleton. oid(%d)\n", oid);
         fprintf(errfp, "OMval: Count in channel header not = 1 for a singleton. oid(%d)\n", oid);
         continue;
      }
   }
}
...
unsigned long      begin_fp;
{
   OMuint             i, oid;
   OMuword            flags;
   OM_S_CHANNUM       channum;
   unsigned long      temp_objid = 0;
...
begin_fp += 4;  /*  begin_fp pointed to the channel header.  Make it point
                 *  to 1st object in the channel.
                 */
 ...
 /*
  * For restricted singletons (1_m/m_1), the repair would have already been
  * performed. Hence, ignore the check below.
  */
...
fprintf(errfp, "Offset %d.  Invalid object id (%d) found on channel (%d,%d)\n        of object %d.  Channel index: %d.\n",
           objTable[my_oid].fp, oid, my_channum.classid, 
           my_channum.number, my_oid, i);

printf("Setting object id to NULL\n");
fprintf(errfp, "Setting object id to NULL\n");
fseek(osfp, begin_fp + i*4, 0);
...
if (fwrite(&temp_objid, 1, 4, osfp) != 4)
{
   printf("Error writing out null objid\n");
   fprintf(errfp, "Error writing out null objid\n");
}
...
int   endfp;
{
   ...
   while (fp < endfp)
   {
	 ...
	 omval_errsts |= OMVAL_OBJSIZE_INVALID;
         return(0);
      }
 ...
 *                                                            *
 *============================================================*/
int validateObjSizes(cluster, tosave)
...
int                        *tosave;
{
   ...
   char              tosave_header = 0, tosave_cluster = 0;
   unsigned int      screwed_objid = NULL;
   unsigned short    screwed_classid = 0;
   static char       err_logo = 0;

   if (!err_logo)
   {
      err_logo = 1;
      fprintf(errfp, "\n\nCluster instance and Object header validation\n");
      fprintf(errfp, "----------------------------------------------\n");
   }
   ...
   while (fp < endfp)
   {
      ...
      screwed_objid = filed_oid[numObjs];
      screwed_classid = filed_classid[numObjs];
      if (objHdr.oid != filed_oid[numObjs])
      {
         ...
         if ( filed_oid[numObjs] != 0) 
         { 
            fprintf(errfp, "Offset %d, cluster %d. Bad object id (%d) objHdr.oid for %dth object.\nObject id should be %d. Class id : %d\n", fp, cnum,
            objHdr.oid, numObjs+1, filed_oid[numObjs]);
            printf("Correcting object id\n");
            fprintf(errfp, "Correcting object id\n");

            objHdr.oid = filed_oid[numObjs];
#ifdef BIG_ENDIAN
            swapint(&objHdr.oid, &tempint);
            objHdr.oid = tempint;
#endif
         ...
         else if (objHdr.oid != 0)
         {
            fprintf(errfp, "Offset %d, cluster %d. Bad object id (%d) objHdr.oid for %dth object.\nObject id should be %d. Class id : %d\n", fp, cnum,
            filed_oid[numObjs], numObjs+1, objHdr.oid);
            printf("Correcting object id\n");
            fprintf(errfp, "Correcting object id\n");

            filed_oid[numObjs] = objHdr.oid;
#ifdef BIG_ENDIAN
            swapint(&filed_oid[numObjs], &tempint);
            filed_oid[numObjs] = tempint;
#endif
            tosave_cluster = 1;
	    screwed_objid = objHdr.oid;
         }
         else
         {
            printf("Oops!! oid in the header and the cluster instance are both corrupt.\n");
            fprintf(errfp, "Oops!! oid in the header and the cluster instance are both corrupt.\n");
         }
    ...
    /*     return(0); */
      ...
      if (objHdr.classid != filed_classid[numObjs])
      {
         ...
         if ( filed_classid[numObjs] != 0) 
         { 
            fprintf(errfp, "Offset %d, Cluster %d.  Bad class id (%d) for %dth object.\nClass id should be %d.  Object id: %d\n", fp, cnum, objHdr.classid, 
            numObjs+1, filed_classid[numObjs], filed_oid[numObjs]);
            printf("Correcting class id\n");
            fprintf(errfp, "Correcting class id\n");

            objHdr.classid = filed_classid[numObjs];
#ifdef BIG_ENDIAN
            swapshort(&objHdr.classid, &tempshort);
            objHdr.classid = tempshort;
#endif
         ...
         else if (objHdr.classid != 0)
         {
            fprintf(errfp, "Offset %d, Cluster %d.  Bad class id (%d) for %dth object.\nClass id should be %d.  Object id: %d\n", fp, cnum,
            filed_classid[numObjs], numObjs+1, objHdr.classid,
            filed_oid[numObjs]);
            printf("Correcting class id\n");
            fprintf(errfp, "Correcting class id\n");

            filed_classid[numObjs] = objHdr.classid;
#ifdef BIG_ENDIAN
            swapshort(&filed_classid[numObjs], &tempshort);
            filed_classid[numObjs] = tempshort;
#endif
            tosave_cluster = 1;
	    screwed_classid = objHdr.classid;
         }
         else
         {
            printf("Oops!! classid in the header and the cluster instance are both corrupt.\n");
            fprintf(errfp, "Oops!! classid in the header and the cluster instance are both corrupt.\n");
         }

    /*     return(0);  */
      }
      /*
       * Changing the below <= check from 0 to 8. This is to effect
       * calling the function establishObjectLoop if the size
       * becomes 0 (corrupted). Since GETOBJSIZE returns 8 (the minimum)
       * even if we pass 0, we can never detect objects of size 0.
       * - Srikanth
       */

      if (((objsize = (GETOBJSIZE(objHdr.i_size))) <= 8) ||
         ((objsize + fp) > endfp))
      {
         /*  The size is screwed. Now, try to find the next correct
          *  objid/classid combination in this cluster.
          *  This condition holds if object size becomes 0 (most cases)
          *  and if it exceeds the cluster limits.
          */
         if ((objsize = establishObjectLoop(fp, osfp, numObjs, cluster)) == 0)
         {
            printf("Unable to establish object link\n");
      ...
      else if ((objsize - 8) < oscobyid[filed_classid[numObjs]]->component_size)
      {
         ...
         {
            /*
             *  Need to call this function here too in case the object size
             * is screwed but does'nt become zero.
             */
            if ((objsize=establishObjectLoop(fp, osfp, numObjs, cluster)) == 0)
            {
               printf("Unable to establish object link\n");
      ...
      /* now, to put back the correct object header */
      if (tosave_header)
      {
         tosave_header = 0;
         fseek(osfp, fp, 0);
         if (fwrite(&objHdr, 1, sizeof(OM_S_FILEOBJHDR), osfp) !=
            sizeof(OM_S_FILEOBJHDR))
         {
            reportError("\n%6d. Offset %d, Cluster %d.  Error writing back %dth object header after correction.\n        Object id: %d  Class id: %d\n",
               ++errcnt, fp, cnum, numObjs+1, filed_oid[numObjs],
               filed_classid[numObjs]);

            fprintf(errfp, "Offset %d, Cluster %d.  Error writing back %dth object header after correction.\nObject id: %d  Class id: %d\n",
               fp, cnum, numObjs+1, filed_oid[numObjs],
               filed_classid[numObjs]);
            return(0);
         }
      }
      ...
      if (classRefcnt[ii])
      {
         ...
         else if (oscobyid[ii]->OSCO.i_refcnt != classRefcnt[ii])
         {
            if (strcmp(oscobyid[ii]->OSCO.s_name, "GRreffile") &&
               ...
               {
               char            ans[2];
               unsigned long   my_fp = 0, my_remember = 0;
               unsigned int    correct_refcnt = 0;
               ...
               reportError("\n%6d. %d objects of class <%s> (class id: %d) found,\n        but class object reference count is %d.\n",
                ...
                printf("Want to set OSCO's reference count to %d ?\n",
                      classRefcnt[ii]);
                  scanf("%s", ans);
                  if ( tolower(ans[0]) == 'y' )
                  {  
                     correct_refcnt = classRefcnt[ii];
                     my_fp = objTable[oscobyid[ii]->OSCO.OSCO_oid].fp;
                     printf("%s 's osco resides at %u\n",
                         oscobyid[ii]->OSCO.s_name, my_fp);
                     fprintf(errfp, "%s 's osco resides at %u\n",
                         oscobyid[ii]->OSCO.s_name, my_fp);
                     fprintf(errfp,"Setting OSCO's refcount to %d\n", 
                          classRefcnt[ii]);
                     my_remember = ftell(osfp);
                     /* 60 is the offset where refcnt resides */
                     fseek(osfp, my_fp + 60, 0);
#ifdef BIG_ENDIAN
      swapint(&correct_refcnt, &tempint);
      correct_refcnt = tempint;
#endif
                     if (fwrite(&correct_refcnt, 1, 4, osfp) != 4)
                     {
                        printf("Error setting correct refcnt\n");
                        fprintf(errfp, "Error setting correct refcnt\n");
                     }
                     fseek(osfp, my_remember, 0);
                  }
                }
         }
      }
...
struct OMCluster_instance  *c0;
{
   ...
   char           *buf;
   unsigned long  *remember = NULL, fp_remember=0;
   int            tosave=0;
   unsigned       offset=0;
...
int tempint;
   ...
   if (!(remember = (unsigned long *)
       om$calloc(num=highCnum+1, size=sizeof(unsigned long))))
   {
      printf("Cannot allocate remember arra\n");
      return(0);
   }
...
printf("Cluster num : %d\n", cluster[c_index-1]->lcd.w_cluster_num);
            remember[cluster[c_index-1]->lcd.w_cluster_num] = fp_remember;
printf("Cluster's instance fp : %u\n", remember[cluster[c_index-1]->lcd.w_cluster_num]);
...
*/
   for (cnum=jj=1; cnum<=highCnum; cnum++)
   {
      if (clustbyid[cnum])
      {
         if ((count = validateObjSizes(clustbyid[cnum], &tosave)) !=
         ...
         if (tosave)
         {
            unsigned int   *filed_oid, objsize;
            unsigned short *filed_classid;

            tosave=0;
            filed_oid = (unsigned int *) ((char *) &(clustbyid[cnum]->i_filed_oid_list_descr.i_offset) + clustbyid[cnum]->i_filed_oid_list_descr.i_offset); 
            filed_classid = (unsigned short *) ((char *) &(clustbyid[cnum]->w_filed_classid_list_descr.i_offset) + clustbyid[cnum]->w_filed_classid_list_descr.i_offset); 
            objsize = clustbyid[cnum]->i_filed_oid_list_descr.i_count;

printf("cnum,fp : %d %u\n", cnum, remember[cnum]);
            fseek(osfp, remember[cnum], 0);
            fread(&offset, 1, 4, osfp);
#ifdef BIG_ENDIAN
            swapint(&offset, &tempint);
            offset = tempint;
#endif
printf("Oid offset, beginning fp: %u %u\n", offset, remember[cnum]+offset);
            fseek(osfp, remember[cnum]+offset,0);
            if (fwrite(filed_oid, 1, 4*objsize, osfp) != 4*objsize)
            {
               printf("Problem writing corrected cluster no. %d\n",cnum);
            }
            fseek(osfp, remember[cnum]+8, 0);
            fread(&offset, 1, 4, osfp);
#ifdef BIG_ENDIAN
            swapint(&offset, &tempint);
            offset = tempint;
#endif
printf("Classid offset, beginning fp: %u %u\n", offset, remember[cnum]+ 8 +offset);
            fseek(osfp, remember[cnum]+8+offset,0);
            if (fwrite(filed_classid, 1, 2*objsize, osfp) != 2*objsize)
            {
               printf("Problem writing corrected cluster no. %d\n",cnum);
            }
printf("Corrected corrupted filed oid/classid list in cluster %d\n", cnum);
         }
         jj++;
      }
   }
...
drosInit()
{
   ...
   {
      printf("Error opening error file %s. Exitting\n", errfile);
      exit(1);
   }
   fprintf(errfp, "Error log for EMS file %s\n", filename);
   fprintf(errfp, "-----------------------------------------\n\n");
...
/*
 * This is the guy who tries to estimate, calculate, guess and do whatever
 * possible gimmics to put back the object size so that the link is
 * established back.  
 * Returns
 *         ---->  0 on failure
 *         ---->  objsize of the first corrected object on success
 * -Srikanth
 */

int establishObjectLoop(fp, osfp, curObjNum, cluster)
unsigned long		   fp;
FILE                       *osfp;
int                        curObjNum;
struct OMCluster_instance  *cluster;
{
   int            first = 0;
   unsigned long  endfp, hit_fp, size = 0, remember_fp = 0, oid = 0;
   unsigned long  save_size = 0, min_size = 0;
   unsigned int   objid, next_objid, numObjs;
   unsigned short classid, next_classid, cid = 0;
   int            flags = 6, loopObjNum, i;
   unsigned int   *filed_oid;
   unsigned short *filed_classid;
   static char                err_logo = 0;
   ...
   if (!err_logo)
   {
      fprintf(errfp, "\n\nPhysical cluster validataion\n");
      fprintf(errfp, "---------------------------\n");
      err_logo = 1;
   }

   /* 
    * Hardcoding flags as hell to 6 (tagged and has VLA). Need to 
    * determine and then accordingly set the flags to
    * tagged, has VLA, has outrigger.
    */

   loopObjNum = curObjNum + 1;
   filed_oid = (unsigned int *) ((char *) &(cluster->i_filed_oid_list_descr.
      i_offset) + cluster->i_filed_oid_list_descr.i_offset);
   filed_classid = (unsigned short *) ((char *) &(cluster->
      w_filed_classid_list_descr.i_offset) +
   ...
   endfp = cluster->lcd.i_file_ptr + cluster->i_fil_byte_size;
   numObjs = cluster->i_fil_num_objects;
   objid = filed_oid[curObjNum];
   classid = filed_classid[curObjNum];

printf("Cluster %d contains %d objects\n", cluster->lcd.w_cluster_num, numObjs);
printf("Current object num : %d\n", curObjNum);
printf("oid, cid, fp : %x %x %u\n", objid, classid, fp);

   /*
    * Check if the incoming object is the last one in the cluster
    * Need the +1 becos curObjNum starts from 0.
    */
   if ((curObjNum+1) == numObjs)
   {
      size = (endfp - fp + 1) - 8 + flags; 
      fseek(osfp, fp, 0);
#ifdef BIG_ENDIAN
            swapint(&size, &tempint);
            size = tempint;
#endif
      printf("Replacing oid %d's size with %d\n", filed_oid[curObjNum], size);
      fprintf(errfp, "Replacing oid %d's size with %d\n", filed_oid[curObjNum],
        size);
      if (fwrite(&size, 4, 1, osfp) != 1 )
      {
         printf("Error correcting object size.\n");
         fprintf(errfp, "Error correcting object size.\n");
      }
      fseek(osfp, remember_fp, 0);
      return(GETOBJSIZE(size));
   }
   next_objid = filed_oid[loopObjNum];
   next_classid = filed_classid[loopObjNum];

   /*
    * Iteratively search the cluster for a good objid/classid match.
    */

   while (loopObjNum <= numObjs)
   {
printf("Trying obj. no %d with oid %x and cid %x\n", loopObjNum,
           next_objid, next_classid);
      fseek(osfp, fp + 16, 0); // start searching from the object instance
      while( ftell(osfp) <= (endfp - 4) ) 
      {
         if (fread(&oid, 4, 1, osfp) != 1)
         {
            printf("Error reading file while repairing\n");
            fseek(osfp, remember_fp, 0);
            return(0); 
         }
#ifdef BIG_ENDIAN
         ...
         {
            /*
             * Read in classid and see if it matches. Then we can 
             * assume this to be a hit.
             */
            fseek(osfp, -6, 1); // backo to classid
            if (fread(&cid, 2, 1, osfp) != 1)
            {
               printf("Error reading file while repairing\n");
               fseek(osfp, remember_fp, 0);
               return(0); 
...
#ifdef BIG_ENDIAN
            swapshort(&cid, &tempshort);
            cid = tempshort;
#endif
            if (cid == next_classid)
            {
               /*
                * Yes, this IS a hit. (An intelligent guess, of course)
                * Fill the size fields with minimum size from the 
                * OSCO object until the last bad object.
                * For the last bad object, adjust the size field 
                * so that the chain is established.
                */
              
               hit_fp = ftell(osfp) - 8;  // Start of this object  
printf("HIT: next oid, cid at fp %u: %x %x\n", hit_fp, oid, cid);
               fseek(osfp, fp, 0);
               for(i=curObjNum; i<loopObjNum- 1; i++)
               {
                  min_size =  oscobyid[classid]->component_size + 8 + flags;
                  printf("Making oid %d's size %d\n", filed_oid[i], min_size);
                  fprintf(errfp, "Making oid %d's size %d\n", 
                        filed_oid[i], min_size);
...
#ifdef BIG_ENDIAN
                  swapint(&min_size, &tempint);
                  min_size = tempint;
#endif
                  fseek(osfp, 12, 1); // Move to the size field
                  if (fwrite(&min_size, 4, 1, osfp) != 1)
                  {
                     printf("Error writing object size during file repair\n");
                     fprintf(errfp, "Error writing object size during file repair\n");
                  }
                  fseek(osfp, min_size - 8 - flags, 1);
               }
               size = (hit_fp - ftell(osfp) - 8) + flags;
               if (!first)
                  save_size = size;
printf("last size : %x\n", size);
#ifdef BIG_ENDIAN
               ...
               if (fwrite(&size, 4, 1, osfp) != 1)
               {
                  printf("Error writing last object size during file repair\n");
                  fprintf(errfp, "Error writing last object size during file repair\n");
               }

               fseek(osfp, remember_fp, 0);
               return(GETOBJSIZE(save_size));
            }
         }
   ...
   /*
    * Now, we've come to a point, which we suspect the whole cluster to be
    * wiped out.  Hence, put in the minimum sizes for all the objects 
    * and for the last object in the cluster, make the size equal to the 
    * remaining bytes in the cluster.
    */
   ...
   size = (endfp - ftell(osfp) + 1) - 8 + flags;
   fseek(osfp, 12, 1);
#ifdef BIG_ENDIAN
            swapint(&size, &tempint);
            size = tempint;
...
unsigned int calculate_main_checksum()
{
   unsigned int main_checksum = 0;
   int          fp, remember_fp;
   int          bcount, size, ii;
   char         buffer[61440];

   if (firstObjfp >= os_size)
      return(0);

   fp = OMFILEHDRSIZE;
   remember_fp = ftell(osfp);
   fseek(osfp, OMFILEHDRSIZE, 0);
   bcount = os_size - OMFILEHDRSIZE;
   while(bcount)
   {
      if (bcount > 61440)
         size = 61440;
      ...
      if ((int) fread(buffer, 1, size, osfp) != size)
      {
         printf("Read error during checksum calculation\n");
         fseek(osfp, remember_fp, 0);
         return(0);
      }
      bcount -= size;
...
unsigned int calculate_header_checksum()
{
   unsigned int                hdr_checksum = 0;
   ...
   buff = (unsigned char *) &my_filehdr;
   for (ii=hdr_checksum=0; ii<434; ii++)
      hdr_checksum += buff[ii] + ii;
   fseek(osfp, remember_fp, 0);
   return(hdr_checksum);
}

int detect_irrepairable_loses()
{
   return (0);
}

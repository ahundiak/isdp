...
extern void RISdldFRM_report_error(
	char **errorp)
{
	RIS_error_col = 0;
	...
	add_asterisk_line();
	add_string_line("Dynamic Loading of RIS Forms DLL has failed.");
	add_asterisk_line();
	RETURN(errorp);
}
#endif
...
extern void RISdldFRM_error_msg_box(
	int  error_code,
	char *string)
{
	char error_string[512];

	DLD_DBG(("RISdldFRM_error_msg_box(error_code:%d string:%s\n", 
	...
	DLD_DBG(("RISdldFRM_error_msg_box: complete.\n"));
}

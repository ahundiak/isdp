# NEXT EDIT: please add 'whence command' and 'ls -ls command' in the
# next go-round of this script. 4//7/93/ SBC as per DMB.
...
#dmb:04/16/93:Set up multiple platform support
...
in
  ADM )
    ...
    RESULTS_MAILING_LIST="sbcrowel@susie cdsieja@cdyer steve@gondor dmb@tardis jhmartin@msdpo"
    ;;
  CADC )
    INSTR_SOURCE_KEY=CADC
    OUT_FILE_NAME_ROOT=cadc
    RESULTS_MAILING_LIST="sbcrowel@susie cdsieja@cdyer htsai@htsai mbmarrer@pdb"
    ;;
  XFORMS* )
    INSTR_SOURCE_KEY=FB
    OUT_FILE_NAME_ROOT=xf
    RESULTS_MAILING_LIST="sbcrowel@susie dmb@tardis steve@gondor cdsieja@cdyer jhmartin@msdpo"
    ;;
  FORMS*|IFORMS* )
    INSTR_SOURCE_KEY=FB
  ...
  HELP* )
    ...
    RESULTS_MAILING_LIST="sbcrowel@susie cdsieja@cdyer alan@alan dash@skyy bucky@id1377"
    ;;
  UMS )
    INSTR_SOURCE_KEY=UMS
    OUT_FILE_NAME_ROOT=ums
    RESULTS_MAILING_LIST="sbcrowel@susie dmb@tardis steve@gondor cdsieja@cdyer"
    ;;
  LGLASS|LG )
    INSTR_SOURCE_KEY=LG
...
#echo "Converting Report $HELP_SOURCE_NAME to HELP format"
##dmb:03/10/93:Added to get around current HELP implementation limit (CR???)
#set -x
...
#set -x
#${BROWSER} -D $OUT_FILE_NAME -l english -v $REPORT_VERSION
#set +x
#
#

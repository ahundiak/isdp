...
MEMptr   *list_bufr;   /*  modified - value list */
{
     ...
     MEMptr  ValListBufr = NULL;
     char    **val_data ;
     int     list_comma, val_count;
...
*/
    ...
    /* Get default values for the data list and value list buffers */
     ...
     sprintf (sql_str1, 
       "SELECT %s %s  FROM %s WHERE %s AND n_attrno in ( ",
       "n_attrno, n_type, n_defaultvalue, n_defaultno, ",
       "n_minvalue, n_maxvalue, n_listno, n_programno",
       " NFMVALUES ",
       "(n_type = 'C' OR n_type = 'A') ") ;
...
/* This check is probably redundant as we filter them out
   in the query which gets them in the first place.. MaC 010794*/
/*
      if ( (!strcmp (data[count+f], "N")) && 
                 (!strcmp (data[count+g], "N")) ) continue;
*/
/*
      if (!strcmp (data[count+a], "n_itemno")) continue;
      else if (!strcmp (data[count+a], "n_itemname")) continue;
      else if (!strcmp (data[count+a], "n_itemrev")) continue;
      else if (!strcmp (data[count+a], "n_itemdesc")) continue;
      else if (!strcmp (data[count+a], "n_setindicator")) continue;
      else if (!strcmp (data[count+a], "n_versionlimit")) continue;
      else if (!strcmp (data[count+a], "n_status")) continue;
      else if (!strcmp (data[count+a], "n_pendingflag")) continue;
      else if (!strcmp (data[count+a], "n_itemlock")) continue;
      else if (!strcmp (data[count+a], "n_prevstateno")) continue;
      else if (!strcmp (data[count+a], "n_creationdate")) continue;
      else if (!strcmp (data[count+a], "n_creator")) continue;
      else if (!strcmp (data[count+a], "n_updater")) continue;
      else if (!strcmp (data[count+a], "n_updatedate")) continue;
      else if (!strcmp (data[count+a], "n_colevel")) continue;
      else if (!strcmp (data[count+a], "n_archivestate")) continue;
      else if (!strcmp (data[count+a], "n_iconname")) continue;
      else if (!strcmp (data[count+a], "n_lgfiletype")) continue;
      else if (!strcmp (data[count+a], "p_explode")) continue;
      else if (!strcmp (data[count+a], "p_maxlevel")) continue;
      else if (!strcmp (data[count+a], "p_parttype")) continue;
      else if (!strcmp (data[count+a], "p_incpartrpt")) continue;
      else if (!strcmp (data[count+a], "p_mrpbomuom")) continue;
      else if (!strcmp (data[count+a], "p_mrpeffectdate")) continue;
      else if (!strcmp (data[count+a], "p_mrpinvuom")) continue;
      else if (!strcmp (data[count+a], "p_mrpfieldupd")) continue;
      else if (!strcmp (data[count+a], "p_mrppart")) continue;
      else if (!strcmp (data[count+a], "p_mrpstruct")) continue;
      else if (!strcmp (data[count+a], "n_fileno")) continue;
      else if (!strcmp (data[count+a], "n_itemnum")) continue;
      else if (!strcmp (data[count+a], "n_filenum")) continue;
      else if (!strcmp (data[count+a], "n_cofilename")) continue;
      else if (!strcmp (data[count+a], "n_fileversion")) continue;
      else if (!strcmp (data[count+a], "n_filetype")) continue;
      else if (!strcmp (data[count+a], "n_cifilename")) continue;
      else if (!strcmp (data[count+a], "n_cocomment")) continue;
      else if (!strcmp (data[count+a], "p_filedesc")) continue;
*/
      if (comma) 
         strcat (sql_str, ", ") ;
      comma = 1 ;
      /*sprintf (tmp_str, "n_attrno = %s", data[count + h]) ;*/
      _PDMdebug (fn, "adding attrno for [%s]\n", data[count + a]) ;
      strcat (sql_str, data[count + h]) ;
      } 
    ...
    if (PDMdebug_on) 
     ...
     listno_str  = (char *) malloc (200 + 60 * attr_list-> rows) ;
     if (!listno_str) {
	 MEMclose (data_list);
	 MEMclose (list_bufr);
	 free (sql_str1) ;
     ...
     val_data   = (char **) attrs -> data_ptr;

     list_comma = 0;
     listno_str[0] = '\0';
     for (x = 0; x < attrs -> rows; ++x) {
      val_count = x * attrs -> columns  ;
      if (atol (val_data[val_count+6]) > 0) {
        if (list_comma) 
         strcat (listno_str, ", ") ;
        list_comma = 1 ;
        /*sprintf (tmp_str, "n_attrno = %s", data[val_count + h]) ;*/
        _PDMdebug (fn, "adding listno for [%s]\n", val_data[val_count + 6]) ;
        strcat (listno_str, val_data[val_count + 6]) ;
        }
      }

/*
    sprintf (sql_str1, "SELECT %s FROM NFMVALUES a, NFMLISTS b WHERE (a.n_type = 'C' OR a.n_type = 'A') AND (a.n_listno = b.n_listno) AND a.n_attrno in (",
              "a.n_attrno, a.n_listno, b.n_value") ;
*/

   if (strlen (listno_str) > 0) {
    sprintf (sql_str1, "SELECT %s FROM NFMLISTS  WHERE n_listno in ( %s ) ",
              "n_listno, n_value", listno_str) ;
    ...
    status = _PDMCreateValueListBufr (attrs, list_buffer, &ValListBufr);
    if (status != PDM_S_SUCCESS) {
	 MEMclose (&temp_list);
	 MEMclose (data_list);
	 MEMclose (list_bufr);
	 MEMclose (&attrs) ;
	 _PDMdebug (fn, "_PDMCreateValueListBufr: < 0x%.8x >\n", status);
	 return (status);
       }
    MEMclose (&list_buffer);

    list_buffer = ValListBufr;
...
*/
  ...
  if (in_bufr->columns <= 2)  {
    _PDMdebug (fn, "This buffer does not have any dynamic attrs: Skipping..\n");
    continue;
    }
...
*/
    sprintf(sql_str,
     "SELECT t2.p_dyntable FROM nfmcatalogs t1,pdmcatalogs t2 \
      where t1.n_catalogno = t2.n_catalogno AND t1.n_catalogname = '%s'\
      AND NOT (t2.p_dyntable is NULL)",
      catalog);

    status = SQLquery(sql_str, &qry_bufr,512);
    if (status != SQL_S_SUCCESS) {
      _PDMdebug (fn, "SQL Qry for Dyn Table Name failed 0x%.8x\n", status);
      MEMclose (attr_bufr);
      MEMclose (&qry_bufr);
      _PDMdebug (fn, 
      "This catalog [%s] does not have a dynamic table associated with it.\n", 
        catalog);
      _PDMdebug (fn, "EXIT\n");
      return (PDM_S_SUCCESS);
       }
...
int _PDMCreateValueListBufr (attr_bufr, list_bufr, ValListBufr)
MEMptr  attr_bufr;
MEMptr  list_bufr;
MEMptr  *ValListBufr;
{
	char		*fn = "_PDMCreateValueListBufr";
	char		**ListData;
	char		**AttrData;
	char		MemStr[256];
	MEMptr		TmpBufr = NULL;
	int		attr, list, ListIndex, AttrIndex;
...
*/
  ...
  if (!list_bufr) {
      _PDMdebug (fn, "No lists found...\n");
      return (PDM_S_SUCCESS);
   }

   status = MEMopen (&TmpBufr, 1024);
   if (status != MEM_S_SUCCESS) {
      _PDMdebug (fn, "MEMopen failed status <0x%.8x>\n",status);
      return(PDM_E_OPEN_BUFFER);
   }

   status = MEMwrite_format (TmpBufr, "n_attrno", "integer");
   if (status != MEM_S_SUCCESS) {
      ERRload_struct (NFM, NFM_E_MEM, "", NULL);
      _PDMdebug (fn, "MEM Wr Format : status = <0x%.8x>\n",
			status);
   ...
   status = MEMwrite_format (TmpBufr, "n_listno", "integer");
   if (status != MEM_S_SUCCESS) {
      ERRload_struct (NFM, NFM_E_MEM, "", NULL);
      _PDMdebug (fn, "MEM Wr Format : status = <0x%.8x>\n",
			status);
   ...
   status = MEMwrite_format (TmpBufr, "n_value", "char(40)");
   if (status != MEM_S_SUCCESS) {
      ERRload_struct (NFM, NFM_E_MEM, "", NULL);
      _PDMdebug (fn, "MEM Wr Format : status = <0x%.8x>\n",
			status);
      ...
      AttrData = (char **) attr_bufr->data_ptr;
      ...
      ListData = (char **) list_bufr->data_ptr;

      MemStr [0] = '\0';

      for (attr = 0; attr < attr_bufr->rows; attr++) {
      AttrIndex = attr * attr_bufr->columns;
       for (list = 0; list < list_bufr->rows; list++) {
       ListIndex = list * list_bufr->columns;
       if (!strcmp (AttrData[AttrIndex+6], ListData[ListIndex] )) {
         MemStr [0] = '\0';
         sprintf (MemStr, "%s\1%s\1%s\1", 
                          AttrData[AttrIndex], ListData[ListIndex], 
                          ListData[ListIndex+1]);
     
        _PDMdebug ("MemStr", "%s\n", MemStr);
     
        status = MEMwrite(TmpBufr, MemStr);
        if(status != MEM_S_SUCCESS) {
           _PDMdebug (fn, "MEMwrite failed status = %d\n",status);
           return(status);
         }
 ...
 if (PDMdebug_on) {
    MEMprint_buffer("ValListBufr PDMCreateValListBufr", 
                                         *ValListBufr,  PDM_DEBUG_FILE);
 }

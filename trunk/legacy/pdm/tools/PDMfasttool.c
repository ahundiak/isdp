...
char *revision;
{
	...
	_PDMdebug(fname,"ENTER:\n");
       ...
       type[0] = '\0';
 /* qry type added 062093 */
       status = PDMquery_catalogtype_no (catalog, type, &catno);
       if (status != PDM_S_SUCCESS) {
           _PDMdebug (fname, "PDMquery_catalogtype_no failed status 0x%.8x\n", status);
 ...
 t1.p_parttype, t1.p_incpartrpt, t1.n_aclno FROM %s t1, f_%s t2 \
 WHERE t1.n_itemname = '%s' AND \
	...
	if (status != SQL_S_SUCCESS) {
	    if (status == SQL_I_NO_ROWS_FOUND) {
		    ...
		    return (status);
               ...
               if (status == PDM_I_TABLE_LOCKED) return (status);
 ...
 if(PDMdebug_on)
    ...
    _PDMdebug (fname, "Part type is %s\n", type);
    sprintf(sql_str, 
       "SELECT n_itemno, n_creator, n_creationdate,  \
       n_updatedate,  n_status, n_setindicator, n_archivestate,\
       n_stateno, n_itemlock, \
       p_explode,\
       p_parttype FROM %s WHERE \
       n_itemrev = '%s' AND n_itemname= '%s'",
       catalog, revision, partnum);
     ...
     if (status != SQL_S_SUCCESS) {
       if (status == SQL_I_NO_ROWS_FOUND) {
           _PDMdebug (fname, "Part not found\n");
     	   MEMclose(&bufr);
...
sprintf(sql_str, 
 ...
 %s.p_parttype, %s.n_aclno FROM %s, f_%s WHERE %s.n_itemname = '%s' AND \
 %s.n_itemrev = '%s' AND %s.n_itemno = f_%s.n_itemnum",
catalog,  catalog, catalog, 
catalog, catalog, catalog, catalog, catalog, catalog, catalog, 
catalog, catalog, catalog, catalog, catalog, catalog, catalog, 
catalog, catalog, catalog, catalog, catalog, catalog, catalog, 
catalog, catalog, catalog, catalog, catalog, partnum, catalog, revision,  
catalog, catalog);

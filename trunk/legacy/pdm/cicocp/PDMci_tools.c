...
/* added for init 03/05/93 */
 ...
 file_type [0] = '\0';
  ...
  { 
    ...
    if (! strcmp (data [count + INDEX_N_NFMNAME], "n_catalogname")) 
      i1 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_status")) 
      j1 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_type"))   
      k1 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_level"))
      l1 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_citno"))
      m1 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_itemno"))
      i2 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_itemname"))
      j2 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_itemrev"))
      k2 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_status"))
      l2 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_itemnum"))
      i3 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_fileno"))
      j3 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_fileco"))
      k3 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_fileversion"))
      l3 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_cisano"))
      m3 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_cifilename"))
      n3 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_cofilename"))
      o3 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_cifilesize"))
      p3 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_fileclass"))
      q3 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_filetype"))
      r3 = x;
    else if (! strcmp (data [count + INDEX_N_NFMNAME],"n_setindicator"))
      s3 = x;
  }
...
long _NFMupd_ci_cat_fcat   ( user_id, c_user, c_date, c_time, sa_no,
...
MEMptr    attr_list, data_list, file_list;
{
  ...
  _NFM_str ("_NFMupd_ci_cat_fcat: Date       = <%s>  ", c_date   );
  _NFM_str ("_NFMupd_ci_cat_fcat: Time       = <%s>  ", c_time   );
      ...
      for (y = 0; y < attr_list -> rows; ++y)
      {
        ...
        */
        ...
        for (z = 0; ((z < list -> rows) && (! found)); ++z)
        {
          ...
          if ((! strcmp (data [count + INDEX_N_NFMNAME], data3 [count3 + 2])) &&
              (! strcmp (data [count + INDEX_N_NFMAPPLICATION], "NFM")))
          {
            found = 1;

            if (! strcmp (data [count + INDEX_N_NFMNAME], "n_fileno"))
            {
            ...
            else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_ciuser"))
            {
              sprintf (value, "%s", c_user);
              update = 1;
            }
            else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_cidate"))
            {
              sprintf (value, "%s", c_date);
              update = 1;
            }
            else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_citime"))
            {
            ...
            else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_cosano"))
            {
...

    {
      ...
      if (! strcmp (data [count + INDEX_N_NFMAPPLICATION], "NFM"))
      {
        if (! strcmp (data [count + INDEX_N_NFMNAME], data3 [count3 + 2]))
        {
          ...
          if (! strcmp (data [count + INDEX_N_NFMNAME], "n_itemno"))
          {
            strcat (sql_str1, data3 [count3]);
            strcat (sql_str1, " = ");
          ...
          else if (! strcmp (data [count + INDEX_N_NFMNAME], "n_colevel"))
          {
            if (comma) strcat (sql_str, ", ");
            comma = 1;
...
int  command;
{
  ...
  if (command == NFM_ITEM_IN)
  {
    ...
    if ( (strcmp(NFMitem_ptr->cat_item.n_status.data_value, "O")) &&
         ((NFMitem_ptr->cat_item.n_status.data_count))
       )
    {
  ...
  else if (command == NFM_ITEM_OUT)
  {
    ...
    if ( strcmp(NFMitem_ptr->cat_item.n_status.data_value, "I") ||
         (! NFMitem_ptr->cat_item.n_status.data_count) )
    {
  ...
  {
    if ( ( strcmp(NFMitem_ptr->cat_item.n_status.data_value, "I")) &&
         ( strcmp(NFMitem_ptr->cat_item.n_status.data_value, "O")) &&
         ( strcmp(NFMitem_ptr->cat_item.n_status.data_value, "S")) &&
         ( strcmp(NFMitem_ptr->cat_item.n_status.data_value, "M")) &&
         ( strcmp(NFMitem_ptr->cat_item.n_status.data_value, "TO"))  )

    {
      _NFMload_message (item_name, item_rev, operation, 
                        (NFMitem_ptr->cat_item.n_status.data_count)?
                        NFMitem_ptr->cat_item.n_status.data_value : "");
...
long _NFMchk_pendingflag (cat_name, item_name, item_rev, operation)
char *cat_name;
char *item_name;
char *item_rev;
int operation;
{
  /***
  This function will check the pending flag to make sure the command can be 
  proceeded.

  CHANGE HISTORY:
    02/04/91 MFB creation
  ***/
  char fcat_name[100];
  char sql_str[1024];
  MEMptr list = NULL;
  ...
  if (NFMmsg_ptr.debug_on)
  _NFM_str ("NFMchk_pendingflag: enter ...");
  sprintf (fcat_name, "f_%s", cat_name);

  /* SELECT n_pendingflag FROM f_<catalog>
     WHERE n_itemnum = ? AND n_fileclass = '?' AND n_fileversion = ? */ 

  /* n_pendingflag is not expected to be changed by other applications */ 

  sprintf (sql_str, "SELECT n_pendingflag FROM %s WHERE %s = ",
           fcat_name, NFMitem_ptr->fcat_item.n_itemnum.attr_name);
  ...
  /* n_fileclass is not expected to be changed by other applications */ 

  strcat (sql_str, " AND n_fileclass = 'C' AND ");
  strcat (sql_str, NFMitem_ptr->fcat_item.n_fileversion.attr_name);
  ...
  if (status != SQL_S_SUCCESS)
  {
    ...
    if (status != SQL_I_NO_ROWS_FOUND)
    {
      ...
      _NFM_num ("_NFMchk_pendingflag: SQLquery <0x%.8x>",
      NFM_E_BAD_ATTRIBUTE);
      return (NFM_E_BAD_ATTRIBUTE);
    }
    
    ERRload_struct (NFM, NFM_E_SQL_QUERY, "", NULL);
    _NFM_num ("_NFMchk_pendingflag: SQLquery <0x%.8x>", NFM_E_SQL_QUERY);
    return (NFM_E_SQL_QUERY);
  }
  ...
  if (status != MEM_S_SUCCESS)
  {
    ...
    _NFM_num ("_NFMchk_pendingflag: MEMbuild_array (list) <0x%.8x> ",
    NFM_E_MEM );
    return (NFM_E_MEM);
  }
  ...
  if (flagged)
  {
    _NFMload_message (item_name, item_rev, operation, data[x]);
    MEMclose (&list);
    _NFM_num ("_NFMchk_pendingflag: Item pendingflag set <0x%.8x>",
    NFM_E_PENDINGFLAG);
  ...
  MEMclose (&list);
  if (NFMmsg_ptr.debug_on)
  _NFM_num ("_NFMchk_pendingflag: exit successful...");
  return (NFM_S_SUCCESS);
}

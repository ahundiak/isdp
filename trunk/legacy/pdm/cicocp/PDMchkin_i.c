...
/* APOGEE STARTstatic 		long		RIScode; APOGEE END */
static 		long		status;
...
int PDMcomplete_item_checkin(PDMexec)
PDMexec_ptr	PDMexec;
{
...
/* APOGEE START char		n_status[3]; APOGEE END */
   ...
   PDMdebug("ENTER:PDMcomplete_item_checkin:\n");
 ...
 status = PDMstop_transaction(1200);
   ...
   {    
		sprintf(s,"Incorrect Number of Buffers Recieved\n");
		PDMdebug(s);
...
/* 10/23/95 - MJG - TR 139424087 DM Manager */
   if(!strncmp(n_setindicator,"Y",1) || !strncmp(n_setindicator,"A",1))
   {
     ...
     status = NFMcomplete_set_checkin(PDMexec->user_id,
     ...
     if(status != NFM_S_SUCCESS)
     {
		...
		MEMclose(&attr_bufr); MEMclose(&data_bufr);
		sprintf(s," %s %d\n",
		"NFMcomplete_set_checkin failed status -> ",status);
		PDMdebug(s);
   ...
   {
     status = NFMcomplete_item_checkin(PDMexec->user_id,
					  strg_data[0],strg_data[1],
					  strg_data[2],strg_data[3],
		  			  strg_data[4],
					  attr_bufr,data_bufr);
     if(status != NFM_S_SUCCESS)
     {
		...
		MEMclose(&attr_bufr); MEMclose(&data_bufr);
		sprintf(s," %s %d\n",
		"NFMcomplete_item_checkin failed status -> ",status);
		PDMdebug(s);
		return(PDM_E_COMPLETE_CHECKIN);
     }    
   }
   MEMclose(&data_bufr);
   MEMclose(&strg_bufr);
   PDMdebug("EXIT:PDMcomplete_item_checkin\n");
   return(PDM_S_SUCCESS);
}

int PDMprepare_item_checkin(PDMexec)
PDMexec_ptr	PDMexec;
{
	char		n_setindicator[3];
/* APOGEE START char		n_status[3]; APOGEE END */
   ...
   PDMdebug("ENTER:PDMprepare_item_checkin:\n");
...
*/
   if(!strncmp(n_setindicator,"Y",1) || !strncmp(n_setindicator,"A",1))
   {
     ...
     status = NFMprepare_set_checkin(PDMexec->user_id,
									 PDMexec->catalog,
									 PDMexec->part_num,
									 PDMexec->revision,
									 &attr_bufr,&data_bufr);
...
*/
     ...
     status = NFMprepare_item_checkin(PDMexec->user_id,
									 PDMexec->catalog,
									 PDMexec->part_num,
									 PDMexec->revision,
									 &attr_bufr,&data_bufr);
     ...
     {
		sprintf(s," %s %d\n",
		" NFMprepare_item_checkin failed status -> ",status);
		PDMdebug(s);
		if(status == NFM_E_ITEM_IN)
		{
			PDMdebug("Part in checked-in status\n");
   ...
   MEMclose(&data_bufr);
   PDMdebug("EXIT:PDMprepare_item_checkin\n");
   return(PDM_S_SUCCESS);
}

int PDMcancel_item_checkin(PDMexec)
PDMexec_ptr	PDMexec;
{
	char		n_setindicator[3];
/* APOGEE START char		n_status[3]; APOGEE END */
   ...
   PDMdebug("ENTER:PDMcancel_item_checkin:\n");
...
/*     status = NFMcheck_item(PDMexec->user_id,PDMexec->catalog,
     ...
     if(status != NFM_S_SUCCESS)
     {
		...
		PDMdebug(s);
		PDMdebug("Part can not be checked in\n");
		return(PDM_I_ALREADY_CHECKIN);
     }
*/
	   /* CANCEL NFMcheckin */

     status = NFMcancel_set_checkin(PDMexec->user_id,
									 attr_bufr,data_bufr);
     if(status != NFM_S_SUCCESS)
     {
		MEMclose(&attr_bufr); MEMclose(&data_bufr);
...
*/
     ...
     status = NFMcancel_item_checkin(PDMexec->user_id,
									 attr_bufr,data_bufr);
     if(status != NFM_S_SUCCESS)
     {
		MEMclose(&attr_bufr); MEMclose(&data_bufr);
   ...
   MEMclose(&attr_bufr);
   MEMclose(&data_bufr);
   PDMdebug("EXIT:PDMcancel_item_checkin\n");
   return(PDM_S_SUCCESS);
}

...
int pdmoChangeFiles(	cpr_catalog,
			cpr_partnum,
			cpr_revision,
			cpr_stg_name,
			spr_chg_files)
  ...
  struct   chg_file_info  *spr_chg_files;
{
  ...
  _pdm_debug("in the function pdmoChangeFiles", 0);
  ...
  if (file_data_bufr && stg_name)
    {
    _pdm_debug("writing storage area name to file_data_bufr", 0);
    status = PDUget_col_number(file_data_bufr,"n_cisaname",dummy_col, &cisaname_col);
    _pdm_status("PDUget_col_number", status);
    if (status != PDM_S_SUCCESS)
      {
      PDUcleanup_change_part(cpr_catalog, cpr_partnum, cpr_revision, 
    ...
    status = MEMwrite_data(file_data_bufr, cpr_stg_name,1, cisaname_col+1);
    _pdm_status("MEMwrite_data", status);
    if (status != MEM_S_SUCCESS)
      {
      PDUcleanup_change_part(cpr_catalog, cpr_partnum, cpr_revision, 

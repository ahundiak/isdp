...
Sample Invocation:
     ...
     pdm$query_for_catalogs(status = &status,
                 count          = &count,
                 catalogs       = &catalogs)
               );
...
Macro:
     ...
     pdm$set_alt_bom_switch

     This macro sets a global flag which forces all the subsequent boms
     created to be having alternate parents system

Arguments:

     ARGUMENT            DATA TYPE          I/O  DESCRIPTION
     status              *int                O   Status.
     alt_parent_attr     *char               I   Name of the dyn parent attr

Sample Invocation:

     int   status = PDM_S_SUCCESS, alt_parent_attr = name of the parent_attr;
...
******************************************************************************/
#omdef pdm$set_alt_bom_switch (  status, alt_parent_attr )

       PDMppl_set_alt_bom_switch (status, alt_parent_attr);
...
Macro:
     ...
     pdm$reset_alt_bom_switch

     This macro resets a global flag which makes all the subsequent boms
     created to be having regular (cit) parents system
...
******************************************************************************/
#omdef  pdm$reset_alt_bom_switch (  status )

       PDMppl_reset_alt_bom_switch (status);

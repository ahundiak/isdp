...
#omdef td$locate( msg, 
					...
					cycle_flag = FALSE,
					fence_objects = NULL,
					num_fence_objects = NULL
					
				 )

td_locate_an_element( &( msg ), 
...
td_locate_an_element( msg, locate_prompt, locate_str, 
			...
			ignore_classes, locate_ref, obj_md_env, locate_fence,
                        cycle_flag, fence_objects, num_fence_objects )
...
int		*num_fence_objects;
{
	...
	struct		GRlc_locate gs_attrs;	/* TR# 139418660 */
	IGRlong		gs_msg;			/* TR# 139418660 */
...
*/
	...
	/* TR# 139418660. Begin. */
	/* Set locate criteria for the save set.	*/
/* 29/Sep/94 - raj. TR# 139422045. Begin.
 * Set owner action along the lines of Save Element command.
 * Re-write the following.
 *
 *	gs_attrs.owner_action = LC_FLEX_COMP | LC_ASSOCIATIVE | 
 *				  LC_RIGID_OWNER |
 *				  LC_RIGID_COMP |
 *				  LC_FLEX_OWNER |
 ...
 */
	...
	gs_attrs.owner_action = LC_GEOM_MOD| LC_ASSOCIATIVE|
				LC_EXTERN_CONSTRAINED| LC_REF_HEADER;

	gs_attrs.properties = LC_LC_ONLY |
				LC_RW |
				LC_DP_ONLY;

	gr$gsput_locate_criteria ( msg = &gs_msg, attributes = &gs_attrs);
	...
	while ( 1 )
	{
		...
		*msg = 0;
		...
		_pdm_debug("Response from ci$getevent(): %d", *response);
		if ( LOCATE_ABORT )
		{
			LOCATE_ABORT = 0;
			return;
...
/* Now highlight all the objects and prompt the user to accept */
			...
			/*
			 * Copy the objects in the fence to an array.
			 */
			if (fence_objects)
			{
			  *fence_objects = (struct GRid *) malloc (LCfence_objects_count 
			  						* sizeof(struct GRid));
			  if (*fence_objects == NULL)
			  {
                          ...
                          /* Get active fence id.	*/
                          gr$gsget_fence (msg = &fence_msg, fence_id = &fence_id);
                          
			  *num_fence_objects = 0;
                          for ( LC_tmp = 0; LC_tmp < LCfence_objects_count ; LC_tmp = LC_tmp + 1)
                          {
                            /* Copy only if it is not the fence id it-self.	*/ 
                            if (LCfence_objects[LC_tmp].obj_id.objid != fence_id.objid)
                            {
                              (*fence_objects)[*num_fence_objects].objid = LCfence_objects[LC_tmp].obj_id.objid;
                              (*fence_objects)[*num_fence_objects].osnum = LCfence_objects[LC_tmp].obj_id.osnum;
                              *num_fence_objects = *num_fence_objects + 1;
                            }
                          }
                        }

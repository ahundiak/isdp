...
if [ ! -f "$FIXDIR/pdu_engint.p" ]
  then
    ...
    find HELPINFO forms messages/*.msg -depth -print 2>/dev/null \
    | cpio -oc | compress > $FIXDIR/pdu_engint.p
...
if [ "${INTERFACE}" = "y" ]
  then
    ...
    if [ "${LANG}" = "german" ]
    then
     getfile pdu_gerint.p | compress -d | cpio -imud 
     mv $FIXDIR/PDUcommand.msg ${DIR}/config/english/messages/PDUcommand.msg
    else
     getfile pdu_freint.p | compress -d | cpio -imud 
     mv $FIXDIR/PDUcommand.msg ${DIR}/config/english/messages/PDUcommand.msg
   fi
fi
...
if [ "${HELPRESP}" = "y" ]
  ...
  helppath=`proddata +%p helprt`
   ...
   if [ ! -f "$FIXDIR/pdu_enghelp.p" ]
    then
      echo "Backing up English HELP files"
      find HELPFILE -print 2>/dev/null \
      | cpio -oc | compress > $FIXDIR/pdu_enghelp.p
    fi
    echo "Installing ${LANG} Help files in ${PROD}."
    getfile pdu_gerhelp.p | compress -d | cpio -imud 
    AOK=$?
...
if [ ! -f "$FIXDIR/pdm_enginit.p" ]
  then
    ...
    | cpio -oc | compress > $FIXDIR/pdm_enginit.p
fi
cd ${PDMDIR}/config
if [ "${LANG}" = "german" ]
 then
   getfile pdm_gerinit.p | compress -d | cpio -imud
 else
   getfile pdm_freinit.p | compress -d | cpio -imud 
fi
}

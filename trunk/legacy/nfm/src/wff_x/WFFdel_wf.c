...
long WFFdelete_workflow (workflowname)
  char	*workflowname;
  {
	long	status;
	char	prompt1[120];
	...
	static	char *fname = "WFFdelete_workflow";
	...
	if ((status = (long) UMSGetByNum
		(prompt2, NFI_P_CONFIRM_WORKFLOW, "%s", workflowname)) != UMS_S_SUCCESS)
	  {
		_NFIdebug ((fname, "UMSGetByNum = <%d>\n", status));
		_NFIdebug ((fname, "defaulting prompt ...\n"));
		sprintf (prompt2, "Workflow \"%s\" ?", workflowname);
	  }

	/*  Set the working message  */

	WFFglobal.working_no = NFI_I_DELETE_WORKFLOW;

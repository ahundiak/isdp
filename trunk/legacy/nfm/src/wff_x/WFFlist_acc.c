...
long WFFlist_class_access (form, workflow_name)
  ...
  char  *workflow_name;
  {
	...
	static	char *fname = "WFFlist_class_access";
	...
	_NFIdebug ((fname, "calling WFGlist_class_access ...\n"));
	if ((status = WFGlist_class_access (WFFglobal.user_id, workflow_name,
                             "", "", &attr_list, &data_list)) != NFM_S_SUCCESS)
	  {
		...
		MEMclose (&data_list);
		_NFIdebug ((fname, " returning FAILURE\n"));
                if (status == NFM_E_NO_CLASS_ACCESS_DEFINED) 
                   ERRload_struct (NFM, status, NULL);
		return (status);
	  }
	_NFIdebug ((fname, "returned from WFGlist_class_access\n"));
	...
	FIg_erase (form, LISTING_TITLE);
        ...
        NFI_MAX_COLUMNS = MAX_NO_ACCESS_MCF_COLUMNS;

	if ((status = _WFFlist_load_mcf (form, FI_REVIEW, 
		                         4, WFFglobal.attr_list,
	...
	if ((status = (long) UMSGetByNum
	    (s, NFI_P_LIST_CLASS_ACCESS, "%s", workflow_name)) != UMS_S_SUCCESS)
	  {
		_NFIdebug ((fname, "UMSGetByNum = <0x%.8x>\n", status));
		_NFIdebug ((fname, "defaulting title\n"));
		sprintf (s, "List of I/NFM Class Access for Workflow \"%s\"",
                         workflow_name);
	  }
	...
	WFFglobal.current_list = WFF_LIST_CLASS_ACCESS;

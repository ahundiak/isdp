...
long WFFlist_transitions_long (form, workflow_name)
  ...
  char  *workflow_name;
  {
	...
	static	char *fname = "WFFlist_transitions_long";
	...
	_NFIdebug ((fname, "calling WFGlist_transitions ...\n"));
	if ((status = WFGlist_transitions (WFFglobal.user_id, workflow_name, "",
                 "", WFF_ALL_DATA, &attr_list, &data_list)) != NFM_S_SUCCESS)
	  {
		_NFIdebug ((fname, "WFGlist_transitions = <0x%.8x>\n", status));
		_WFFerase_message ();
		MEMclose (&attr_list);
		MEMclose (&data_list);
		_NFIdebug ((fname, " returning FAILURE\n"));
                if (status == NFM_E_NO_TRANSITIONS_DEFINED)
                   ERRload_struct (NFM, status, NULL);
		return (status);
	  }
	_NFIdebug ((fname, "returned from WFGlist_transitions\n"));
	...
	FIg_erase (form, LISTING_TITLE);
        ...
        NFI_MAX_COLUMNS = MAX_NO_TRANS_MCF_COLUMNS;
	...
	if ((status = (long) UMSGetByNum
		(s, NFI_P_LIST_TRANSITIONS, "%s", workflow_name)) != UMS_S_SUCCESS)
	  {
		_NFIdebug ((fname, "UMSGetByNum = <0x%.8x>\n", status));
		_NFIdebug ((fname, "defaulting title\n"));
		sprintf (s, "List of I/NFM Transitions in Workflow \"%s\"",
                         workflow_name);
	  }
	...
	WFFglobal.current_list = WFF_LIST_TRANSITIONS;
...
long WFFlist_transitions_short (form, workflow_name)
  Form	form;   /*  i - main form  */
  char  *workflow_name;
  {
	long	status;
	...
	static	char *fname = "WFFlist_transitions_short";

	WFFglobal.current_popup = WFF_LIST_TRANSITIONS;
	...
	_NFIdebug ((fname, "calling WFGlist_transitions ...\n"));
	if ((status = WFGlist_transitions (WFFglobal.user_id, workflow_name, "",
                "", WFF_NAME_ONLY, &attr_list, &data_list)) != NFM_S_SUCCESS)
	  {
		_NFIdebug ((fname, "WFGlist_transitions = <0x%.8x>\n", status));
		_WFFerase_message ();
	...
	if ((status = _WFFlist_load_popup
		("n_transitionname", data_list)) != NFI_S_SUCCESS)
	  {

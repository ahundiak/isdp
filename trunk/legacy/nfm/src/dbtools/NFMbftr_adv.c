...
*		NFMftr_search. The query will be		           *
...
*									   *
*	This function allows a combination of FTR, Advance and Attribute   *
*	search.								   *
*									   *
***************************************************************************/
...
long NFMblockftr_adv(attr_list,out_list,cat_name,cat_no,search_name , sort_name,adv_select,adv_from,adv_where,ftr_where,query_flag,row_size, cursor_id)
	MEMptr	attr_list, *out_list;
	char *search_name , *sort_name;
	char *cat_name,*cat_no;
	char *adv_select, *adv_from, *adv_where,*ftr_where;
	long query_flag,row_size,cursor_id;
{
	...
	char **data,**format;
	char *fname="NFMblockftr_adv";

	_NFMdebug((fname,"Entry: attr_list <%x>: out_list <%x>: catalog name <%s>\
Catalog no <%s>: search_name <%s>: sort_name <%s> adv_select <%s> adv_from <%s>\
\n adv_where <%s> query_flag <%d> row_size <%d>\
 cursor_id <%d> \n",
attr_list,out_list,cat_name, cat_no,search_name,sort_name,adv_select,adv_from,adv_where,query_flag,row_size,cursor_id));
...
/* Extract Information about advance Query */
	if(adv_select != (char *)0 && strlen(adv_select) > 0 )
		size = size+ strlen(adv_select)+5;
	if(adv_from != (char *)0 && strlen(adv_from) > 0 )
		size = size+ strlen(adv_from)+5;
	if(adv_where != (char *)0 && strlen(adv_where) > 0 )
		size = size+ strlen(adv_where)+5;
	if(ftr_where != (char *)0 && strlen(ftr_where) > 0 )
		size = size+ strlen(ftr_where)+5;
...
/* Append the advance search select clause if any */
	if(adv_select != (char *)0 && strlen(adv_select) > 0 )
        {
			strcat(qry_str,", ");
			strcat(qry_str,adv_select);
	}
...
/* Append the advance search from clause if any */
	if(adv_from != (char *)0 && strlen(adv_from) > 0 )
        {
			strcat(qry_str,", ");
			strcat(qry_str,adv_from);
	}
...
/* Append the advance search where clause if any */
	if(adv_where != (char *)0 && strlen(adv_where) > 0 )
        {
			strcat(qry_str," AND ");
			strcat(qry_str,adv_where);
	}

/* Append the ftr search where clause if any */
	if(ftr_where != (char *)0 && strlen(ftr_where) > 0 )
        {
			strcat(qry_str," AND ");
			strcat(qry_str,ftr_where);
	}
	if (sort_name != NULL  )
	{
		if(strlen(sort_name))

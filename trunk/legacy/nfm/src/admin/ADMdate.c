...

{
	...
	static char *fname = "ADMdate_conf";
...
/*  sprintf(sql_str, "SELECT n_rowno, n_description \
  ...
  status = ADMadd_date_type (&next_row_no, date_type) ;
    ...
    sprintf (data.title, "%s","Configure The Date Format");
    ...
    memset (data.data, NULL, (tot_data * sizeof (char *)));
      ...
      NFMput_string (&data.data[0],"This Is The Current Date Format");
      NFMput_string (&data.data[1], date_type) ;
      NFMput_string (&data.data[2],"Would Like To Change The Date Format [Y/N]"); 
       ...
       if (data.sts == FORM_CANCELED) 
          ...
          (strcmp (data.data[3], "Y") == 0))
        {
         ...
         fdata.h2_pos[1] = -1;
          ...
          sprintf (fdata.title, "%s","Select Desired Date Format");
          ...
          NFMput_string (&fdata.data[0], "AMERICAN" );
          NFMput_string (&fdata.data[1], "FRENCH" );
          NFMput_string (&fdata.data[2], "BRITISH" );
    	  NFMput_string (&fdata.data[3], "GERMAN" );
          NFMput_string (&fdata.data[4], "SPANISH" );
          NFMput_string (&fdata.data[5], "ITALIAN" );

          _NFMdebug((fname, "%s%s%s%s%s%s", fdata.data[0],
         fdata.data[1], fdata.data[2], fdata.data[3], fdata.data[4],
fdata.data[5])) 
            ...
            if (fdata.select_set[i] == 1)
             {
              ...
              sprintf (qry_str,"UPDATE nfmsyscat SET n_description='%s'\
               WHERE n_infotype ='DATE_FORMAT'", fdata.data[count]);

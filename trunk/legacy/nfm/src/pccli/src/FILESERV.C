...
#define RAS_ATTACH (element->buf[0] == 0x5a01)
  ...
  if (struct_ptr->request == ABORT_OPERATION)
    {
      ...
      *src = 1 ; 
      ...
      _CLI_str ("size of buffer sent %d", size) ;
...
/******** what happen if path end in \ **************/
  ...
  while (!FEOF && !EOD)
	...
	{
	  _CLI_str ("Encountered END OF FILE\n") ;
	}
      else if ((buf[bufpos] & 0x8000) == 0x8000)
	{
	  ...
	  if ((curpos + (WTF+2) * 2) >= endbuf)
	    {
	      curpos += (unsigned long) ((unsigned long) (WTF + 2) * (unsigned long) 2) ;
	      continue ;
	    }
	}
      else if (WTF > 766)
		...
		if (status == NFM_S_SUCCESS)
		  {
		    ...
		    sprintf (tmp_str, "%s\001", filename ) ;
	  ...
	  if ((abs_path[i] == '/') || (abs_path [i] == '\\') ||
	      ...
	      if (abs_path[i] == ':')
		{
		  if (abs_path [i+1] == '\\')
		    {
		      *flag = REF_ABS_PATH ;
		      _CLI_str ((fname, "device - must be absolute path\n")) ;
		      continue ;
		    }
		  *flag = REF_ENV_PATH ;
		  strcpy (env, abs_path) ;

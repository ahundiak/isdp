...
long UpdateAutoexec()
{
  ...
  char dataline[255],oldpath[255], newpath[255],*line_ptr;
  ...
  line_ptr= fgets(dataline,255,new_file);
  while ((!feof(new_file)) && (line_ptr != (char *)0))
    {
  ...
  line_ptr= fgets(dataline,255,old_file);
  done = 0 ;
  while ((!feof(old_file)) && (line_ptr != (char *)0))
    {
...
*/
	  sprintf(newpath,"PATH=%s;%%path%%",bpath);
	  fprintf(new_file,"%s\n",newpath);
	  dataline[strlen(dataline)-1] = '\0';
/*
	  sprintf(newpath,"%s;%s", dataline, nfm_path);
*/
	  sprintf(newpath,"%s;%s", dataline, bpath);
...
long InstallProduct()
{
  ...
  sprintf(wrk,".\\CNFM -o %s\\bin nfmpcnfs.exe >NUL",nfm_path);
  ...
  sprintf(wrk,".\\CNFM -o %s\\bin nfmpctcp.exe >NUL",nfm_path);
  status = SystemCall(wrk);
  if (status != CLI_S_SUCCESS)
    {
      fprintf (stderr, "%s : failed\n", wrk) ;
  ...
  if (access(wrk,0)==0)
    {
      ...
      fprintf (stderr, "%s : will be stored in nfmpc.old\n", wrk) ;
      fprintf (stderr, "%s : New one will be loaded\n", wrk) ;
      _CLI_str ("%s : Exists", wrk) ;
      sprintf(work, "del %s\\bin\\nfmpc.old >NUL", nfm_path);
      SystemCall(work);
      if (status != CLI_S_SUCCESS)
      {
        fprintf (stderr, "%s : failed\n", wrk) ;
	fprintf(stderr,"Please perform: %s and re-install\n",wrk);
        _CLI_str ("%s : failed", wrk) ;
        return(CLI_E_FAILURE);
       }
      sprintf(work, "ren %s\\bin\\nfmpc.exe nfmpc.old >NUL", nfm_path);
      SystemCall(work);
      if (status != CLI_S_SUCCESS)
      {
        fprintf (stderr, "%s : failed\n", wrk) ;
  ...
  if(nfs_package==FTP )
	  sprintf(work, "ren %s\\bin\\nfmpctcp.exe nfmpc.exe >NUL", nfm_path);
  else
	  sprintf(work, "ren %s\\bin\\nfmpcnfs.exe nfmpc.exe >NUL", nfm_path);
  SystemCall(work);
  if (status != CLI_S_SUCCESS)
    {
      fprintf (stderr, "%s : failed\n", wrk) ;
      _CLI_str ("%s : failed", wrk) ;
      sprintf(work, "ren %s\\bin\\nfmpc.old nfmpc.exe >NUL", nfm_path);
      SystemCall(work);
      if (status != CLI_S_SUCCESS)
      {
        fprintf (stderr, "%s : failed\n", wrk) ;
  ...
  if(nfs_package==FTP )
	  sprintf(work, "del %s\\bin\\nfmpcnfs.exe >NUL", nfm_path);
  else
	  sprintf(work, "del %s\\bin\\nfmpctcp.exe >NUL", nfm_path);
  SystemCall(work);
  if (status != CLI_S_SUCCESS)
    {
      fprintf (stderr, "%s : failed\n", wrk) ;

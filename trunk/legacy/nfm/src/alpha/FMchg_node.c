...
NDXptr index;
static char *fname = "change_node";

    _NFIdebug ((fname, "Enter.\n"));

    SCdisplay_msg ("Working..");    

    status = NFMRquery_change_node_attributes (NFMuser_id, NFMnode,&attr_list, 
    ...
    _NFIdebug ((fname, "Suppress input :\n"));
    for (j=pos; j<tot_data; j+=data.num_of_cols)
        {
        for (i=0; i<attr_list->rows*attr_list->columns; i+=attr_list->columns)
            {
    ...
    _NFIdebug ((fname, "MEMbuild Data List :\n"));
    ...
    if (status != MEM_S_SUCCESS)
        ...
        free (data.data);
	ERRload_struct( NFI, NFI_E_MEMORY, "%s%s", "NFMchange_node",
			"MEMbuild_array" );
        return (status);
        }
    ...
    for (i=pos; i<tot_data; i+=data.num_of_cols)
		...
		{
                    strcpy (node_name, data.data[i+2]);
		    strcpy (NFMnode, data.data[i+2] );
		}
                break;
                }
            }
	...
	_NFIdebug ((fname,  "NFMRtic_it : <0x%.8x>\n", status ));
        ...
        MEMclose (&data_list);
        MEMclose (&value_list);
	return( NFI_E_CHANGE_NODE );
    }

    SCdisplay_msg ("Working..");    

    status = NFMRchange_node (NFMuser_id, attr_list, data_list);

    _NFIdebug ((fname, "NFMRchange_node -> <0x%.8x>\n", status));

...
#include "NFMf_xferinc.h"
 ...
 long NFMcl_get_files_in (node_name, user_name, passwd,
                          protocol, in_list, out_list)
   char     *node_name, *user_name, *passwd;
   long     protocol;
   MEMptr   in_list, *out_list;
   {
char *fname="NFMcl_get_files_in ";
...
node_name,user_name,passwd,protocol));
      ...
      status = NFMcl_send_buffer (&sock_no, NFM_RECEIVE_FILES, in_list);
      if (status != NFM_S_SUCCESS)
       {
      ...
      if (status1 != NFM_S_SUCCESS)
       {
         _NFMdebug ((fname,"Ser Rec File Failed : status1 = <0x%.8x>\n",
         status1));
	ERRload_struct(NFM,NFM_E_FAILURE,"%s %x","Server Receive File Failed",status1);
         return (status1);
       }
 ...
 long NFMser_get_files_in (in_list, out_list)
   MEMptr    in_list;
   MEMptr    *out_list;
   {
char *fname="NFMser_get_files_in ";
      long     status;
	...
	ERRload_struct(NFM,status,"%s","MEMreset_buffer_no");
	 ...
	 _NFMdebug((fname,"Mach_id <%s> Status <%s> size <%s>\n",data[count+q1],data[count+w1],data[count+u1]));
         ...
         if ((! strcmp (data [count + w1], "move")) &&
		  ...
		  ERRload_struct(NFM,status,"%s","Password");
               ...
               if ((! strcmp (data [count1 + w1], "move")) &&
                   ...
                   (! strcmp (data [count1 + i1], temp_sa)))
                {
                  status = NFMappend_path_file (data [count1 + r1],
                           data [count1 + m1], data [count1 + s1], 
                           src_file);
                  if (status != NFM_S_SUCCESS)
                   {
                     NFMdisconnect(FILE_SOCK);
                     MEMclose (&sa_list);
                     _NFMdebug ((fname,"App Path : status = <0x%.8x>\n",
                     status));
                     return (status);
                   }

                  status = NFMappend_path_file (data2 [r2],
                           data2 [m2], data [count1 + t1], dst_file);
                  if (status != NFM_S_SUCCESS)
                   {
                     NFMdisconnect(FILE_SOCK);
                     MEMclose (&sa_list);
                  ...
                  if ((! strcmp (data [count1 + q1], "CLIX")) ||
                       (!strcmp (data [count1 + q1],  "UNIX")))
                      
                   {
                     status = NFMfile_receive (src_file, dst_file,
                              data [count1 + v1], (int *)&size,FILE_SOCK);
                     if (status != NFM_S_SUCCESS)
                      {
                        NFMdisconnect(FILE_SOCK);
                        MEMclose (&sa_list);
		   ...
		   {
		    status = NETfmu_con_receive(data [count1+j1 ],
                   ...
                   {
                     status = NFMftp_receive (data [count1 + j1],
                              data [count1 + k1], dec_pass, src_file, dst_file,
                              data [count1 + v1], &size);
                     if (status != NFM_S_SUCCESS)
                      {

...
/*****************************************************************************
 *                                                                           *
 *   This function is used to process the archival of flagged items.         *
 *                                                                           *
 *****************************************************************************/
...
long _NFFarchive_flagged_items ()
  {
    ...
    ptr->type = ARCHIVE_FLAGGED_ITEMS;
    ptr->done = FALSE;
    ptr->utility = 0; 
    strcpy (ptr->label, "");
    ptr->sa_list = NULL;
    ...
    NFFglobal.NFMRfunction[0] = NFMRarchive_files;

    NFFform_message (CURRENT_LABEL, NFI_I_RETRIEVE, NULL);

    /* Call the NFMRquery_archive function to get the information */

    status = NFMRquery_archive (&(ptr->sa_list),  
                                &(ptr->item_list), &(ptr->label_list));
    if (status != NFM_S_SUCCESS)
      {
        _NFIdebug ((fname, " NFMRquery_archive : status = <%d>\n", status));
        NFFerase_message ();
    ...
    if (ptr->item_list == NULL)
     {
        status = NFI_W_NO_ITEMS_FLAGGED_A;
    ...
    if (_NFMdebug_st.NFIdebug_on)
      {
        ...
        MEMprint_buffer ("sa_list", ptr->sa_list,
            _NFMdebug_st.NFIdebug_file);
        MEMprint_buffers("label_list", ptr->label_list,
            _NFMdebug_st.NFIdebug_file);
      }
    ...
    if ((status = (long) UMSGetByNum (title, NFT_P_ARCHIVE_FLAGGED_ITEMS,
                                      NULL)) != UMS_S_SUCCESS)
     {
        _NFIdebug ((fname, "UMSGetByNum = <0x%.8x>\n", status));
        _NFIdebug ((fname, "defaulting title\n"));
        strcpy (title, "Archive Flagged Items");
     }

...
long NFFadd_files_to_item (catalog, item, revision)
    char    *catalog;
    char    *item;
    char    *revision;
  {
    ...
    static  char *fname = "NFFadd_files_to_item";
    ...
    status = NFMRget_file_information (catalog, item, revision,
                                       (long) NFM_ADD_FILE_TO_ITEM, &attr_list,
                             &(ptr->temp_list), &((ptr->file_ptr)->value_list));
    ...
    if (_NFMdebug_st.NFIdebug_on)
      {
        ...
        MEMprint_buffer ("data_list", ptr->temp_list,  
    ...
    data = (char **) (ptr->temp_list)->format_ptr;
    ...
    /* copy the temp_list column/format info into data_list and treat
       the data_list as if it had no rows *****************************/

    status = MEMopen (&((ptr->file_ptr)->data_list),   
                      (ptr->temp_list)->row_size + MEM_SIZE);
    if (status != MEM_S_SUCCESS)
     {
        NFMRreset_file_information (catalog, item, revision);
        _NFIdebug ((fname, "MEMopen = <0x%.8x>\n", status));
    ...
    column_ptr = (char **) ptr->temp_list->column_ptr;
    format_ptr = (char **) ptr->temp_list->format_ptr;

    /* write all the existing columns in the temp_list into the data_list */

    for (i = 0; i < (ptr->temp_list)->columns; ++i)
     {
        status = MEMwrite_format ((ptr->file_ptr)->data_list,
                                  column_ptr[i], format_ptr[i]);
        if (status != MEM_S_SUCCESS)
         {
            NFMRreset_file_information (catalog, item, revision);

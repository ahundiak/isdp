...
long NFFplot_item (display_flag, catalog, item, revision)
  int     display_flag;
  char    *catalog;
  char    *item;
  char    *revision;
  {
    ...
    static  char    *fname = "NFFplot_item";
    ...
    if ((status = (long) UMSGetByNum
        (prompt1, NFT_P_CONFIRM_PLOT, NULL)) != UMS_S_SUCCESS)
      {
        _NFIdebug ((fname, "UMSGetByNum = <%d>\n", status));
        _NFIdebug ((fname, "defaulting prompt ...\n"));
        strcpy (prompt1, "Are you sure you want to plot");
      }
    ...
    NFFglobal.working_no = NFI_I_PLOT_ITEM;

    /*  Set the NFMR function  */

    NFFglobal.NFMRfunction[0] = NFMRplot_item;

    if ((status = _NFFconfirm (PLOT_ITEM_LABEL, display_flag,
...
long NFFplot_set (display_flag, catalog, set, revision)
  int     display_flag;
  char    *catalog;
  char    *set;
  char    *revision;
  {
    ...
    NFFglobal.NFMRfunction[0] = NFMRplot_set;
...
long NFFplot_set_parent (catalog, set, revision)
  char    *catalog;
  char    *set;
  char    *revision;
  {
    ...
    auto    char    prompt1[NFF_MAX_MESSAGE];
    auto    char    prompt2[NFF_MAX_MESSAGE];
    static  char    *fname = "NFFplot_set_parent";

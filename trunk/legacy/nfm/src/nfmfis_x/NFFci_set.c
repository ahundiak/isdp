...
long NFFcheckin_set (display_flag, catalog, item_name, revision)
  int     display_flag;
  char    *catalog;
  char    *item_name;
  char    *revision;
  {
    ...
    static  char    *fname = "NFFcheckin_set";
  ...
  /* prompt needs to change for confirm */
    ...
    NFFglobal.working_no = NFI_I_CHECKIN_SET;

    strcpy (command, CHECKIN_SET);

    /*  Set the NFMR function  */

    NFFglobal.NFMRfunction[0] = NFMRcheckin_set;

    if ((status = _NFFset_io (CHECKIN_CONFIRM_LABEL, display_flag, command,

...
static char *fname = "NFMSadd_node";
...
ret_status = NFMadd_node (user_id, attr_list, data_list);

MEMclose (&attr_list);
MEMclose (&data_list);

_NFMSdebug(( fname, "NFMadd_node -> <0x%.8x>\n", ret_status));
...
long NFMSquery_add_node_attributes (in_list)
...
static char *fname = "NFMSquery_add_node_attributes";
...
ret_status = NFMquery_add_node_attributes (user_id, &attr_list, &data_list,
                                           &value_list);

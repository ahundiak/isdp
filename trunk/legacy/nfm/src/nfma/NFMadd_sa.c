...
#include "NFMA.h"
 ...
 long NFMadd_return_sa_name (attr_list, data_list, saname)
   MEMptr  attr_list, data_list;
   char    *saname ;
   {
     static char *fname = "NFMadd_return_sa_name" ;
      long      status;

     _NFMdebug ((fname, "ENTER\n")) ;

     status =  _NFMadd_return_sa_name (attr_list, data_list, saname) ;
     if (status != NFM_S_SUCCESS)
       {
         _NFMdebug ((fname, "Failure : return saname  : status = <0x%.8x>\n",
		     status)) ;
	 return (status) ;
       }

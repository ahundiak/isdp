...
long _NFMlock_and_retrieve_values (sql_str, ret_type, ret_str, ret_value, 
         ret_list, lock_flag, table)

char *sql_str, *ret_str, *table;
long ret_type, lock_flag, *ret_value;
MEMptr *ret_list;

{
  /* NFM_NO_LOCK NFM_LOCK_NO_COMMIT NFM_LOCK_COMMIT NFM_COMMIT_ONLY */     
  char *fname="_NFMlock_and_retrieve_values"; 
  long status;
  long NFMretrieve_values ();

  _NFMdebug ((fname, "ENTER:\n"));
 
  if (lock_flag == NFM_LOCK_NO_COMMIT || lock_flag == NFM_LOCK_COMMIT)
  {
    status = NFMlock_table (table);
    if (status != NFM_S_SUCCESS)
    {
      _NFMdebug ((fname, "NFMlock_table 0x%.8x\n", status));
      return (status);
  ...
  status = NFMretrieve_values (sql_str, ret_type, ret_str, ret_value,
            ret_list);
  if (status != NFM_S_SUCCESS)
  {
  ...
  Note:
    query_type == NFM_ITEM_QUERY,
         pre_str1 = "select n_itemno, n_status, n_archivestate, n_itemlock, 
                     n_pendingflag, n_versionlimit, n_itemdesc from",
...

{
  ...
  long status;
  long NFMretrieve_values ();
  char *pre_str1="select n_itemno, n_status, n_archivestate, n_itemlock, n_pendingflag, n_versionlimit, n_itemdesc from";
...

{
  ...
  char sql_str[1024];
  long x, index;
  long i_fileversion, i_ciuser, i_cidate, i_cosano, i_couser, i_codate, i_fileco;
  long q_fileversion=3;
...
long _DMcall_do_nothing (count)

long count;

{

  char *fname="_DMcall_do_nothing";
  long x;

  _NFMdebug ((fname, "ENTER, call do_nothing <%d> times\n", count));

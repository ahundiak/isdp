...
long NFMcancel_check_out_set (catalog_name, catalog_no, set_name, 
                      set_revision, set_no,
...
{
  char *fname="NFMcancel_check_out_set";
  char fcat_name[NFM_CATALOGNAME+3];
  char sql_str[1024], itemdesc[NFM_ITEMDESC+1];
  char row_str[2048];
  char read_str[10], delete_str[10], nfs_str[10];
  char **data, **data1;
  /* Added 20 to the size of the array - SSRS 12 Apr 94 */
  char fileco_str[NFM_FILECO+21];
  ...
  if (operation->transfer == NFM_NO_TRANSFER)
  {
         ...
         x < num_members; x++)
    {
      ...
      cur_set = cur_set->next;
      /* Changed if condition to take care of NULL - SSRS 12 Apr 94 */
  ...
  else 
  {
    ...
    sprintf ((format+14)->data_type, "char(%d)", NFM_ITEMREV);
    strcpy ((format+15)->attr_name, "n_catname");
    sprintf ((format+15)->data_type, "char(%d)", NFM_CATALOGNAME);
    strcpy ((format+16)->attr_name, "n_itemname");
    sprintf ((format+16)->data_type, "char(%d)", NFM_ITEMNAME);
    strcpy ((format+17)->attr_name, "n_itemdesc");
    sprintf ((format+17)->data_type, "char(%d)", NFM_ITEMDESC);
  ...
  "n_cosano, n_cifilename, n_cofilename, n_cisano, n_couser, n_cifilesize from",
    ...
    if (DM_MANAGER)
      ...
      if (!x)
      {
        ...
        sano = atol (data[5]);
        /* 6/8/94, Initialize the variable in first character */
        working_area[0] = '\0';
    ...
    for (x = lfm_count = 0; x < files_info->rows; x ++)
    {
      ...
      if (status != NFM_S_SUCCESS)
      {
          ...
          free (lfm_files);
          NFMrollback_transaction (0);
          _NFMdebug ((fname, "NFMlock_tables 0x%.8x\n", status));
          if (DM_MANAGER)
            _DMdo_nothing ();
          return (status);
      }
    ...
    if (status != NFM_S_SUCCESS)
    {
      ...
      if (!DM_MANAGER && (status1 != NFM_S_SUCCESS))
      {
        NFMrollback_set_status (NFM_CANCEL_CHECK_OUT,set_members, 
		 (MEMptr) NULL,num_members, 0, 0,
              (struct NFMlfm_file *) NULL, (struct NFMlfm_file *) NULL);

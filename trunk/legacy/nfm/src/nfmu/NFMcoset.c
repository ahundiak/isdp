...
long NFMcheckout_set (catalog_name, catalog_no, set_name, set_revision, set_no,
...

{
  ...
  /* 20 is added to array size as the existing array size is insufficient
     SSRS 10 Mar 94 
  */
  char fileco_str[NFM_FILECO+1+20];
  char file_status[NFM_STATUS+1], itemdesc[NFM_ITEMDESC+1];
  char dumb_str[1];
  char error_msg[100];
  long error_no;
  int  subsystem;
  long degree, native_and_raster;
  long x, y, z, count, or;
  ...
  */
  ...
  if (operation->transfer == NFM_NO_TRANSFER)
  {
         ...
         x < num_members; x++)
    {
      ...
      cur_set = cur_set->next;
      /* changed if condition - SSRS 10 Mar 94 */
  ...
  else 
  {
         ...
         x < num_members; x ++, cur_set=cur_set->next)
    {
      ...
      /* 11/2/94 Mobil Solution */
      if (list->rows == 2)
      {
        if ( ( (strcmp (data[3], "DOCN") == 0) &&
             (strcmp (data[list->columns+3], "DOCR") == 0) ) ||
             ( (strcmp (data[3], "WPN") == 0) &&
             (strcmp (data[list->columns+3], "WPR") == 0)  ) ||
             ( (strcmp (data[3], "LOTN") == 0) &&
             (strcmp (data[list->columns+3], "LOTR") == 0) ) ||
             ( (strcmp (data[3], "XLSN") == 0) &&
             (strcmp (data[list->columns+3], "XLSR") == 0) )    )
        {
          native_and_raster = 1;
      ...
      for (y = 0; y < list->rows; y ++)
      {
        ...
        if ( y == 1 && native_and_raster)
          strcat (row_str, "NR"); /* native_and_raster pair item,
                                     don't move raster */
        /* end 11/2/94 */
      ...
      if (operation->transfer == NFM_CONDITIONAL_TRANSFER)
      {
        ...
        if (status == SQL_S_SUCCESS)
        {
		 ...
		 (MEMptr) NULL,num_members, 0, 0,
          ...
          sprintf ((lfm_files+x)->sano, "%ld", sto_list->sano);

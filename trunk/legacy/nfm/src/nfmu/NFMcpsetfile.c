...
extern MEMptr DMhighlight_files;

long NFMcopy_set_files (catalog_name, catalog_no, set_name, set_revision, 
...
{
  char *fname="NFMcopy_set_files";
  char fcat_name[NFM_CATALOGNAME+3];
  char sql_str[1024], upd_str[2048], temp_str[10];
  char row_str[2048], ftr_row[2048];
  char nfs_str[10], lfm_nfs_str[10], dumb_str[1];
  char **data, **temp_data, **data1;
  char filename[NFM_COFILENAME+1];
  char error_msg[100], itemdesc[NFM_ITEMDESC+1];
  long error_no;
  int subsystem;
...
if (DM_MANAGER) _DMcall_do_nothing (2);
  ...
  sprintf ((format+13)->data_type, "char(%d)", NFM_ITEMREV);
  strcpy ((format+14)->attr_name, "n_catname");
  sprintf ((format+14)->data_type, "char(%d)", NFM_CATALOGNAME);
  strcpy ((format+15)->attr_name, "n_itemname");
  sprintf ((format+15)->data_type, "char(%d)", NFM_ITEMNAME);
  strcpy ((format+16)->attr_name, "n_itemdesc");
  sprintf ((format+16)->data_type, "char(%d)", NFM_ITEMDESC);
...
if (DM_MANAGER) _DMcall_do_nothing (2);
    ...
    sprintf (sql_str, "%s %s %s %s %d %s %s %s %s %d %s",
             ...
             cur_set->item_no, ") order by n_filetype DESC");
...
if (DM_MANAGER) _DMcall_do_nothing (2);
    ...
    if (list->rows == 2)
    {
      ...
      {
        native_and_raster = 1;
        _NFMdebug ((fname, "the item has  native and raster files\n"));
      }
      else
      {
        native_and_raster = 0;
    ...
    for (y = 0; y < list->rows; y ++)
    {
      ...
      if (y == 1 && native_and_raster)
        strcat (row_str, "NR"); /* Native_and_Raster pair item, don't move
                                   native file */
      /* end of 11/2/94 */
...
if (DM_MANAGER) _DMcall_do_nothing (2);
  ...
  if (DMhighlight_files != (MEMptr) NULL)
  {
    ...
    _NFMdebug ((fname, "free and close DMhighlight_files\n"));
  }
...
if (DM_MANAGER) _DMcall_do_nothing (2);
    ...
    if (operation->transfer == NFM_CONDITIONAL_TRANSFER)
    {
      if ((lfm_files+x)->action == NFM_UPDATE_COPY)
      {
        ...
        {
          if (strcmp (data[index+i_status1], "NR") == 0)
            strcat (row_str, "");
          else
            strcat (row_str, nfs_str);
  ...
  else
  {
         ...
         x++, cur_set = cur_set->next)
    {
      ...
      for ( ; y < files_info->rows; y++)
      {
        ...
        {
          _NFMdebug ((fname,
            "native_and_raster pair, do not update LFM for native file\n"));
          continue;
        }
        ...
        if (status != NFM_S_SUCCESS)
        {
          ...
          degree = 1;
          _NFMcleanup (&degree, wf_info, "", "", "");
          _NFMdebug ((fname,"NFMlock_table failed 0x%.8x\n", status));
          return (status);
        }
  ...
  {
    DMhighlight_files = (MEMptr) file_list;
    if (_NFMdebug_st.NFMdebug_on)
    {

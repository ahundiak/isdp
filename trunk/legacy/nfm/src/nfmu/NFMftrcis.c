...
long NFMftr_checkin_set (catalog_name, catalog_no, set_name, set_revision,
                         set_no, working_area, wf_info, member_operation,
                         operation, level, files, keywords)

char *catalog_name; /* set's catalog */
long catalog_no;
char *set_name; /* set's name */
char *set_revision; /* set's revision */
long set_no;
...

{
  ...
  _NFMdebug ( (fname, "ENTER: cat_name[%s] set_name[%s] set_rev[%s]\n\
                       ...
                       operation.purge[%d] member_operation[%d]\n\
                       level %d files addr %ld keywords addr %ld",
  ...
  status = NFMcheckin_set (catalog_name, catalog_no, set_name, set_revision,
             set_no, working_area, wf_info, member_operation, operation, level);

  if (status != NFM_S_SUCCESS)
  {
    _NFMdebug ((fname, "NFMcheckin_set 0x%.8x\n", status));

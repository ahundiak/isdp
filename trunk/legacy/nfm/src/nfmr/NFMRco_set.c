...
/* 
 Doc: NFMRprepare_set_checkout
 
 Abstract:
     This function will prepare a set to be checked out.  The output buffers
     are for NFMRcomplete_set_checkout.

 */
 
long NFMRprepare_set_checkout (user_id, catalog_name, set_name, set_rev,
                               level, attr_list, data_list)

long    user_id;       /* i - user id for access */
char    *catalog_name; /* i - catalog of set */
char    *set_name;     /* i - set to checkout */
char    *set_rev;      /* i - set revision */
long    level;         /* i - how many levels to checkout */
...
char   id[20];
char   lev[20];
static char *fname = "NFMRprepare_set_checkout";
    ...
    status = NFMcl_send_buffer (&NFMRglobal.NETid, NFM_PREPARE_SET_CHECKOUT, info);
    ...
    if (NFMRmsg_ptr.debug_on)
        {
        MEMprint_buffers ("PREPARE CHECKOUT DATA", *data_list, 
                          NFMRmsg_ptr.debug_file);
        }
...
/* 
 Doc: NFMRcancel_set_checkout
 
 Abstract:
     This function will cancel the preparation of a set to be checked out.
     It must be called if a checkout has been initiated
     (NFMRprepare_set_checkout has been called) but needs to be cancelled.
     The input buffers are from NFMRprepare_set_checkout.
     
 */
 
long NFMRcancel_set_checkout (user_id, attr_list, data_list)
...
long   ret_status = 0;
char   id[20];
static char *fname = "NFMRcancel_set_checkout";
    ...
    status = NFMcl_send_buffer (&NFMRglobal.NETid, NFM_CANCEL_SET_CHECKOUT, info);
    _NFMRdebug(( fname, "Send Buffer : = <0x%.8x>\n", status));
    if (status != NFM_S_SUCCESS)
        {
        MEMclose (&info);
...
/* 
 Doc: NFMRcomplete_set_checkout
 
 Abstract:
     This function will complete the checkout of a set.  The input buffers are
     from NFMRprepare_set_checkout.
     
 */
 
long NFMRcomplete_set_checkout (user_id, storage_name, tmp_node, tmp_user,
...
long   ret_status = 0;
char   id[20];
static char *fname = "NFMRcomplete_set_checkout";
    ...
    status = NFMcl_send_buffer (&NFMRglobal.NETid, NFM_COMPLETE_SET_CHECKOUT, info);
    _NFMRdebug(( fname, "Send Buffer : = <0x%.8x>\n", status));
    if (status != NFM_S_SUCCESS)
        {
        MEMclose (&info);

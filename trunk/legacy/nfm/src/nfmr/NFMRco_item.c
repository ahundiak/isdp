...
/* 
 Doc: NFMRprepare_item_checkout
 
 Abstract:
     This function will prepare an item to be checked out.  The output buffers
     are used for NFMRcomplete_item_checkout.

 */
 
long NFMRprepare_item_checkout (user_id, catalog_name, item_name, item_rev,
                                attr_list, data_list)

long    user_id;       /* i - user id for access */
char    *catalog_name; /* i - catalog of item */
char    *item_name;    /* i - item to checkout */
...
static char *fname = "NFMRprepare_item_checkout";
    ...
    status = NFMcl_send_buffer (&NFMRglobal.NETid, NFM_PREPARE_ITEM_CHECKOUT, info);
...
/* 
 Doc: NFMRcancel_item_checkout
 
 Abstract:
     This function will cancel the preparation of an item to be checked out.
     It must be called if a checkout has been initiated
     (NFMRprepare_item_checkout has been called) but needs to be cancelled.
     The input buffers are from NFMRprepare_item_checkout.
     
 */
 
long NFMRcancel_item_checkout (user_id, attr_list, data_list)
...
static char *fname = "NFMRcancel_item_checkout";
    ...
    status = NFMcl_send_buffer (&NFMRglobal.NETid, NFM_CANCEL_ITEM_CHECKOUT, info);
    _NFMRdebug(( fname, "Send Buffer : <0x%.8x>\n", status));
    if (status != NFM_S_SUCCESS)
        {
        MEMclose (&info);
...
/* 
 Doc: NFMRcomplete_item_checkout
 
 Abstract:
     This function will complete the checkout of an item.  The input buffers
     are from NFMRprepare_item_checkout.
     
 */
 
long NFMRcomplete_item_checkout (user_id, storage_name, tmp_node, tmp_user,
...
static char *fname = "NFMRcomplete_item_checkout";
    ...
    status = NFMcl_send_buffer (&NFMRglobal.NETid, NFM_COMPLETE_ITEM_CHECKOUT, info);
    _NFMRdebug(( fname, "Send Buffer : <0x%.8x>\n", status));
    if (status != NFM_S_SUCCESS)
        {
        MEMclose (&info);

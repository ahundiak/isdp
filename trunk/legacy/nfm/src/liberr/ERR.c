...

{
  ...
  long      status;    /* i - I/NFM error status to be
                              loaded for this subsystem        */
  char      *format;   /* i - printf () format specification
                              for variables to be loaded       */

/*  ...                   i - variables to be loaded in
                               the I/NFM error structure        */
#endif
...
/* 12/8/92 - KT - SUN port */
/* Added #if stmt to take function definition for SCO. MSSP 16 Dec 93 */
#if (defined(OS_SUNOS) || defined(OS_SOLARIS) || \
     defined(OS_SCO_UNIX) || defined(OS_HPUX))
    va_start ( vars );

    type = ( int ) ( va_arg ( vars, int ) );
    status = ( long ) ( va_arg ( vars, long ) );
    format = va_arg ( vars, char * );
#endif
    ...
    if (format != NULL)
      {
        ...
        if ((token_fmt = (char *) malloc (strlen (format) + 1)) == NULL)
          {
            _ERRdebug ((fname,

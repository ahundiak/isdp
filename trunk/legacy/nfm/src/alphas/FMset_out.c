...
static char *fname = "NFAcheckout_set";

    NFAconfirm( "checkout this set", &confirm );
    if( confirm == 0 )
    ...
    NFAset_global_config_info( NFM_WORKFLOW, CHECKOUT_SET,
                               NFMworkflow, "", "NFM" );

    status = NFMRcheckout_set( NFMcatalog, catalog_no, NFMset,
	...
	NFAdisplay_set_messages( "Set Checkout Messages", msg_list );
    }
    MEMclose( &msg_list );
...
struct NFMoperation *operation;
MEMptr msg_list = NULL;
static char *fname = "NFAcheck_set_out_from_list";
...
"", "", title);
    ...
    NFAconfirm( "check out these set(s)", &confirm );
    if( confirm == 0 )
    {
	NFAfree_list_struct( &display_struct );
        return( NFI_S_SUCCESS );
    ...
    for (i=0,k=0; i<data.num_of_rows; ++i)
    {
	    ...
	    status = NFMRcheckout_set( NFMcatalog, catalog_no, data.data[j], 

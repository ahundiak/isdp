...
char status_flag[2];
char *sort_str;
char *new_sort_str = NULL;
char **rep_ptr;
struct NFMquery_info query_info;
MEMptr rep_buff = NULL;
long CVTget_report_name ();
FILE *fopen(), *out_file;
static char *fname = "CVTsort_files";

_NFMdebug ((fname, "Enter.\n"));

/* see whether or not sort criterias have been converted */
status = CVTquery_cvtlog_entry ("sort_criteria", status_flag, &last_row);
if (status == NFM_S_SUCCESS)
    {
    _NFMdebug ((fname, "Sort Criterias have already been converted.\n"));
    fprintf (stderr, "Sort Criterias have already been converted.\n");
...
sprintf (sql_str, "select %s from o_nfmreports where n_type = 'O'",
...
if (status != SQL_S_SUCCESS)
    ...
    if (status != SQL_I_NO_ROWS_FOUND)
        ...
        {
        _NFMdebug ((fname, "SQLquery: No sort criterias found: <0x%.8x>\n",
                    status));
        return (NFM_S_SUCCESS);
        }
    }
...
/* for each sort string, convert the report */
for (i=0; i<rep_buff->rows; ++i)
    ...
    _NFMdebug ((fname, "\nConverting sort criteria <%s>...\n", reportname));
    fprintf (stderr, "\nConverting sort criteria <%s>...\n", reportname);
    fprintf (out_file, "\nConverting Sort Report <%s>:\n", reportname);
    ...
    if (CONTINUE)
        {
        /* read OLD report */
        status = CVTread_report_sort (report_file, reportname, &sort_str);
        if (status != NFM_S_SUCCESS)
            {
            _NFMdebug ((fname, "CVTread_report_sort failure: <0x%.8x>\n", status));
            fprintf (stderr, "CVTread_report_sort failure: <0x%.8x>\n", status);
            fprintf (stderr, "CVTread_report_sort failure: <%s>\n", report_file);
            fclose (out_file);
            CONTINUE = FALSE;
            }
        }
    ...
    if (CONTINUE)
        ...
        status = CVTload_definition2 (sort_str, &new_sort_str);

        NFMwrite_query_struct (&query_info, "", "CATALOG_NAME a", "", new_sort_str, "");
        }

    if (CONTINUE)
        {
        fprintf (out_file, "    report file  : <%s>\n", report_file);
        fprintf (out_file, "    old/new sort : <%s>\n", sort_str);
        ...
        status = CVTmodify_query (reportname, query_desc, type, application,
                                  apptype, query_info, &new_sort_no);
        if (status != NFM_S_SUCCESS)
            {
    ...
    if (CONTINUE)
        {
        /* update nfmusers table -> set old n_sortno to new one */
        sprintf (sql_str, "UPDATE NFMUSERS SET n_sortno = %d WHERE \
n_sortno = %s", new_sort_no, itemnum);
...
status = CVTupdate_cvtlog ("sort_criteria", "C", 0);;

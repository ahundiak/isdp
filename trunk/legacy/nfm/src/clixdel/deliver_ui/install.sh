...
curnfmver=`grep SR01095AA-0305A /usr/ip32/ingrconfig | cut -f4 -d"#"`
...
echo "ware is now delivered with I_NFMSRV (SR01094AA-0305A)."
...
#
        ...
        if [ "$PREPROCESS" = "Y" -o "$PREPROCESS" = "y" ]
        then
        echo "\nThe I/NFM BLUbulkloader Interface is available"
        echo "built on I/Forms or X/Forms (Also called The DataEntry Tool).\n"
        fi
        ...
        formsblkans=`getanswer "Install I/NFM I/Forms BLUbulkloader Interface software (y/n/h) ?" "n"`
            ...
            *)
                ...
                if [ "$formsblkans" = "y" ]
                then
                dmdbtools=`getanswer "Enter BLUbulkloader Interface base directory." "/usr/dmd_mgr"`
                    xformsblkans=n
                    flsans=y
                    msgblkans=y
                    riswarn=y
    ...
    if [ "$formsblkans" = "n" ]
     then
        ...
        while test $cant_decide -eq 1
        do
        xformsblkans=`getanswer "Install I/NFM X/Forms BLUbulkloader Interface software (y/n/h) ?" "n"`
            case "$xformsblkans" in
            [Hh]*)
                echo "\nThe I/NFM X/Forms BLUbulkloader Interface is an X/Forms-based \c"
                echo "Utility for Dmands Bulkloader "
                echo "The I/NFM X/Forms BLUbulkloader Interface requires approximately \c"
            ...
            *)
                ...
                if [ "$xformsblkans" = "y" ]
                then
                dmdbtools=`getanswer "Enter the I/NFM X/Forms BLUbulkloader Interface base directory." "/usr/dmd_mgr"`
                    flsans=y
                    msgblkans=y
                    riswarn=y
                fi
...
#    
        ...
        blkans=`getanswer "Install I/NFM Alpha BulkLoader Interface software(y/n/h) ?" "n"`
            case "$blkans" in
            [Hh]*)
                echo "\nThe I/NFM Alpha BulkLoader can be used to quickly \c"
...
#
      if [ "$formsans" = "y" -o "$xformsans" = "y" -o "$xformsblkans" = "y" -o "$formsblkans" = "y" ]
        then
            cant_decide=1
            while test $cant_decide -eq 1
            do
...
then
    ...
    if [ "$deliver_c100" = "y" ]
    then
        getfile nfmdbasrv.p100 | compress -d | cpio -ivmud >>$LOGFILE 2>&1
        chmod +s ${UNIXDIR}/bin/dbasrv    >>$LOGFILE 2>&1
    fi
    if [ "$deliver_c400" = "y" ]
    then
        getfile nfmdbasrv.p400 | compress -d | cpio -ivmud >>$LOGFILE 2>&1
        chmod +s ${UNIXDIR}/bin/dbasrv    >>$LOGFILE 2>&1
    fi
    AOK=$?
  if [ "$AOK" != "0" ] ; then exit $AOK; fi
...
then
    ...
    AOK=$?
    if [ "$AOK" != "0" ] ; then exit $AOK; fi
    echo "export DMDS_MGR_BASE=$dmdbtools" >${UNIXDIR}/bin/DataEntry.sh
    echo "export MGR_LOCATION=$UNIXDIR" >>${UNIXDIR}/bin/DataEntry.sh
    echo "export BLK_LOCATION=$UNIXDIR" >>${UNIXDIR}/bin/DataEntry.sh
    echo '$MGR_LOCATION/bin/DataEntry.exe $*' >>${UNIXDIR}/bin/DataEntry.sh
    chmod 555 ${UNIXDIR}/bin/DataEntry.sh
    chmod +s ${UNIXDIR}/bin/DataEntry.sh	
    chmod 555 ${UNIXDIR}/bin/DataEntry.exe
    chmod +s ${UNIXDIR}/bin/DataEntry.exe	
    /bin/ln -s ${UNIXDIR}/bin/DataEntry.sh /usr/bin/DataEntry
fi

#  Load X/Forms Forms BLUbulkload UI
if [ "$xformsblkans" = "y" ]
then
    Check_Space $UNIXDIR $Fbx_Size "I/NFM X/Forms User Interface"
    echo Installing I/NFM X/Forms BLUbulkloader User Interface 
    echo Installing I/NFM X/Forms BLUbulkloader User Interface >>$LOGFILE 2>&1
    date >>$LOGFILE 2>&1
    ...
    echo "export DMDS_MGR_BASE=$dmdbtools" > ${UNIXDIR}/bin/DataEntryx.sh
    echo "export MGR_LOCATION=$UNIXDIR" >> ${UNIXDIR}/bin/DataEntryx.sh
    echo "export BLK_LOCATION=$UNIXDIR" >> ${UNIXDIR}/bin/DataEntryx.sh
    echo '$MGR_LOCATION/bin/DataEntryx.exe $*' >> ${UNIXDIR}/bin/DataEntryx.sh
    chmod 555 ${UNIXDIR}/bin/DataEntryx.sh
    chmod +s ${UNIXDIR}/bin/DataEntryx.sh	
    chmod 555 ${UNIXDIR}/bin/DataEntryx.exe
    chmod +s ${UNIXDIR}/bin/DataEntryx.exe	
    /bin/ln -s ${UNIXDIR}/bin/DataEntryx.sh /usr/bin/DataEntryx
fi
...
if [ "$xworkflans" = "y" ]
then
    Check_Space $UNIXDIR $Wfg_Size "I/NFM X/Forms Workflow Interface"

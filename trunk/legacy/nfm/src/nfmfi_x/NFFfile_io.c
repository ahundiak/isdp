...
long _NFFfile_io_confirm_notify (g_label, form)
  int       g_label;
  Form      *form;
  {
    auto    long              status = NFI_S_SUCCESS;
    auto    NFFconfirm_ptr    ptr = NULL;
    static  char              *fname = "_NFFfile_io_confirm_notify";
    ...
    switch (g_label)
      {
        ...
        case OK_BUTTON:
            ...
            status = NFFglobal.NFMRfunction[0] (ptr->entity1, (long) 0,
                          ptr->entity2, ptr->entity3, NFFglobal.working_area,
                          NFFwf_info, NFFoperation);
            ...
            /*  See if this item has become a set (MicroStation)  */

            if (status == NFM_I_ITEM_PROMOTED_TO_MS_SET)
              {
                _NFIdebug ((fname, "item was promoted to a set\n"));

...
/*****************************************************************************
 *                                                                           *
 *   This function is used to process the deletion of flagged items.         *
 *                                                                           *
 *****************************************************************************/
...
long _NFFdelete_flagged_items ()
  {
    ...
    NFFglobal.NFMRfunction[0] = NFMRdelete_files;

    NFFform_message (CURRENT_LABEL, NFI_I_RETRIEVE, NULL);

    /* Call the NFMRquery_delete function to get the information */

    status = NFMRquery_delete (&(ptr->item_list));
    if (status != NFM_S_SUCCESS)
      {
        _NFIdebug ((fname, " NFMRquery_delete : status = <%d>\n", status));
        NFFerase_message ();
    ...
    if (ptr->item_list == NULL)
     {
        status = NFI_W_NO_ITEMS_FLAGGED_D;
    ...
    if ((status = (long) FIf_new (AB_UTILITY_LABEL, DELETE_UTILITY_FORM,
    ...
    if ((status = (long) UMSGetByNum (title, NFT_P_DELETE_FLAGGED_ITEMS,
                                      NULL)) != UMS_S_SUCCESS)
     {
        _NFIdebug ((fname, "UMSGetByNum = <0x%.8x>\n", status));
        _NFIdebug ((fname, "defaulting title\n"));
        strcpy (title, "Delete Flagged Items");
     }

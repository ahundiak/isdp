...
archive: $(XTEST_A) $(FORMS_ARLIB)
	...
	then \
		echo ""; \
		echo "Build static with $(FORMS_ARLIB) for $(OS_KEY)"; \
		echo " ... using $(SUN_XLIB) on $(OS_KEY)"; \
		set -x ; \
		$(CC) $(CFLAGS) -o $(AREXE) $(XTEST_A) -Bstatic \
		$(XC)/lib/libxcmalloc.a $(FORMS_ARLIB) \
		-Bdynamic $(SUN_FS_LIB) $(SUN_XLIB) \
		-lm -lc; \
		set +x ; \
	elif [ "$(OS_KEY)" = "SUNOS5" ]; \
	then \
		...
		then \
			...
			echo " ... using $(SUN_XLIB) on $(OS_KEY)"; \
			set -x ; \
			$(CC) $(CFLAGS) -o $(AREXE) $(XTEST_A) -Bstatic $(FORMS_ARLIB) \
			-Bdynamic $(SUN_FS_LIB) $(SUN_XLIB) \
			-lmalloc -lm; \
			set +x ; \
		else \

...
/*************************************/
/* */
/* This file contains functions for the "delete group" command. */
/* */
...
/* flag is set to DELETE_GRP indicating that this command is */
...
void kill_delete_group(void)
{
  char t_str[256];

  if (command_is_active == DELETE_GROUP)
  {
     command_is_active = 0;
...
void execute_delete_group(void)
{
  char t_str[256];
  int status;
  int grp_gadget;
  ...
  {
    UMSGetByNum (t_str, FB_I_DELGRP, 0);
    ...
    while (not_finished)
    {
      ...
      if (status == 2) /* Success */
      {
        ...
        else
        {
          ...
          UMSGetByNum (t_str, FB_P_ACCREJDELGRP, 0);
          FIfld_set_text (Message_form, 33, 0, 0, t_str, 0);
          ...
          if (status == 2) /* Success */
          {
            /*******************************************************/
            /* */
            /* Check each gadget on the form to see whether it */
            /* is a member of the active group. If it is, move */
            /* it to the forms group, and reset it to its original */
            /* displayed/highlighted state. */
            /* */
            /* Finally delete the Active group gadget. */
            /* */
            /*******************************************************/
            ...
            for (x = 0; x < Total_gadgets; x++)
            {
              ...
              {
                FIg_add_to_group (work_form, seq_list[x].gadget, 0);
            ...
            /* FIgrp_delete (work_form, Active_group); */
          }
          else
          {
            /*******************************************************/
            /* */
            /* Check each gadget on the form to see whether it */
            /* is a member of the active group. If it is, reset */
            /* it to its original displayed/highlighted state. */
            /* */
            /*******************************************************/

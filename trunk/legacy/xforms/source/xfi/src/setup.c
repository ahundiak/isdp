                          ...
                          int  *num_dirs)
{
   ...
   *num_dirs = 0;
   ...
   /* ISDP Bump from 10 to 100 */
   space_available = 100;

   *dirs = (char **) calloc (100, sizeof (char *));
...
if (fontname) {
  printf("FI_load_font '%s' '%d'\n",fontname,bodysize);
  /*if ((!strcmp(fontname,"swiss742")) && (bodysize == 25)) bodysize = 18;*/
}
else          printf("FI_load_font NULL fontname\n");

if ((!strcmp(fontname,"swiss742") && bodysize == 25)) bodysize = 18;

  if ( _FI_font_available (fontname, bodysize, font_id, font_type) )
    {
printf("FI_load_font success 1\n");
      return ( FI_SUCCESS );
    }
printf("FI_load_font 2\n");
  ...
  if ( bodysize < 0 )
   {
	...
	/****************************************/
     ...
     fprintf(stderr, "Can't load font %s with size %d\n", fontname,
		bodysize);
     *************************************************************/
printf("FI_load_font load error\n");
  ...
  if ( FI_font_strategy == FI_FS_FONTS_FIRST )
  {
     ...
     /***************************************************************/
printf("FI_load_font FI_FS_FONTS_FIRST 1\n");
...
printf("FI_load_font FI_FS_FONTS_FIRST 2\n");
...
printf("FI_load_font FI_FS_FONTS_FIRST 3\n");
 	   status = FSNewFont ( &fontspec, NULL, font_id );
printf("FI_load_font FI_FS_FONTS_FIRST 4\n");
...
printf("FI_load_font FI_X_FONTS_FIRST 1\n");
...
printf("FI_load_font FI_X_FONTS_FIRST 2\n");
    Xfont_list = XListFonts ( _FI_display, FI_data, 2, &num_fonts );
printf("FI_load_font FI_X_FONTS_FIRST 3\n");
...
/* ---------------------------------------
 * ISDP Question, why is the number of forms
 * not tied to the MAX_WINDOW, they seem
 * to track in our product
 */
#if 1
#define ISDP_FORM_LIST_SIZE 250
#define ISDP_SYM_LIST_SIZE   50
#define ISDP_FONT_LIST_SIZE  50
#else
#define ISDP_FORM_LIST_SIZE  28
#define ISDP_SYM_LIST_SIZE    5
#define ISDP_FONT_LIST_SIZE   5
#endif
              ...
              int       cmap_size)
{
  ...
  char * font_strat_env;
   ...
   _FI_grp_alloc ( _FI_form_list, ISDP_FORM_LIST_SIZE );
...
*/
  ...
  for ( xx = 0; xx < MAX_WINDOW; xx++ )
   ...
   _FI_symbol_array_size = ISDP_SYM_LIST_SIZE;
  ...
  _FI_font_array_size = ISDP_FONT_LIST_SIZE;

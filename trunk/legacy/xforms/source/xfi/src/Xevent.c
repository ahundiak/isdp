                      ...
                      unsigned int trans)
{
  ...
  /* ISDP Check for using up resources */
  if (window_index > (MAX_WINDOW - 10)) {
    printf("#### WARNING #### XFORM WINDOWS ARE ALMOST USED UP\n");
  }

  if ( (window_index == -1) || (window_index >= MAX_WINDOW))
    {
      return ( used_event );
    }
  else

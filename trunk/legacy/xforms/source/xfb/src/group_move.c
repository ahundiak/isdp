...
/* flag is set to GROUP_MOVE indicating that this command is */
...
void kill_group_move(void)
{
  char t_str[256];

  if (command_is_active == GROUP_MOVE)
  {
     command_is_active = 0;
...
void execute_group_move(void)
{
  ...
  {
    UMSGetByNum (t_str, FB_I_GRPMVE, 0);
        ...
        else
        {
          ...
          while (status == 2)
          {
            ...
            else if (gadget_type == FI_TEXT)
            {
              ...
              FIg_get_font (work_form, grp_gadget, font_name, &bodysize);
            ...
            if (status == 2) /* success */
            {
              ...
              for (x = 0; x < Total_gadgets; x++)
              {
                ...
                if (temp_group == Active_group)
                {
                  ...
                  FIg_set_location (work_form, seq_list[x].gadget,

...
**      INCLUDES
*/
#if defined(__clipper__) || defined(sun) || defined(__Sol2__) || defined(Soli)
#include <stdio.h>
#include <errno.h>
#include <signal.h>
#include <fcntl.h>
...
#if defined(sco) || defined(__hpux__)
#include <stdio.h>
#include <string.h>
#include <errno.h>
#include <signal.h>
...
#if defined(sco)
extern int t_errno;
#define T_FUNC_MAX_RETRIES 10
#endif

#if defined(WIN32)
#define COMPARE_NUMBER  11
#define COMPARE_SIZE    28
#define STARTUP(net) \
if (!NET_startup_called) \
{ \
	if (NET_startup_tcp(net) != 0) return; \
	NET_startup_called = 1; \
}
#endif
...
{
    "user ", "put ", "get ", "delete ", "220 ", "331 ",
    "Connected to ", "Not connected", "530 ", "221 ", "200 PORT "
};
#endif
...
#if defined(DOS)
#include "dostcpip.h"   /* must be #included after io.h */
#endif
...
/* 
** these are temporary until the network include files are
** modified for the apogee __clipper__ token.
*/
#if defined(__STDC__) && !defined(DOS) && !defined(__hpux__)
#ifdef htons
# undef htons
# undef ntohs
#endif
...
#if defined(WIN32)
static  int 		NET_startup_called = 0;
extern SOCKET	closesd, asd;
static  int		winsockerr = 0;
#endif
	...
	net_s                   *net)
{
	...
	NET_DBG(("NET_startup(net:0x%x)\n", net));

	/*
	** start winsockets
	*/
	if ((status = WSAStartup(wVerReq, &wsadata)) != 0)
	{
	    NET_DBG(("NET_startup: WSAStartup failed; status %d\n", status));
		NET_DBG(("\tVer %d.%d HighVer %d.%d szDesc %s\n", 
				 LOBYTE(wsadata.wVersion), HIBYTE(wsadata.wVersion),
				 LOBYTE(wsadata.wHighVersion), HIBYTE(wsadata.wHighVersion),
				 wsadata.szDescription));
		NET_DBG(("\tszSysStat %s MaxSockets %d iMaxUdpDg %d\n",
		     wsadata.szSystemStatus, wsadata.iMaxSockets, 
		     wsadata.iMaxUdpDg));
		NET_error_tcp(net, NET_E_STARTUP_ERROR, "");
		NET_DBG(("NET_startup:returning 1\n"));
		return 1;
	}
    NET_DBG(("NET_startup: WSAStartup succeed; status %d\n", status));
	NET_DBG(("\tVer %d.%d HighVer %d.%d szDesc %s\n", 
				 LOBYTE(wsadata.wVersion), HIBYTE(wsadata.wVersion),
				 LOBYTE(wsadata.wHighVersion), HIBYTE(wsadata.wHighVersion),
				 wsadata.szDescription));
	...
	/*
	** Confirm that winsock DLL supports 1.1
	*/
	if (LOBYTE(wsadata.wVersion) != 1 || HIBYTE( wsadata.wVersion ) != 1 )
	{
		NET_DBG(("NET_startup: winsock dll supports max Ver %d.%d\n", 
				 LOBYTE( wsadata.wVersion ), HIBYTE( wsadata.wVersion )));
		if ( WSACleanup() )
		{
			NET_DBG(("NET_startup WSACleanup error %d\n", WSAGetLastError()));
		}
		NET_error_tcp(net, NET_E_STARTUP_ERROR, "");
		NET_DBG(("NET_startup:returning 1\n"));
		return 1;
	...
	/*
	** Close risinetd listening socket
	*/	
	if (net->cs_flag == SERVER_FLAG) {
		status = closesocket( closesd );
		if ( status == SOCKET_ERROR ) {
			NET_DBG(("NET_startup: closesocket %d call failed: error %d\n",
					 closesd, WSAGetLastError()));
		}
		NET_DBG(("NET_startup: closed parent socket %d\n", closesd));
	}
	NET_DBG(("NET_startup:returning 0\n"));
	return 0;
}
#endif
...
{
	NET_DBG(("NET_cleanup_tcp()\n"));
	if (NET_startup_called)
	{
		if ( WSACleanup() ) {
			NET_DBG(("NET_cleanup_tcp WSACleanup error %d\n",
				WSAGetLastError()));
		}
		NET_startup_called = 0;
	}
	NET_DBG(("NET_cleanup_tcp:returning\n"));
}
#endif

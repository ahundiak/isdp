...
/******************************************************************************
	File Name:		winhan.c
	Author:			Rick Kramer
	Description:	This program contains Windows message handling functions.
	Change History:
******************************************************************************/
#include <windows.h>
#include <windowsx.h>
...
/* -----------------------------------------------------------------------  */
BOOL wmhandler_isextramsg(WMTAB wmTab, WPARAM wMsg, WPARAM *pwExtraMsg)
{
	UINT i;
...
/* -----------------------------------------------------------------------  */
WMHANDLER wmhandler_getextratab(WMTAB wmTab, WORD wMsg)
{
	WORD i;
...
/* -----------------------------------------------------------------------  */
WMHANDLER wmhandler_setextratab(WMTAB wmTab, WORD wMsg, WMHANDLER f)
{
	WORD i, iUse = 0xffff;
...
/* -----------------------------------------------------------------------  */
long FAR PASCAL _export wmhandler_wndproc(HWND hWnd, UINT wMsg,
...
WMHANDLER wmhandler_settab(WMTAB wmTab, WORD wMsg, WMHANDLER f)
{
	WMHANDLER oldf;
...
WMHANDLER wmhandler_set(HWND hWnd, WORD wMsg, WMHANDLER f)
{
	WMTAB wmTab = (WMTAB) GetWindowLong(hWnd, 0);
...
WMTAB wmhandler_create(void)
{
	...
	if (! (wmTab = (WMTAB) malloc(sizeof(WMHANDLERDATA))))
		return NULL;
...
{
	free((LPVOID)GetWindowLong(hwnd, 0));
...
HWND wmhandler_hwnd(char *name)
{
	...
	if (! __hPrevInst) // NOT VALID! ASSUMES LAST APP DID WMHANDLER_HWND()
	{
        ...
        wndclass.lpfnWndProc = wmhandler_wndproc;

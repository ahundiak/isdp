...
extern void RISdldLDULD_report_error(
	char **errorp)
{
	RIS_error_col = 0;
	...
	add_asterisk_line();
	add_string_line("Dynamic Loading of RIS Loader/Unloader objects/DLL has failed.");
	add_asterisk_line();
	RETURN(errorp);
}
...
extern void RISdldLDULD_error_msg_box(
	int  error_code,
	char *string)
{
	char error_string[512];

	DLD_DBG(("RISdldLDULD_error_msg_box(error_code:%d string:%s\n", 
	...
	switch(error_code)
	{
		...
		case RIS_COULD_NOT_LOAD_RIS_DLL:
			sprintf(error_string,"%s %s",
				"Unable to load RIS Loader/Unloader DLL\n", string);
		break;
	...
	DLD_DBG(("RISdldLDULD_error_msg_box: complete.\n"));
}

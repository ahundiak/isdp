...
int UMS_int_convert (int *value)
{
	...
	for (ii = 0 , vp = (char *) value,
		tp = (char *) &temp + int_size - 1;
		ii < int_size; ii++ , vp++ , tp--)
	{
		*tp = *vp;
	}
	*value = temp;
#endif	/* arch. conversion - RIS port - Surjit 05/05/95 */
...
int UMS_long_convert (long *value)
{
	...
	for (ii = 0 , vp = (char *) value,
		tp = (char *) &temp + long_size - 1;
		ii < long_size; ii++ , vp++ , tp--)
	{
		*tp = *vp;
	}
	*value = temp;
...
int UMS_short_convert (short *value)
{
	...
	for (ii = 0 , vp = (char *) value,
		tp = (char *) &temp + short_size - 1;
		ii < short_size; ii++ , vp++ , tp--)
	{
		*tp = *vp;
	}
	*value = temp;

  ...
  for (x = 1; x < (*buffer_ptr) -> rows; ++x)
  {
       ...
       n_status == NFM_NFS_MOVE || n_status == NFM_LFM_NFS_MOVE)
    {
      ...
      /* Check the source location operating system . Use the appropriate method
         ...
         The source is where the files live which are windows machines
       * so all this gets skipped and just use ftp
       */
      if ((! strcmp (data [count + FTO_OPSYS], "CLIX")) || (! strcmp (data [count + FTO_OPSYS], "UNIX")))
      {
		    connect_status = TLI_MC_CONN_LESS;

        /* SAVE THE CONNECTION FOR LATER USE JUST IN CASE NFS IS THE ONLY OPERATION */
    ...
    {
      status = NETfmu_connect (data [count + FTO_NODENAME],data [count + FTO_USERNAME],dec_pass);
      ...
      )
      {
         ...
         */
           ...
           */
				...
				status = NFMnfs_file_remove(
                data[FTO_NODENAME],
                data[count1+FTO_NODENAME],
                data[count1+FTO_PATHNAME],
                data[count1+FTO_CIFILENAME],
                data[count1+FTO_COFILENAME],NFS_CO_DIR,
                data[count1+FTO_MOUNTPOINT], 0);
				if(status != NFM_S_SUCCESS)
				{
          NFMset_buf_stat(*buffer_ptr,y+1,FTO_STATUS1+1,NFM_E_NFS_REMOVE_FILE);
			  ...
			  {
				  status = NFMnfs_file_transfer(
                  data[FTO_NODENAME],data[count1+FTO_NODENAME],data[count1+FTO_PATHNAME],
                  data[count1+FTO_CIFILENAME],data[count1+FTO_COFILENAME],NFS_CO_DIR,data[count1+FTO_MOUNTPOINT],NO_LFM_CHECK);
				if(status == NFM_S_SUCCESS)
				{
...
/* Load the error  for n_status and continue with next buffer */
			switch (status)
			{
				case NET_E_FMU_RECEIVE:
					status1 = NFM_E_FMU_RECEIVE_CO_FILENAME;
					break;
				case NET_E_FMU_SET:
				...
				case NET_E_STAT:
					...
					status2 = errno ;
status = NFMset_buf_stat ( *buffer_ptr,y+1,FTO_STATUS2+1,status2);
					break;
				default:
					status1 = NFM_E_FMU_RECEIVE_CO_FILENAME;
					break;

 ...
 *	
 * Revision 1.1  1999/04/10  21:11:52  pinnacle
 * initial revision
 *
 ...
 *	04/11/99	Alwin	Added a new structure to handle piping lines
 *
 *************************************************************************/
/***************************************************
 rtiges.h
...
extern int RT_init_read(int batch_mode,
			...
			char *db_passwd,
			int unit_flag,
			char *system_name ); /* added the last argument for
			generalizing the classification manner. Alwin */
...
/* this structure is added to take care of the Pipe Lines. Previously we assumed
that every tip of a network constitutes a piping line. Now we may have any
number of lines of a network. Alwin */

struct PipeLine_Info {
	IGRint  NbRuns;
	IGRchar seq_no[20];
        pipe_run_info run_info[20];
};

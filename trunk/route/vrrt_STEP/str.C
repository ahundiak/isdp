 ...
 * I/ROUTE
 *
 * File:	vrrt_STEP/str.C
 *
 * Description:
 *      This is another copy of the vrrt/str.C file. This might've undergone
 ...
 *	
 * Revision 1.2  1998/11/06  23:34:38  pinnacle
 * Replaced: vrrt_STEP/str.C for:  by apazhani for route
 *
 * Revision 1.1  1998/11/06  19:26:46  pinnacle
 * initial revision
 *
 *
 * History:
...
charp str;
{
  ...
  copy = (char*)diag_malloc( DIAG_FATAL, len+1 );
  strcpy( copy, str );
  /*  strdup( copy, str ); */
...
charp strncopy(str,n) charp str; int n; { charp copy;
	...
	/* type casted it. Alwin */
	copy = (char*) diag_malloc(DIAG_FATAL,n+1);
...
charp str; int sepc; int dosuffix; int dor; 
{
	...
	strcpy(buffer,str);
	ifnull (sep = (*(dor?strrchr:strchr))(buffer,sepc)) 
	     return NULL;
...
/**** Commenting this temporarily. Alwin 
#define DEFINE_FIXFUNC(name,want_suffix,want_last) \
charp self(str)name( str, sepc) \
charp str; int sepc; { \

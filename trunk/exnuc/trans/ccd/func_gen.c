...
char *argv[];
{
	...
	printf("/* - special conversion routine %s$%s - */\n\n", argv[1], argv[2]);
	...
	else
	{
		...
		sts = print_special_proc_func(argv[1], argv[2]);
		check_status(sts);
	}
...
print_special_proc_func(Total_class, Comp_class)
char *Total_class, *Comp_class;
{
	int sts;
	int i, pcount, dcount;
	DROS_p_REJECT p_reject;
...
%s$%s(classid, ME, me)\n\
short classid;\n\
struct %s_total_instance *ME;\n\
struct %s_instance *me;\n\
{\n\
\n\
	return(1);\n\
}\n", Total_class, Comp_class, Total_class, Comp_class );

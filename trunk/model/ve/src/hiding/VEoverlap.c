...
static IGRboolean VEis_span_overlapped __((
					IGRdouble            u0,
					IGRdouble            u1,
...
static IGRint VEget_span_visib_from_visib_list __(( 
					IGRdouble            u0,
					IGRdouble            u1,
					struct VEvisib_node  *visib_list,
					IGRdouble            knot_tol ));

static struct VEtemp_node *VEadd_temp_node __(( 
					IGRdouble            parm,
					struct VEedge_header *edge,
					IGRdouble            knot_tol ));

static void VEadd_visibility_region_to_temp_list __(( 
					IGRdouble            u0,
					IGRdouble            u1,
					struct VEedge_header *edge,
					IGRint               visib,
...
static void VEprocess_same_z_overlap __((
					struct VEedge_header *edge0,
					struct VEedge_header *edge1,
					IGRdouble            *e0_parm,
					IGRdouble            *e1_parm,
...
DESCRIPTION
	...
	edge we must put UNKNOWN temp nodes because the visibility of both
	edges can change at the overlap endpoints.  This function does some
	special tests to try to take the randomness out of the choice of which
	edge should get the overlap region and which edge should get the
	UNKNOWN temp nodes.
...
HISTORY
	xx/xx/xx    S.P. Rogers
	   Creation Date
	04/08/92    S.P. Rogers
	   Changed to get the span visibilities from the visib_list instead
	   of from the self-hidden span array.
*/
	...
	/* Determine the visibilities of the OVERLAP regions on both    */
	/* edges.  If we are still self-hiding, the visibilities should */
	/* be UNKNOWN.                                                  */
	if ( self_hiding )
	   {
	   vis0 = vis1 = UNKNOWN;
	...
	if ( (edge0->status & WAS_A_DUMMY) || (edge1->status & WAS_A_DUMMY) )
	   ...
	   if (elem0_can_hide_elem1)
	      VEadd_same_z_overlap_unknown_nodes( edge1, e1_parm, vis0, vis1, knot_tol1 );
	      
	   if (elem1_can_hide_elem0)
	      VEadd_same_z_overlap_unknown_nodes( edge0, e0_parm, vis0, vis1, knot_tol0 );
	   }
	else if ( VEis_span_overlapped( e0_parm[0], e0_parm[1], edge0, knot_tol0 ) ||
	          VEis_span_overlapped( e1_parm[0], e1_parm[1], edge1, knot_tol1 ) )
	   {
	   /* One of the edges is already overlapped for this */
	   /* span => put UNKNOWN nodes on both edges         */
	   if (elem0_can_hide_elem1)
	      VEadd_same_z_overlap_unknown_nodes( edge1, e1_parm, vis0, vis1, knot_tol1 );
	   if (elem1_can_hide_elem0)
	      VEadd_same_z_overlap_unknown_nodes( edge0, e0_parm, vis0, vis1, knot_tol1 );
	   }
	else if ( edge0->gm_curve3d->bspcv->order > edge1->gm_curve3d->bspcv->order )
	   {
	   /* Curve0 has a higher order than curve1 => put overlap */
	   /* on edge1 and UNKNOWN nodes on edge0                  */
	...
	else if ( edge1->gm_curve3d->bspcv->order > edge0->gm_curve3d->bspcv->order )
	   {
	   /* Curve1 has a higher order than curve0 => put overlap */
	   /* on edge0 and UNKNOWN nodes on edge1                  */
	   ...
	   if ( edge0_totally_overlaps_edge1 )
	      {
	      /* Edge0 totally overlaps edge1=> put overlap */
	      /* on edge1 and UNKNOWN nodes on edge0        */
	      VEadd_overlap_region( e1_parm[0], e1_parm[1], edge1,
	                            (elem0_can_hide_elem1 ? UNKNOWN : VISIBLE) , knot_tol1 );
	      ...
	      {
	      /* Edge1 totally overlaps edge0=> put overlap */
	      /* on edge0 and UNKNOWN nodes on edge1        */
	      VEadd_overlap_region( e0_parm[0], e0_parm[1], edge0,
	                            (elem1_can_hide_elem0 ? UNKNOWN : VISIBLE), knot_tol0 );
	      ...
	      {
	      VEadd_overlap_region( e1_parm[0], e1_parm[1], edge1, UNKNOWN, knot_tol1 );

	      if (elem1_can_hide_elem0)
	         VEadd_same_z_overlap_unknown_nodes( edge0, e0_parm, vis0, vis1, knot_tol0 );
	      }
	   else if ( elem1_can_hide_elem0 )
	      VEadd_overlap_region( e0_parm[0], e0_parm[1], edge0, UNKNOWN, knot_tol0 );
	   else
	      VEadd_overlap_region( e0_parm[0], e0_parm[1], edge0, VISIBLE, knot_tol0 );

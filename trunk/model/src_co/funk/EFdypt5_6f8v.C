...
extern EFdyrbpt5_6f8v();

struct pt5_struct
{
 struct DPele_header ele_header[4];
 IGRvector           h_vec[2], w_vec;
};


IGRint EFdypt5_6f8v( pt1, pt2, pt3, pt4, display )
...

{
 ...
 for (i=0; i<3; i++)
 {
  header.h_vec[0][i] = pt4[i] - pt1[i];
  header.h_vec[1][i] = pt3[i] - pt2[i];
  header.w_vec[i] = pt2[i] - pt1[i];
 }

 /**MOVE THE INPUT POINTS TO THE POLYLINE POINT BUFFER**/
  OM_BLOCK_MOVE(pt1,pt_buffer,size_pnt);   
  OM_BLOCK_MOVE(pt2,&pt_buffer[10][0],size_pnt);    
  OM_BLOCK_MOVE(pt3,&pt_buffer[8][0],size_pnt);
 ...
 polyline[1].num_points = 4;
 polyline[1].points = &pt_buffer[4][0];
 polyline[2].num_points = 2;
 polyline[2].points = &pt_buffer[8][0];
 polyline[3].num_points = 2;
 polyline[3].points = &pt_buffer[10][0];
 ...
 /*  INVOKE DYNAMICS  */
 dp$dynamics( dyn_fun = EFdyrbpt5_6f8v, information = &header );
...
/*
NAME    EFdyrbpt5_6f8v -->  function called by dp$dynamics to put new
                            stuff in
DESCRIPTION
...
IGRint EFdyrbpt5_6f8v( header, cursor_pt, matrix, objects, num_objects,
 ...
 IGRchar               **out_dummy1, **out_dummy2, **out_dummy3;
{
 ...
 ptr->points[6] = cursor_pt->x + header->h_vec[0][0];
 ptr->points[7] = cursor_pt->y + header->h_vec[0][1];
 ptr->points[8] = cursor_pt->z + header->h_vec[0][2];

 /**SECOND POLYLINE**/
 OM_BLOCK_MOVE(&header->ele_header[0].geometry.polyline->points[3], 
               &header->ele_header[1].geometry.polyline->points[0], 
               sizeof(IGRpoint));

 ptr = header->ele_header[1].geometry.polyline;
 ptr->points[3] = cursor_pt->x + header->w_vec[0];
 ptr->points[4] = cursor_pt->y + header->w_vec[1];;
 ptr->points[5] = cursor_pt->z + header->w_vec[2];;

 ptr->points[6] = cursor_pt->x + header->w_vec[0] + header->h_vec[1][0];
 ptr->points[7] = cursor_pt->y + header->w_vec[1] + header->h_vec[1][1];
 ptr->points[8] = cursor_pt->z + header->w_vec[2] + header->h_vec[1][2];

 ptr->points[9] = cursor_pt->x + header->h_vec[0][0];
 ptr->points[10] = cursor_pt->y + header->h_vec[0][1];
 ptr->points[11] = cursor_pt->z + header->h_vec[0][2];
 ...
 /**FOURTH POLYLINE**/
 OM_BLOCK_MOVE(&header->ele_header[1].geometry.polyline->points[3], 

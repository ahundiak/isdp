...
int	curve_element( src_file, out_file, line, end_token )
...
int	end_token;	/* token to end with */
	...
	if( !( _order & attrs_seen ) )
		{
		fprintf( stderr, "line %d: curve order not specified\n", *line );
		return HSplot_error;
		}

	if( !( _poles & attrs_seen ) )
		{
		fprintf( stderr, "line %d: no pole for curve\n", *line );
		return HSplot_error;
		}
	...
	if( _non_uniform & attrs_seen )
		...
		if( num_knots !=( order + num_poles ) )
			{
			fprintf( stderr, "line %d: need %d knots for curve\n", *line,
					order+num_poles );
			return HSplot_error;
			}
		}
	...
	if( _rational & attrs_seen )
		...
		if( num_weights != num_poles )
			{
			fprintf( stderr, "line %d: need %d weights for curve\n", *line,
					num_poles);
			return HSplot_error;
			}
		}
...
dup_error:
	fprintf( stderr, "line %d: duplicate curve attribute \"%s\"\n",
			*line, token_str );

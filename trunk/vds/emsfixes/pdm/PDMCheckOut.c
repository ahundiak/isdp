/* $Id: PDMCheckOut.c,v 1.1 2001/03/13 16:39:03 ad Exp $ */
/*----------------------------------------------------------------
 * I/VDS
 *
 * File:        vds/emsfixes/pdu1/PDMCheckOut.c
 *
 * Description: 
 *
 * Revision History
...
/* Added check to see if the part was archived.. MaC 041494 */
	...
	if( !status1 )
	{
	    _PDMdebug(fn, "File not known to PDM, but with the same filename as the file n_cofilename if found in cwd : <%s>\n", PDMpart.n_cofilename);
	    msg = (char *)PDUtranslate_message(PDS_M_ALREADY_LOCAL);
	    sprintf(msg_str,"ABORTING RETRIEVE PROCESS - <%s> %s",
						    PDMpart.n_cofilename, msg);
//	    UI_status(msg_str);

	    _PDMdebug(fn, "Aborting command because of : <%s>\n",
							 PDMpart.n_cofilename);
	    /*

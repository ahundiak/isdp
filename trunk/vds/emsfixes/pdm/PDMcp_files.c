/* $Id: PDMcp_files.c,v 1.1 2001/03/13 16:39:03 ad Exp $ */
/*----------------------------------------------------------------
 * I/VDS
 *
 * File:        vds/emsfixes/pdu1/PDMcp_files.c
 *
 * Description: 
 *
 * Revision History
...
long PDMreview_checkout_process ()
{
  char 		*fname="PDMreview_checkout_process";
  char 		msg_str[512];
  char 		dumb_str[1];
  char 		*msg = NULL;
  ...
  int		x;
    ...
    char			*working_area, cpw_SetInd[7];
    char			nodename[NFM_NODENAME+1],
				username[NFM_USERNAME+1], passwd[NFM_PASSWD+1],
				pathname[NFM_PATHNAME+1],
				devicetype[NFM_DEVICETYPE+1],
				machid[NFM_MACHID+1], opsys[NFM_OPSYS+1],
				tcpip[NFM_TCPIP+1], xns[NFM_XNS+1],
				decnet[NFM_DECNET+1], netware[NFM_NETWARE+1],
				plattername [NFM_PLATTERNAME+1],
    ...
    if( strlen(PDMpart.n_cofilename) )
    {
	status1 = stat ( PDMpart.n_cofilename, &file_stat );
	if( !status1 )
	{
	    _PDMdebug(fname, "File with the same filename as file %s found in cwd\n", PDMpart.n_cofilename);
	    msg = (char *)PDUtranslate_message(PDS_M_ALREADY_LOCAL);
	    sprintf(msg_str,"ABORTING RETRIEVE PROCESS - <%s> %s",
						    PDMpart.n_cofilename, msg);
	    //	    UI_status(msg_str);

	    _PDMdebug(fname, "Aborting command because of : <%s>\n",
							 PDMpart.n_cofilename);
    ...
    /* STEP 2: Find if it's an assembly (query for n_setindicator in catalog */

    cpw_SetInd[0] = '\0';
    status = PDMquery_assembly_ind( PDMexec->catalog, PDMexec->part_num,
    				    PDMexec->revision, cpw_SetInd);
    if (status != PDM_S_SUCCESS)
    {
	_PDMdebug (fname,"Unable to query assembly indicator: status=<0x%.8x>\n",
								       status);
//	PDMcancel_checkout_part(NULL, NULL, NULL);
    ...
    {
	/* Not an assembly */
	_PDMdebug (fname,"%s is not an assembly", PDMpart.n_cofilename );
    ...
    status = NFMget_storage_info( working_area, &sano, nodename, username,
				  passwd, pathname, devicetype, plattername,
				  partition, nfs, compress, machid, opsys,
				  tcpip, xns, decnet, netware );
    ...
    status=_NFMbuild_working_buffer_format(format, NFM_TEMP_SIZE1, files_info);
    if (status != NFM_S_SUCCESS)
    {
	MEMclose (&files_info);
	PDMrollback_transaction (1200);
...
if (n1 == -1 || n2 == -1 ||
  ...
  /* Anand: Add information related to working area */
  strcpy(working_area, storage->storage_name);
...
*/
  ...
  /* Added by kumar to check for the override of the local filename 
...
#ifdef COMMENTED_FOR_ETL
       msg = (char *)PDUtranslate_message(PDS_M_ALREADY_LOCAL);
      sprintf(msg_str,"<%s> %s", data[index+i_cofilename],msg);
     UI_status (msg_str);
     msg = (char *)PDUtranslate_message(PDS_M_NOT_MANAGED_BY_PDM);
     sprintf(msg_str,"<%s> %s", data[index+i_cofilename], msg);
     UI_prompt (msg_str);
     strcpy(answer,"Y");
     do
    ...
    /* if (strlen (data[index+i_cifilesize]))
     ...
     if(!strcmp((sto_list+y)->nfs,"Y"))
      strcat (row_str, "2033\1\1\1");
     else
/* This caused all the children files to be brought loca
   with  read-write permissions during checkout

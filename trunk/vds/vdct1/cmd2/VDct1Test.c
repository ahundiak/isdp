/* $Id: VDct1Test.c,v 1.3 2001/10/18 20:43:29 ahundiak Exp $  */
 ...
 * Revision History:
 *      $Log: VDct1Test.c,v $
 *      Revision 1.3  2001/10/18 20:43:29  ahundiak
 *      ah
 *
...
IGRstat VDct1CmdTestStdPartsTree()
{
  ...
  // Expand A's
  VDfrmSetActiveRow(treeForm,gadget,2,1);
  VDfrmSetSelect   (treeForm,gadget,2,1,1);
  VDct1CmdNotifyRootLocate(treeForm,gadget,NULL);

  // Expand First A
  VDfrmSetActiveRow(treeForm,gadget,3,2);
  ...
  // Pick first part
  VDfrmSetActiveRow(treeForm,gadget,4,3);
  VDfrmSetSelect   (treeForm,gadget,4,4,1);
  VDct1CmdNotifyRootLocate(treeForm,gadget,NULL);
  ...
  // Cycle
  VDfrmSetActiveRow(treeForm,gadget,5,4);
  VDfrmSetSelect   (treeForm,gadget,5,1,1);
  VDct1CmdNotifyRootLocate(treeForm,gadget,NULL);
 ...
 * 18 Oct 2001 - Obsolete, VDxdoM no longer useful
 */
IGRstat VDct1CmdTestLoadXml()
{
...
#if 0  
  // Make sure initted
  VDdonInitSystem(0,0,0);

  // Get the manager
  VDdonGetMgr(NULL_OSNUM,1,&mgrID);
  VDASSERTW(mgrID.objid != NULL_OBJID);

  // Have any sets?
  VDctxGetNthMgrChild(&mgrID,0,&setID);
  ...
  VDdonCreateTreeFromXmlFile("test1.xml",&treeID);
  VDASSERTW(treeID.objid != NULL_OBJID);

  // Change it to a set and hook to the manager
  VDdonConvertToSetObj(&treeID);
  
  VDdonAppendChildObj(&mgrID,&treeID);
#endif

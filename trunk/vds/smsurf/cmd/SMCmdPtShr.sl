/* $Id: SMCmdPtShr.sl,v 1.1.1.1 2001/01/04 21:07:38 cvs Exp $  */

/* --------------------------------------------------------------------
 * I/VDS
 *
 * File:	vds/smsurf/cmd / SMCmdPtShr.sl
 *
 * Description:
 *
 * Dependencies:
...
/*    File          SMCmdPtShr.sl
       Description   Command object to place a Sheer Deck (projection of a 
 		    profile defined by pts )_
       Auteur        Pascale Lacroix (ISDC) 
       Date          Oct-92     
 */
 
command_string       SMC_M_PlShDcDX,0,SMPlShDcDX
synonym	      	     SMC_M_MdShDcDX,1,SMMdShDcDX
command_table        "sman.cmd"
class                SMCmdPtShr
super_class          SMCmdSurf
product_name         "$VDS"
 
form "SMPtSheer",0,0,"-1","-1"
form "SMPtShDef",1,0,"-1","-1"
...
instance 
{
 ...
 int			nb_wrk;
 struct def_pts		wrk_frame[MAX_PT];
 struct GRobj_env	dep_frame;
 struct def_pts		frame[MAX_PT];
 char			dir_axis;
 char			z_axis;
 IGRdouble		z_dep;
 IGRdouble		dep_dx;
 ...
 struct def_pts		old_frame[MAX_PT];
 char			old_dir_axis;
 char			old_z_axis;
 IGRdouble		old_z_dep;
...
state start
  ...
  on CMD_TYPE = PLACE	do message_key SM_M_PlShDcDX
                        do display_form  0
   			state modif_form 
   				
   on CMD_TYPE = MODIFY	do message_key SM_M_MdShDcDX
			state loc_macro
   ...
   on GR_UNKNOWN_TYPE.SLOLD 	do FormOf
   				do FormPos
 				do display_form 1 state def_pts
 ...
 state loc_macro
   prompt_key        SM_P_IdPtSh
   accept_key        SM_P_PlAccRej
   locate_class	     "SMPtSheer"
 ...
 state load_frame
   ...
   on EX_DATA or EX_STRING or EX_OBJID 
 			do store_frame  
                        		...
                        		on RETURN_CODE = INV_LOC	state .
                        		state .
   on EX_BACK_UP		state def_pts
   on EX_RJT_MOVEON	state def_pts
 ...
 /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
 /* action defined in SMPtShrNtf.I                                           */
 ...
 if(me->mytype == MODIFY) 
	...
	(void)GetDefName("SMPtSheer_0",me->def_name);
 ...
 if ( form_ptr == me->forms[0].form_ptr ){
 	...
 	switch( gadget_label ){
		...
		if(me->dep_frame.obj_id.objid == NULL_OBJID){
			...
			break;
     		...
     		stat = om$send(msg = message SMCmdPtShr.resetdef(&test),
			 targetid = my_id);
     		if(!(stat & test & 1)){
			printf("ERROR SMCmdPtShr.resetdef\n");
			FIg_set_state_off(form_ptr,gadget_label);
			goto quit;
      		}   
     		if(me->mytype == MODIFY || me->nb_pt){
 	...
 	switch( gadget_label ){
  		...
  		case FI_EXECUTE:
     		...
     		stat = om$send(msg = message SMCmdPtShr.savedef(&test),
		      targetid = my_id);
     		if(!(stat & test & 1)){
			FIg_set_state_off(form_ptr,gadget_label);
			goto quit;
		...
		break;	
   		...
   		case FI_CANCEL:
     		stat = om$send(msg = message SMCmdPtShr.cleardef(&test),
			 targetid = my_id);

/*
	I/STRUCT
	"Split Beam at Points" command.
*/
command_string	VSC_M_VSSpBmPt,0,VSSpBmPt
...
state start
	...
	/*
	 * Command shall not erase/redisplay the points.
	 */
	execute			setFlags( VS_m_POnoOperatorErase )
	on ELSE state main

state main
	prompt_key		VS_P_IdBmOrGr
	accept_key		VS_P_AccW1stPnt
	construct_class		"VSpntBeam"
	/*
...
state getFirstPoint
	...
	prompt_key		VS_P_Id1stPnt
	accept_key		VS_P_AccW2ndPnt
	locate_class		"+GRpoint,GRcurve,EMSsubbs"
	locate_properties	"LC_DP_ONLY | LC_LC_ONLY | LC_RW"
	...
	filter			locate_noauto
		...
		on EX_DATA or EX_OBJID
			do store_operator( &me->ret, 1 )
				on RETURN_CODE != "VS_K_ACT_SUCCESS"
					do handleError( me->ret ) state .
			state getSecondPoint

state getSecondPoint
	prompt_key		VS_P_Id2ndPntMvOn
	accept_key		VS_P_AccRej
	locate_class		"+GRpoint,EMSplane"
	locate_properties	"LC_DP_ONLY | LC_LC_ONLY | LC_RW"
	locate_owner		"  LC_RIGID_COMP
				 | LC_RIGID_OWNER
				 | LC_FLEX_COMP
	...
	filter			locate_noauto
		...
		on EX_DATA or EX_OBJID
			/*
			 * We cannot join sub-beams if there is a gap between
			 * them.
			 */
			do setJoiningAction( VS_K_DO_NOT_JOIN )
...
at init	do	message_key VS_M_VSSpBmPt
	do	dspAutoJoinMode()

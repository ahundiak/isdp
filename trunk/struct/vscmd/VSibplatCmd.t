/*
	I/STRUCT
	"Impose Boundaries on Plate" command.
*/
command_string	VSC_M_VSImBnOnPl,0,VSImBnOnPl
			...
			do store_operand( &me->ret, 1 )
				on RETURN_CODE != "VS_K_ACT_SUCCESS"
					do handleError( me->ret ) 
								 state .
			do ms_on_off  MS_OFF
			do prompt_key VS_P_IdClCv		 state getCurves

state getCurves
	ms_on_off		MS_ON
	accept_key		VS_P_AccWNxClCv
	construct_class		"VSholPlate"
	/*
	...
	filter			locate_noauto
		...
		on EX_RJT_MOVEON
			do operation( &me->ret )
				...
				on RETURN_CODE != "VS_K_ACT_SUCCESS"
					do handleError( me->ret ) state main
			/*
			 * Flush input, restart.
			 */
...
at init do message_key VS_M_VSImBnOnPl
/*----------------------------------------------------------------------------*/

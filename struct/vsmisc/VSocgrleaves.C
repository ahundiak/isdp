...
/*----------------------------------------------------------------------------*/
static long VS__getLeavesOfOwnerCmpGraph( msg, target, count, leaves )
...
struct GRid	**leaves ; {
	...
	if( !( sts & 1 ) || !nbCmps ) {
		...
		for( i = 0 ; i < nbCmps ; i++ ) {
			 ...
			 */
			sts = VS__getLeavesOfOwnerCmpGraph(
...
} /* VS__getLeavesOfOwnerCmpGraph */
/*----------------------------------------------------------------------------*/
long VSgetLeavesOfOwnerCmpGraph( msg, target, count, leaves )
...
struct GRid	**leaves ; {
	...
	*count = 0 ; *leaves = NULL ;
	 ...
	 * First count leaves.
	 */
	sts = VS__getLeavesOfOwnerCmpGraph( msg, target, count,
					    (struct GRid **) NULL ) ;
	if( !( sts & 1 & *msg ) || !*count ) goto wrapup ;

	if( !( *leaves = _MALLOC( *count, struct GRid ) ) ) vs$mem_fail() ;

	/*
	 * Now get leaves.
	 */
	*count = 0 ;
	sts = VS__getLeavesOfOwnerCmpGraph( msg, target, count, leaves ) ;

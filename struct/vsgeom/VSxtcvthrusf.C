...
#include "vsfarsfplfep.h"
/*----------------------------------------------------------------------------*/
double VSfarthestSfPoleFromPt( point, surface, farthest )
...
struct IGRbsp_surface	*surface ; {
	...
	if( farIndex != -1 ) {
		VSvccpy( farthest, poles[farIndex] ) ;
	}

	return farIndex != -1 ? sqrt( maxdSq ) : -1. ;

} /* VSfarthestSfPoleFromPt */
/*----------------------------------------------------------------------------*/
static int VSfindApproxPoint( msg, curve, surface, whichEnd, approxPnt )
...
IGRpoint		approxPnt ; {
	...
	if(nbInt == 0 ) { *msg = EMS_E_NoIntersection ; goto wrapup ; }
...
/*----------------------------------------------------------------------------*/
static int VScvxttsf( msg, curve, isLinear, endPoint, endIndex, surface,
		      p_extCurve )
...
struct IGRbsp_surface	*surface ; {
	...
	VSfindApproxPoint( msg, curve, surface, endIndex, approxPt ) ;
	if( !( *msg & 1 ) ) goto wrapup ;

	if( VSeqPoints( endPoint, approxPt ) ) {
		VScopybc( msg, curve, &extCurve ) ;
		goto wrapup ;
	}
...
/*----------------------------------------------------------------------------*/
int VSextend_curve_to_surface( msg, curve, surface, whichEnd, p_extCurve )
...
struct IGRbsp_surface	*surface ; {
	...
	isLinear = vs$is_curve_linear( msg = msg, curveGeom = curve ) ;

	VScvxttsf( msg, curve, isLinear, endPoint, whichEnd, 
			surface, &extCurve ) ;
	...
	if( !( *msg & 1 ) ) {
		if( !isLinear ) {
			 ...
			 * of non-linear curve.
			 */
			 VScvxttsf( msg, curve, TRUE, endPoint, whichEnd,
			 		surface, &extCurve ) ;
		}
	}
	if( !( *msg & 1 ) ) goto wrapup ;
...
} /* VSextend_curve_to_surface */
/*----------------------------------------------------------------------------*/
int VSextend_bc_through_surface( msg, curve, surface, whichEnd,
					p_throughCurve )
...
Abstract :
	...
	the surface.
	This function interfaced by macro vs$extend_curve_through_surface.
*/
...
struct IGRbsp_surface	*surface ; {
	...
	VSextend_curve_to_surface( msg, curve, surface, whichEnd, &extCurve ) ;
	if( !( *msg & 1 ) ) goto wrapup ;
	...
	extDist = VSfarthestSfPoleFromPt( endPoint, surface, farthest ) ;
	if( extDist == -1 ) {
...
} /* VSextend_bc_through_surface */
/*----------------------------------------------------------------------------*/
int VSextend_ln_through_surface( msg, lineseg, surface, whichEnd,
					p_throughCurve )
...
} /* VSextend_ln_through_surface */
/*----------------------------------------------------------------------------*/
int VSextend_curve_through_surface( msg, curve, surface, whichEnd,
					p_throughCurve )
...
struct IGRbsp_surface	*surface ; {
	...
	if( vs$is_curve_linear( msg = msg, curveGeom = curve ) ) {
		VSextend_ln_through_surface(	msg,
						curve,
						surface,
						whichEnd,
						p_throughCurve ) ;
	} else {
		VSextend_bc_through_surface(	msg,
						curve,
						surface,
						whichEnd,
						p_throughCurve ) ;
...
} /* VSextend_curve_through_surface */
/*----------------------------------------------------------------------------*/

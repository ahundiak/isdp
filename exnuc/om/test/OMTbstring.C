...
#define NUMKEYS 800
#define  MAX_KEYLENGTH    (134)

main()
{
 static char new_key[NUMKEYS][MAX_KEYLENGTH+1], targetKey[MAX_KEYLENGTH];
 int  rett, rettt, ii, j, kl, num_keys;
 ...
 static char low_key[] = {'D', 'L', 'O', 'W', 'K', 'E', 'Y', '\0'};
 static char high_key[] = {'H', 'I', 'G', 'H', 'K', 'E', 'Y', '\0'};
...
struct seq_args_str
 ...
 constr_tree_args.key_type = BTREE_STRING;
 ...
 for (i = 0; i < NUMKEYS; i++) {
   kl =  MAX_KEYLENGTH * drand48();
   if (kl == 0) kl = 1;
   for (j = 0; j < kl; j++)
      new_key[i][j] = 'A' + (int)(25 * drand48());
   new_key[i][kl] = '\0';
   ...
   constr_key_args.key = (char *)&new_key[i][0];
   ...
   btree_key.key.p_string = (char *)(&new_key[i][0]);
 ...
 *(conts_args.num_keys) = 90;
 ...
 do {
   ...
   for (i=0; i<(*(conts_args.num_keys)); i++) {
     ...
     printf("key: ");
     printf("%s", conts_args.p_keyobj_array[i].name.key_name );
     printf("\n");
   }
 ...
 neighbor_args.user_compare = 0;
 neighbor_args.p_neighbor_objid = (OM_p_OBJID)&neighbor_objid;
 for (i = 0; i < NUMKEYS; i+=40) {
   neighbor_objid = NULL_OBJID;
   kl = strlen(&new_key[i][0]);
   if ( kl > 1 ) {
     strncpy(targetKey, &new_key[i][0], kl-1);
     targetKey[kl] = '\0';
     btree_key.key.p_string = (char *)targetKey;
 ...
 if (!(1&(sts=om$get_classid(classname="A", p_classid=myClasslist.p_classes))))
{
 ...
 for (i=0, j=0; i < NUMKEYS; i+=40, j++) {
   ...
   printf("\nsearch key:  ");
   printf("%s\n", new_key[ii]);
   btree_key.key.p_string = (char *)&new_key[ii][0];
   if (!(j/7)){
     pass_args.msg = &A_ans;
   }
   else
 ...
 range_pass_args.user_compare = 0;
 ...
 lbtree_key.type = KEY_STRING;
 lbtree_key.key.p_string = (char *)(low_key);
 range_pass_args.p_key_low = &lbtree_key;
 hbtree_key.type = KEY_STRING;
 hbtree_key.key.p_string = (char *)(high_key);
 ...
 for (i = 0; i < NUMKEYS; i++) {
   ...
   if ( *(remove_args.ret) != DB_SUCCESS ) {
     printf("*** ERROR from BDELETE: %d***\n", *(remove_args.ret));
     exit();
   }
 }

 tree_type = BTREE_STRING;
 sts = om$make_message( classname="OMbtree",

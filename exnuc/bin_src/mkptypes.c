...
#define MAXPARAM 100 		/* max. number of parameters to a function */
...
char *macro_name = "__";	/*   macro to use for prototypes */
	...
	for (w = wlist; w; w = w->next) {
		if (w->string[0]) {
			...
			if (!strcmp(w->string, "static"))
				isstatic = 1;
			else if (!strcmp(w->string, "method"))
				return;
		}
	}
...
int argc; char **argv;
{
	...
	if (use_macro && define_macro) {
		...
		printf("#ifndef %s\n", macro_name);
		printf("#define %s(args) args\n", macro_name);
		printf("#endif\n");
		printf("#else\n");
		printf("#ifndef %s\n", macro_name);
		printf("#define %s(args) ()\n", macro_name);
		printf("#endif\n");
		printf("#endif\n\n");
	}
	Emitted = 0;
...
void Usage()
{
	...
	fputs("   -p nm: use \"nm\" as the prototype macro (default \"__\")\n",
	   stderr);

...
/*
  ABSTRACT
      ...
      face boundaries and returns the trimmed intersections falling in AREA/
      on BOUNDARIES of the face.
  ...
  DESCRIPTION
      ...
      loop. The UV curve is typically, an intersection curve in the UV-space 
      of the surface as interacts with another. The loop is made of topolog-
      ical element Edge. Loop could be either open or closed.
  ...
  ARGUMENTS
      ...
      p_faceid    I  Face ID against which the trimming need to be done.
...
)
{
  ...
  /*
   * Intersect the UV curve with the given face and get all the interaction
   * points in the linked list.
   */
  PWsts = pwIntersectWithFace (p_faceid, p_uvcv, p_xyzcv, uvcvrng, &sfdata,
                               xyztol, &p_cvintpts);
...
wrapup :
  PW_Wrapup (PWsts, "pwTrimAgainstFace");
  return PWsts;
}

...
/*---FILL_UP-------------------------------------------------------------------*/
#ifdef PSUEDO_COLOR
/*Get the psuedo true color dithered color for color buffer*/
#define FILL_UP( y )								\
										\
	{									\
			...
			if( *zb >= rzc.active )					\
				...
				*zb = element_parms.delete ? 			\
					HS_BACKGROUND_Z : rzc.active;		\
										\
*cb = HRindex_rgb(x,y,element_parms.red,element_parms.green,element_parms.blue);\
		 		}						\
										\
			Fixed_add( rzc );					\
										\

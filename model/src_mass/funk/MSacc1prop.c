...
#include "EMS.h"
/*
void MSacc_area_prop(prop, acc_prop)


Given area properties in global system of two objects "prop" and
"acc_prop", this returns the properties at the new centroid in "acc_prop".

IGRdouble	prop[];	area properties
IGRdouble	acc_prop[];	accumulated properties

Note:
1.	area properties  prop are passed in as follows: 
	...
	8	product moment xy
	9	product moment yz  -- 0
	10	product moment zx  -- 0
	11	surface area
	

History:
    gupta   12/06/89       creation date
*/

# include "igrtypedef.h"
# include "emsmass.h"

void MSacc_area_prop(prop, cum)
...

{
  ...
  if (first_pass)
   {
    for (i=0; i<NUMB_PROPS; i++) cum[i] = prop[i];
    return;
   }

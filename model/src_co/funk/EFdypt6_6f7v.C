...
extern EFdyrbpt6_6f7v();

struct pt6_struct
{
 struct DPele_header ele_header[2];
 IGRvector           h_vec;
};


IGRint EFdypt6_6f7v( pt2, pt3, pt5, display )
 IGRpoint            pt2, pt3, pt5;
 struct IGRdisplay   *display;

{
 struct       pt6_struct    header;
 struct       IGRpolyline   polyline[2];
 IGRpoint                   pt_buffer[7];
 ...
 /**MOVE THE INPUT POINTS TO THE POLYLINE POINT BUFFER**/
  OM_BLOCK_MOVE(pt2,&pt_buffer[3][0],size_pnt);    
  OM_BLOCK_MOVE(pt3,&pt_buffer[6][0],size_pnt);
  OM_BLOCK_MOVE(pt5,&pt_buffer[1][0],size_pnt);
 ...
 /*  INVOKE DYNAMICS  */
 dp$dynamics( dyn_fun = EFdyrbpt6_6f7v, information = &header );
...
/*
NAME    EFdyrbpt6_6f7v -->  function called by dp$dynamics to put new
                            stuff in
DESCRIPTION
...
IGRint EFdyrbpt6_6f7v( header, cursor_pt, matrix, objects, num_objects,
 ...
 IGRchar               **out_dummy1, **out_dummy2, **out_dummy3;
{
 ...
 ptr->points[0] = cursor_pt->x;
 ptr->points[1] = cursor_pt->y;
 ptr->points[2] = cursor_pt->z;
 ...
 /**SECOND POLYLINE**/
 OM_BLOCK_MOVE(&header->ele_header[0].geometry.polyline->points[0], 

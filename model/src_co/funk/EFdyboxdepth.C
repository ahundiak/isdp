...
/*
NAME
        EFdyboxdepth - Dynamically place depth area of box by 4 points

SYNOPSIS
        EFdyboxdepth(corner_pt, width_pt, height_pt, display)
        IGRpoint corner_pt,           has the corner point of the box
                 width_pt,            has the width point of the box
                 height_pt;           has the height point of the box
        sturct IGRdisplay *display;  has active display stuff     

DESCRIPTION
        This function takes as input the 3 points defining a face
        of a box and sets up the DPele_header buffers to dynamically
        draw the depth face and connecting edge points. Each polyline
        will from all a face except the face already defined and 1 
        other edge. That edge will be filled by the next buffer 
        (i.e. the polylines will wrap around the sides of the box).
...
extern EFdyrbboxdepth();

struct depth_struct
{
 struct DPele_header ele_header[4];
 IGRvector           nor_vec;
};
       
IGRint EFdyboxdepth( corner_pt, width_pt, height_pt, display )
...

{
 ...
 for (i=0; i<3; i++)
 {
  ...
  pt_buffer[6][i] = width_pt[i] + h_vec[i];
 }
 ...
 OM_BLOCK_MOVE(height_pt, &pt_buffer[9][0], size_pnt); 
 ...
 /*  INVOKE DYNAMICS  */
 dp$dynamics( dyn_fun = EFdyrbboxdepth, information = &header );
...
SYNOPSIS
        EFdyrbboxdepth(header, cursor_pt, matrix, objects, num_objects, 
...
DESCRIPTION
        ...
        Once the vector has been determined, it is added to the 4 corner
        points the have already been defined to get all 8 points of the
        box. These points are then moved to the proper postions in the
...
IGRint EFdyrbboxdepth( header, cursor_pt, matrix, objects, num_objects,
                     buffer, num_buffers, in_dummy1, in_dummy2, in_dummy3,
                     out_dummy1, out_dummy2, out_dummy3 )
 struct  depth_struct  *header;
 ...
 IGRchar               **out_dummy1, **out_dummy2, **out_dummy3;
{
 ...
 header->ele_header[2].geometry.polyline->points[6] = 
         header->ele_header[3].geometry.polyline->points[0] + d_vec[0];
 header->ele_header[2].geometry.polyline->points[7] = 
         header->ele_header[3].geometry.polyline->points[1] + d_vec[1];
 header->ele_header[2].geometry.polyline->points[8] = 
         header->ele_header[3].geometry.polyline->points[2] + d_vec[2];
 ...
 OM_BLOCK_MOVE(&header->ele_header[2].geometry.polyline->points[6],
               &header->ele_header[3].geometry.polyline->points[3],
               size_pnt);

 OM_BLOCK_MOVE(&header->ele_header[0].geometry.polyline->points[3],
               &header->ele_header[3].geometry.polyline->points[6],
               size_pnt);

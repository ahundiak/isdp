...
DESCRIPTION
	...
	when an element is completed (i.e., removed from the active list). If
	VE is running interactively, the message is written to the keyin field
	on the screen for each element that is completed.  If VE is running in
	batch mode, the message	is written to the log file for the last element
	to complete, so a completion time can be seen in the log file.
...
*/
	...
	sprintf( temp_string, "%d %s, %d %s",
	         VEexec_status.VEactivated_elems, VEactivated_string,
	         VEexec_status.VEfinished_elems, VEcompleted_string );
	...
	if ( VEexec_status.VEfinished_elems == VEexec_status.VEtotal_elems )
	   ...
	   if ( VEexec_parms.VEbatch )
	      {
	      /* write the string and time stamp to the log file => if the format    */
	      /* of this line is changed the function used by "Retrieve Edge Results */
	      /* From Server" will have to be modified too.                          */
...
DESCRIPTION
	This function updates the "xxx Activated, yyy Completed" status message
	in the message strip whenever a new element is activated.  If GVE/GPE
	is running in batch mode, it writes the message to the log file along
	with a time stamp each time the activated count reaches a multiple of
	5% of the elements activated.
...
*/
	...
	if ( VEexec_parms.VEbatch )
	   {
	   percent_activated = ( ( (IGRdouble) VEexec_status.VEactivated_elems ) /
...
DESCRIPTION
	This function displays the "Processing xxx total elements" message
	at the beginning of GVE/GPE processing.

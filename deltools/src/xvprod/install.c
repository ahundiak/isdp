 ...
 */
int	mktmpdir(ldirname, fullpathname)
char	*ldirname, *fullpathname;
{
    char	command[STRSIZ];
    struct stat	statbuf;	
	...
	sprintf(fullpathname, "%s/ingr/%s", basedir, ldirname);
#else
	sprintf(fullpathname, "%s/ip32/%s", basedir, ldirname);
#endif
    }
    else
	sprintf(fullpathname, "%s/tmp/%s", mountpoint, ldirname);
...
#else
{
    ...
    strcat(cmdname, " ");
    strcat(cmdname, srcdir);
    strcat(cmdname, " ");
    strcat(cmdname, ws_destdir);    /* $4 */
    if (takedefaults)
	strcat(cmdname, " -y");
    ...
    putenv(strcat(envstr1, ws_destdir));
    putenv(minus_x ? "NULL=/dev/tty" : "NULL=/dev/null");
    rstat = run_shell(cmdname, FALSE, TRUE);
    return(rstat);
}
#endif
 ...
 * shell script files.
 */
void    my_rmdir(ldirname)
register char	*ldirname;
{
    char	command[STRSIZ];
    
    chdir("/");	/* "cd" to / directory, get out of tmp where ever */
    sprintf(command, "rm -rf %s", ldirname);
    system(command);
}
...
char	*unixdir;
{
	...
	break;
    case LOCAL_DIR:
	sprintf(mediaspec, "Local Directory: %s", ldirname);
	break;
    case CDROM:
...
register struct prodnode *pptr;
{
    ...
    reboot = 0;
    FIfld_set_text(delform, CURR_PROD, 0, 0, pptr->prodname, FALSE);
...
#ifdef VPROD
    if (pptr->unixdir[0] == '/') { /* handle absolute path */
	strcpy(unixdir, pptr->unixdir);
	strcpy(basedir, GetMountPoint(unixdir));
	strcpy(mountpoint, basedir);
    } else {
	sprintf(unixdir, "%s/%s", pptr->destfs, pptr->unixdir);
	strcpy(basedir, pptr->destfs);
	strcpy(mountpoint, pptr->destfs);
    }
#else
...
PRINT "\nERROR:  This product is NOT certified for use on your\n");
PRINT "system hardware.  Installation of the product will not\n");
PRINT "continue.  Please contact Intergraph support for information\n");
PRINT "on a fully certified version of the product.\n");

LPRINT "\nERROR:  This product is NOT certified for use on your\n");
LPRINT "system hardware.  Installation of the product will not\n");
LPRINT "continue.  Please contact Intergraph support for information\n");
LPRINT "on a fully certified version of the product.\n");
...
PRINT "\nWARNING: This product is PRE-CERTIFIED for use on\n");
PRINT "your system hardware.  Installation of the product will\n");
PRINT "continue, but you should contact Intergraph support for\n");
PRINT "information on a fully certified version of the product.\n");

LPRINT "\nWARNING: This product is PRE-CERTIFIED for use on\n");
LPRINT "your system hardware.  Installation of the product will\n");
LPRINT "continue, but you should contact Intergraph support for\n");
LPRINT "information on a fully certified version of the product.\n");
...
finishup:
    ...
    FIf_get_location(delform, &xloc, &yloc);
    FIf_get_size(delform, &xsize, &ysize);
    FIf_paint_area(delform, xloc, yloc, xsize, ysize);	
	...
	if (!access(post, R_OK)) {
		 ...
		 */
		PRINT "ERROR: I_LICENSE version 06.01.*.* must be installed\n");
		PRINT "       on your system before this product can be used.\n");
		PRINT "       Please install I_LICENSE, and then re-install this\n");
#ifndef VPROD
		PRINT "       product");
		if (verbose) PRINT ".\n");
#else
		PRINT "       product.");
#endif
		LPRINT "ERROR: I_LICENSE version 06.01.*.* must be installed\n");
		LPRINT "       on your system before this product can be used.\n");
		LPRINT "       Please install I_LICENSE, and then re-install this\n");
		LPRINT "       product.\n");
		goto install_errors;
	    }
	}
...
register struct prodnode	*pptr;
{
...
#ifdef VPROD
    PRINT "Copying the release notes file\n");
    PRINT "for %s (%s)\n", pptr->prodname, pptr->prodnumber);
    /*
     * set up the environment for getfile, getanswer
     */
    strcpy(envstr1, "PRODNAME=");
    putenv(strcat(envstr1, pptr->prodname));
    strcpy(envstr2, "PRODNUMBER=");
    putenv(strcat(envstr2, pptr->prodnumber));
    strcpy(envstr3, "SRCDIR=");
    putenv(strcat(envstr3, pptr->srcdir));
    strcpy(envstr4, "PRODVERSION=");
    putenv(strcat(envstr4, pptr->prodversion));
    ...
    if (file) {
	mess_row = 0;
	FIg_reset(revform, README);
	if ((sfp = fopen((file == 1 ? FIXES_FILE : README_FILE), "r")) == NULL) {
	    PRINT "Could not read release notes file for this product.  Sorry.\n");
	}
	else {
	    get_sections(revform, sfp);
	    /*
	      found = FALSE;
	      while (fgets(instr, 80, sfp))  {
	      strcpy(tmp, instr);
	      cap_case(tmp);
	      
	      if (!(strncmp(tmp, "END OF PRODUCT SECTIONS", strlen("END OF PRODUCT SECTIONS"))))
	      found = FALSE;
	      
	      if ((found) && (strncmp(tmp, "---", 3)) && (tmp[0] != '\n'))
	      {
	      instr[strlen(instr) - 1] = '\0';
	      FIfld_set_text(revform, SECTIONS, sec_row++, 0, instr, FALSE);
	      }
	      ...
	      fix_line(instr);
	      FIfld_set_text(revform, README, mess_row++, 0, instr, FALSE);
	      }
	      */
	    fclose(sfp);
	}
...
register struct prodnode *pptr;
{
	...
	memset(prodarch, 0, 32);
		 ...
		 * Make it from:
		 *	Product number (leave out processor code)
		 *	Architecture name
		 *	  (cause processor code 05 can be SUN *or*
		 *		 SPARC_SOLARIS)
		 *	Major/Minor version
		 */
		prodname[0] = pptr->prodnumber[0];
		prodname[1] = pptr->prodnumber[1];
		prodname[2] = pptr->prodnumber[4];
		prodname[3] = pptr->prodnumber[5];
		prodname[4] = pptr->prodnumber[6];
		prodname[5] = pptr->prodnumber[7];
		prodname[6] = pptr->prodnumber[8];
		prodname[7] = 0;
...
/*	    if ((rstat = get_prod_files(prodarch, (platforms > 1)))) */
	    if ((rstat = get_prod_files(prodarch, 1)) != 0)
		goto cleanup;
...
cleanup:
	    ...
	    if (rstat) {
		PRINT "Unsuccessful load: %s (%s)\n", pptr->prodname, pptr->prodnumber);
		LPRINT "Unsuccessful load: %s (%s)\n", pptr->prodname, pptr->prodnumber);
	    } else {
		free_space_check(tmpmntpoint, pptr, 1);
		PRINT "Successful load: %s (%s)\n", pptr->prodname, pptr->prodnumber);
	...
	make_fslist();
		...
		case 255:
			...
			while (fgets(str, 128, bomf) != NULL) {
				...
				if (!optionlist[j]->destfs[0]) {
				    if (optionlist[j]->loadeddir[0]) 
					strcpy(optionlist[j]->destfs, GetMountPoint(optionlist[j]->loadeddir));
				    else
					strcpy(optionlist[j]->destfs, fs_table[curr_fs].name);
				}
			    }
#endif
			}
		...
		if (!count)
		    PRINT "None of the selected products can be preprocessed.");
		return(rstat);
	    }

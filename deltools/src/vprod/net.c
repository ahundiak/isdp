...
struct prodnode	*pptr;
{
	...
	if (!send_request()) {
		xfersize = 1024;
		if ((errstr = rx((char *)reply))) {
			VPRINT("Load key receive error: ", errstr);
			return(NULL);
		}
	}
...
#define DEBUG		0x04	/* avoid spending time doing checksums */
	...
	if (strcmp(version,"07.06.00.00") >= 0 && strcmp(server_version,"07.06.00.00") < 0) {
		...
		VPRINT1("Server supports Serial Numbers\n");	
		sssn=1;
	}
	return(0);
}
...
char	*connstr;
{
    VPRINT1("Attempting to connect via XNS...");
    if (srcmedia == NETWORK) {
        /*
...
char	*connstr;
{
	...
	in = net_connstr_parse(connstr, &nodename, &username, &password);
		...
		VPRINT1("Could not obtain 'newprod' TCP/IP service");
		if (xns_installed) {
		    return(xns_connect(connstr));
		}
		else
		    return("Could not obtain 'newprod' TCP/IP service");
	    }
		...
		if (!(hp = gethostbyname(nodename))) {
		    ...
		    VPRINT("Could not resolve host name '%s' to a TCP/IP address.\n",nodename);
		    if (xns_installed) {
			return(xns_connect(connstr));
		    }
		    else
		...
		VPRINT1("Could not allocate a TCP/IP socket.");
		if (xns_installed) {
		    return(xns_connect(connstr));
		}
		else
		    return("Could not allocate a TCP/IP socket");
	    }

	    if (connect(sk, (struct sockaddr *)&server, sizeof(server)) < 0)
	    {
		DPRINT1("TCP connect failed");
		VPRINT1("Could not connect to TCP/IP server.");
		if (xns_installed) {
		/*
		 * 92/01/22 SCW
		 ...
		 */
		    ...
		    return("Could not connect to TCP/IP server");
	    }
...
char    *connstr;
{
	...
	connected = 0;
        ...
        if ((sk = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
                DPRINT1("socket allocation failed");
                return("Could not allocate socket for connection");
        }

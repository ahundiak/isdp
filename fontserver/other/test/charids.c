/* This program displays the all the characters in a typeface (limited	*/
/* by screen space) along with their character ids.			*/
...
FontSpec fontSpec = {	"swiss742",	/* typeface	*/
			...
			0.0,		/* slant	*/
			"all",		/* all chars	*/
			FS_ALL_BITMAPS | FS_DEMAND | FS_NO_KERN
		    };

FontSpec fontSpec2 = {	"swiss742",	/* typeface	*/
			16.0,		/* bodysize	*/
			1.0,		/* arf		*/
			1.0,		/* aspect ratio	*/
			0.0,		/* rotation	*/
...
char *argv[];
{
    ...
    if ((status = FSNewFont (&fontSpec2, NULL, &font2)) != FS_NO_ERROR)
    {
	    ...
	    (outl && (FSGetOutlCharInfo (font, i, &outlCInfo) == FS_NO_ERROR)))
	{
	    ...
	    sprintf (str, "%d:", i);
	    FSGetStringVect (font2, str, x, y, &nx, &ny);
	    FSGetCharVect (font, i, nx, ny, &nx, &ny);
	    FSGetCharVect (font2, ' ', nx,ny, &nx, &ny);
	    if (nx > (double) wWidth)
	    {
		x = 10.0;
		y += info.bodySize;
	    ...
	    hidecursor (window);
	    FSDrawString (window, font2, str, x, y, &x, &y);
	    FSDrawChar (window, font, i, x, y, &x, &y);
	    FSDrawChar (window, font2, ' ', x, y, &x, &y);
	    showcursor (window);
	}
    }

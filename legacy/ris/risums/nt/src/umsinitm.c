...
THREAD_DECL UMSHeader	*UMSFileList;

/****************************************************************/
/* sml:12-13-93:TR249305525: Need an expand_backslash mode      */
/*  The default mode is set to not expand, i.e.,                */
/*  consider backslashes as C-control characters.               */
/****************************************************************/
THREAD_DECL int             UMSexpand_backslash_mode = -1;
...
int UMSInitMsg ( msgFile, optFlag )
			...
			/* UMS_MESSAGE - message text		*/
{
  ...
  THREAD_DECL static  int	done_init = 0;
  char *mode_string;
...
/****************************************************************/
/* sml:12-13-93:TR249305525: Need an expand_backslash mode      */
/*  Check if shell variable BACKSLASH_MODE has been             */
/*  set.  UMSSetExpandBackslashMode must be called before       */
/*  UMSInitMsg.                                                 */
/****************************************************************/
  mode_string = getenv("BACKSLASH_MODE");
  if (mode_string == NULL || (strcmp(mode_string, "") == 0))
  {
    ; /* do nothing */
  }
  else if (mode_string && mode_string[0] == '0')
  {
    UMSexpand_backslash_mode = 0;
  }
 
  else /* has been set to something besides 0 */
  {
    UMSexpand_backslash_mode = 1;
  } 

  if (UMSexpand_backslash_mode == -1)
  /* hasn't been set either by user or program */
  {
    UMSexpand_backslash_mode = 0;
  }
...
int  _UMSInitMsgHdr( newHdr, msgFile )
...
UMSHeader *_UMSHdrByFile( msgFile )
...
/*  Need an expanded backslash mode.                             */
/*  Add this function to set the mode programmatically.          */
/*  This function must be called before UMSInitMsg().            */
/*****************************************************************/

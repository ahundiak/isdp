...
Make_ScoSolSun_Libs ()
{
   for dir in   version alpha alphas cmdtools dbtools initial \
		libmem libnet libsql liberr fserver filetransfer \
		nfma nfmd nfmlist nfmr nfms nfmu nfmtools acad sc \
		scpp systools workflow wff nfmfi_x \
                nfmfis_x bulkload admin v_nfms libqui nfmcli \
		convert update_utility NFMmonitor
   do
	cd ../$dir
	echo "make $1 $dir ..."
	make -f makefile.$1

...
long WFFdump_workflow (workflow)
     char   *workflow;
  {
    long    status;
    char    title[256];
    ...
    static  char *fname = "WFFdump_workflow";
    ...
    if (strcmp (workflow, "") == 0)
     {
        status = NFI_E_NO_WF;
        _NFIdebug ((fname, "No workflow given = <%d>\n", status));
    ...
    if ((status = (long) FIf_new (DUMP_WF_LABEL,
    ...
    if ((status = (long) UMSGetByNum  /* NEW Message */
        (title, NFI_P_DUMP_WF, "%s", workflow)) != UMS_S_SUCCESS)
      {
        _NFIdebug ((fname, "UMSGetByNum = <0x%.8x>\n", status));
        _NFIdebug ((fname, "defaulting title\n"));
        sprintf (title, "Dump \"%s\"", workflow);
      }
...
long _WFFdump_wf_notify (g_label, value, form)
  int       g_label;
  double    value;
  Form      form;
  {
    ...
    static  char *command = WFF_M_DUMP_WORKFLOW;
    static  char *fname = "_WFFdump_wf_notify";
    ...
    defined(OS_SCO_UNIX) || defined(OS_HPUX))
            ...
            if (status = _WFFhelp_topic (command))
              {
                ...
                ERRload_struct (NFI, NFI_E_HELP, "%s%d", "_WFFhelp_topic", 
				status);
                status = NFI_E_HELP;
              }
#endif
        ...
        case FI_ACCEPT:
            ...
            _WFFmessage (NFI_I_DUMP_WF, NULL);

            /*  Call NFM to dump/unload the workflow  */

            _NFIdebug ((fname, "calling WFGdump_workflow ...\n"));
            status = WFGdump_workflow (WFFglobal.workflow, filename);
            _NFIdebug ((fname, "WFGdump_workflow = <0x%.8x>\n", status));

...
/* 
 Doc: NFMRchange_table
 
 Abstract:
     This function will change a table with a given set of attributes
     and data (modified buffers returned from 
     NFMRquery_change_table_attributes).
 */
 
long NFMRchange_table (user_id, table_type, table_name,
...
static char *fname = "NFMRchange_table";
    ...
    status = NFMcl_send_buffer (&NFMRglobal.NETid, NFM_CHANGE_TABLE, info);

...
/* 
 Doc: NFMRprepare_item_copy
 
 Abstract:
     This function will prepare an item to be copied.  The output buffers are
     used for NFMRcomplete_item_copy.

 */
 
long NFMRprepare_item_copy (user_id, catalog_name, item_name, item_rev,
                            attr_list, data_list)

long    user_id;       /* i - user id for access */
char    *catalog_name; /* i - catalog of item */
char    *item_name;    /* i - item to copy */
...
static char *fname = "NFMRprepare_item_copy";
    ...
    status = NFMcl_send_buffer (&NFMRglobal.NETid, NFM_PREPARE_ITEM_COPY, info);
...
/* 
 Doc: NFMRcancel_item_copy
 
 Abstract:
     This function will cancel the preparation of an item to be copied.
     It must be called if a copy has been initiated
     (NFMRprepare_item_copy has been called) but needs to be cancelled.
     The input buffers are from NFMRprepare_item_copy.

 */
 
long NFMRcancel_item_copy (user_id, attr_list, data_list)
...
long   status = 0;
long   ret_status = 0;
static char *fname = "NFMRcancel_item_copy";
    ...
    status = NFMcl_send_buffer (&NFMRglobal.NETid, NFM_CANCEL_ITEM_COPY, info);
    _NFMRdebug(( fname, "Send Buffer : <0x%.8x>\n", status));
    if (status != NFM_S_SUCCESS)
        {
        MEMclose (&info);
...
/* 
 Doc: NFMRcomplete_item_copy
 
 Abstract:
     This function will complete the copy of an item.  The input buffers are
     from NFMRprepare_item_copy.
     
 */
 
long NFMRcomplete_item_copy (user_id, storage_name, tmp_node, tmp_user,
...
long   ret_status = 0;
char   id[20];
static char *fname = "NFMRcomplete_item_copy";
    ...
    status = NFMcl_send_buffer (&NFMRglobal.NETid, NFM_COMPLETE_ITEM_COPY, info);
    _NFMRdebug(( fname, "Send Buffer : <0x%.8x>\n", status));
    if (status != NFM_S_SUCCESS)
        {
        MEMclose (&info);

...
/* 
 Doc: NFMRquery_change_user_attributes
 
 Abstract:
     This function will return the attributes needed to change a user

 */
 

long NFMRquery_change_user_attributes (user_id, user_name, attr_list,
                                       data_list, value_list)

long    user_id;       /* i - user id for access */
char    *user_name;    /* i - user to change */
...

{
    ...
    static char *fname = "NFMRquery_change_user_attributes";
    ...
    status = NFMcl_send_buffer (&NFMRglobal.NETid, NFM_QUERY_CHANGE_USER_ATTR, info);
    ...
    if (_NFMdebug_st.NFMRdebug_on)
        {
        MEMprint_buffers ("USER ATTR", *attr_list, 
			   _NFMdebug_st.NFMRdebug_file);
        MEMprint_buffers ("USER DATA", *data_list,
			   _NFMdebug_st.NFMRdebug_file);
        MEMprint_buffers ("USER VALUE", *value_list,
			   _NFMdebug_st.NFMRdebug_file);
        }
...
/* 
 ...
 Abstract:
     This function will change a user entry given the buffers returned
     from NFMRquery_change_user_attributes.
     
 */

long NFMRchange_user (user_id, attr_list, data_list)
...
static char *fname = "NFMRchange_user";
    ...
    status = NFMcl_send_buffer (&NFMRglobal.NETid, NFM_CHANGE_USER, info);
    _NFMRdebug(( fname, "Send Buffer : <0x%.8x>\n", status));
    if (status != NFM_S_SUCCESS)
        {
        MEMclose (&info);

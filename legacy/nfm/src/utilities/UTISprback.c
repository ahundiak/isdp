...
UTIprocess_backup( sa_no, label, flist )
    int		sa_no;
    char	*label;
    UTIpending  flist;
{
    ...
    static	char *fname = "UTIprocess_backup";
...
*********************/
    ...
    status = UTIbuild_script( UTIbackup, nfm_arch_loc->archive_no,
			      uti_arch_loc, flist );
    if( status != UTI_S_SUCCESS )
    {
        UTIrm_saveset_entry( nfm_arch_loc->archive_no );
 ...
 */
    ...
    if( OFF_LINE )
    {
	sprintf( script_name, "BACKUP.%d", nfm_arch_loc->archive_no );
	...
	{
  	    sprintf( script_name, "BACKUP_C.%d", nfm_arch_loc->archive_no );
	    chmod( script_name, 06777 );
            status = UTIxfer_non_nfm_file_in( UTI_CLIENT_SA, script_name );
            if( status != UTI_S_SUCCESS )
            {
	...
	status = UTIupdate_f_catalog( UTIbackup_p, flist );
	if( status != UTI_S_SUCCESS )
	{
            UTIrm_saveset_entry( nfm_arch_loc->archive_no );
	    return( status );
	...
	status = UTIupdate_catalog( UTIbackup_p, flist );
	if( status != UTI_S_SUCCESS )
	{
            UTIrm_saveset_entry( nfm_arch_loc->archive_no );
	    return( status );
        ...
        status = UTIupdate_nfmarchives( UTIbackup_p, nfm_arch_loc );
        if( status != UTI_S_SUCCESS )
	{
            UTIrm_saveset_entry( nfm_arch_loc->archive_no );
    	    return( status );
        ...
        status = UTIupdate_nfmsavesets( UTIbackup_p, nfm_arch_loc->archive_no,
				       flist );
        if( status != UTI_S_SUCCESS )
 	    return( status );

        status = UTIupdate_nfmprojectcit( UTIbackup, nfm_arch_loc->archive_no,
                                          flist );
        if( status != UTI_S_SUCCESS )
            return( status );
    }    
	...
	/*  Transfer cpio file to chosen backup location
	 */
	...
	/*  Update catalog
	 */
	status = UTIupdate_catalog( UTIbackup, flist);
	if( status != UTI_S_SUCCESS )
	{
            UTIrm_saveset_entry( nfm_arch_loc->archive_no );
	    return( status );
	...
	/*  Update file catalog
	 */
	status = UTIupdate_f_catalog( UTIbackup, flist );
	if( status != UTI_S_SUCCESS )
	{
            UTIrm_saveset_entry( nfm_arch_loc->archive_no );
	    return( status );
	 ...
	 */
        status = UTIupdate_nfmarchives( UTIbackup, nfm_arch_loc );
        if( status != UTI_S_SUCCESS )
  	    return( status );

	/*  Update nfmsavesets table
 	 */
        status = UTIupdate_nfmsavesets( UTIbackup, nfm_arch_loc->archive_no,
				       flist );
        if( status != UTI_S_SUCCESS )
  	    return( status );

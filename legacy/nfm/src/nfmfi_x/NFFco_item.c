...
long NFFcheckout_item (display_flag, catalog, item_name, revision)
  int     display_flag;
  char    *catalog;
  char    *item_name;
  char    *revision;
  {
    ...
    static  char    *fname = "NFFcheckout_item";
  ...
  /* prompt needs to change for confirm */
    if ((status = (long) UMSGetByNum (prompt1, NFT_P_CONFIRM_CHECKOUT,
                                      NULL)) != UMS_S_SUCCESS)
      {
        ...
        strcpy (prompt1, "Are you sure you want to check out");
      }
    ...
    NFFglobal.working_no = NFI_I_CHECKOUT_ITEM;

    strcpy (command, CHECKOUT_ITEM);

    /*  Set the NFMR function  */

    NFFglobal.NFMRfunction[0] = NFMRcheckout_item;

    if (strcmp (NFFglobal.set_indicator, "") == 0)
       status = _NFFfile_io (CHECKOUT_CONFIRM_LABEL, display_flag, command,
                             catalog, item_name, revision, prompt1, prompt2);
    else
       status = _NFFfile_io (CO_SET_ITEM_LABEL, display_flag, command,
                             catalog, item_name, revision, prompt1, prompt2);

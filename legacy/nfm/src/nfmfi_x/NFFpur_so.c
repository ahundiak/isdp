...
long NFFpurge_signoff (catalog, item_name, revision)
  char    *catalog;
  char    *item_name;
  char    *revision;
  {
    ...
    static  char    *fname = "NFFpurge_signoff";
    ...
    if ((status = (long) UMSGetByNum
        (prompt1, NFT_P_PURGE_SIGNOFF, NULL)) != UMS_S_SUCCESS)
      {
        _NFIdebug ((fname, "UMSGetByNum = <%d>\n", status));
        _NFIdebug ((fname, "defaulting prompt ...\n"));
        strcpy (prompt1, "Are you sure you want to purge signoff on");
      }

    if ((status = (long) UMSGetByNum (prompt2,
        NFT_P_CONFIRM_ITEM_REV, "%s%s", item_name, revision)) != UMS_S_SUCCESS)
      {
        _NFIdebug ((fname, "UMSGetByNum = <%d>\n", status));
        _NFIdebug ((fname, "defaulting prompt ...\n"));
        sprintf (prompt2,
    ...
    NFFglobal.working_no = NFI_I_PURGE_SIGNOFF;   /* need to change this. */

    /*  Set the NFMR function  */

    NFFglobal.NFMRfunction[0] = NFMRpurge_signoff;

...
long NFFunlock_item (catalog, item_name, revision)
  char    *catalog;
  char    *item_name;
  char    *revision;
  {
    ...
    static  char    *fname = "NFFunlock_item";
    ...
    if ((status = (long) UMSGetByNum
        (prompt1, NFT_P_CONFIRM_UNLOCK, NULL)) != UMS_S_SUCCESS)
      {
        _NFIdebug ((fname, "UMSGetByNum = <%d>\n", status));
        _NFIdebug ((fname, "defaulting prompt ...\n"));
        strcpy (prompt1, "Are you sure you want to unlock");
      }
    ...
    NFFglobal.working_no = NFI_I_UNLOCK_ITEM;

    /*  Set the NFMR function  */

    NFFglobal.NFMRfunction[0] = NFMRunlock_item;

...
long NFFcancel_item_checkout (display_flag, catalog, item_name, revision)
  int     display_flag;
  char    *catalog;
  char    *item_name;
  char    *revision;
  {
    ...
    static  char    *fname = "NFFcancel_item_checkout";
    ...
    if ((status = (long) UMSGetByNum (prompt2, NFT_P_CONFIRM_ITEM_REV, "%s%s",
                                      item_name, revision)) != UMS_S_SUCCESS)
      {
        _NFIdebug ((fname, "UMSGetByNum = <%d>\n", status));
        _NFIdebug ((fname, "defaulting prompt ...\n"));
        sprintf (prompt2, "Item \"%s\" Revision \"%s\" ?", item_name, revision);
      }

    /*  Set the working message and the NFMR function */

    NFFglobal.working_no = NFI_I_CANCEL_ITEM_CHECK_OUT;
    NFFglobal.NFMRfunction[0] = NFMRcancel_check_out_item;
    strcpy (command, CANCEL_ITEM_CHECKOUT);

    if (strcmp (NFFglobal.set_indicator, "") == 0)
       status = _NFFcancel_item_co (CANCEL_CO_CONFIRM_LABEL, display_flag,
                                    command, catalog, item_name, revision,
                                    prompt1, prompt2);
     else
       status = _NFFcancel_item_co (CN_SET_ITEM_CO_LABEL, display_flag,
                                    command, catalog, item_name, revision,
                                    prompt1, prompt2);
     if (status != NFI_S_SUCCESS)
      {
        _NFIdebug ((fname, "_NFFcancel_item_co = <0x%.8x>\n", status));
...
(f_label, display_flag, command, entity1, entity2, entity3, prompt1, prompt2)
  ...
  char   *prompt2;
  {
    ...
    static  char              *fname = "_NFFcancel_item_co";
...
long _NFFcancel_item_checkout_notify (g_label, form)
  int       g_label;
  Form      *form;
  {
    auto    long              status = NFI_S_SUCCESS;
    auto    NFFconfirm_ptr    ptr = NULL;
    static  char              *fname = "_NFFcancel_item_checkout_notify";
    ...
    switch (g_label)
      {
        ...
        case OK_BUTTON:
            ...
            status = NFFglobal.NFMRfunction[0] (ptr->entity1, (long) 0,
                          ptr->entity2, ptr->entity3, NFFwf_info, NFFoperation);

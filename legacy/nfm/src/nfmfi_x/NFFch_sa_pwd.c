...
long NFFchange_storage_area_password (st_area)
  char    *st_area;
  {
    long       status;
    ...
    static    char *fname = "NFFchange_storage_area_password";
    ...
    if ((status = (long) FIf_new (CHANGE_SA_PASSWD_LABEL,
    ...
    if ((status = (long) UMSGetByNum
        (title, NFI_P_CHANGE_SA_PWD, "%s", st_area)) != UMS_S_SUCCESS)
      {
        _NFIdebug ((fname, "UMSGetByNum = <0x%.8x>\n", status));
        _NFIdebug ((fname, "defaulting title\n"));
        sprintf (title, "for Storage Area \"%s\"", st_area);
      }
...
long _NFFchange_sa_password_notify (g_label, form)
  int       g_label;
  Form      *form;
  {
    ...
    static    char *command = NFI_M_CHANGE_SA_PASSWD;
    static    char *fname = "_NFFchange_sa_password_notify";
    ...
    switch (g_label)
      {
        ...
        case FI_ACCEPT:
            ...
            NFFform_message (CURRENT_LABEL, NFI_I_CHANGE_SA_PASSWORD, NULL);

            /*  Call NFMR to change the storage area password  */

            _NFIdebug ((fname,
                "calling NFMRchange_storage_area_passwd ...\n"));
            status = NFMRchange_storage_area_passwd ((long) 0,
                NFFglobal.storage_area, sa->old_password, sa->new_password);
            _NFIdebug ((fname,
                "NFMRchange_storage_area_passwd = <0x%.8x>\n", status));

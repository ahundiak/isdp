...
struct set_info1  *NFMset_list1=NULL; 
struct set_info1  *NFMend_set_list1=NULL;

struct attr_info1  *NFMcat_list=NULL;
struct attr_info1  *NFMend_cat_list=NULL;
...
long NFMprepare_set_checkout (user_id, cat_name, item_name, item_rev,
...
MEMptr  *attr_list, *data_list;
{
  ...
  char  *fname="NFMprepare_set_checkout";  
  long   cat_no, fcat_no, cur_cat_no=0, cur_fcat_no=0;
  long   item_num, *item_no_list=NULL;
  int   quantity;
  long  status, atol ();
  long  item_no;
  ...
  */
  ...
  _NFMdebug ( (fname, "ENTER: cat = %s, item = %s.%s level = %d\n", cat_name, 
                              item_name, item_rev, level))
  ...
  */
  ...
  status = NFMcheck_and_update_items_status1 (head_set, 1, NFM_SET_OUT);
  if (status != NFM_S_SUCCESS)
  {
    _NFMrollback_set ();
    NFMfree_linked_list ();
  ...
  if (quantity)
  {
  status = NFMcheck_and_update_items_status1 (head_set, quantity, NFM_MEM_OUT);
  if ( (status != NFM_S_SUCCESS) && (status != NFM_E_NOT_PRIMARY_MEMBERS) )
  {
    _NFMrollback_set ();
    NFMfree_linked_list ();
  ...
  status = NFMset_process_lock (user_id, NFM_I_PL_CO_SET);
  if (status != NFM_S_SUCCESS)
  {
    _NFMrollback_set ();
    NFMfree_linked_list ();
...
long NFMcancel_set_checkout (user_id, attr_list, data_list)
long      user_id;
MEMptr    attr_list, data_list;
{
  char  *fname="NFMcancel_set_checkout";
  long     status;

  _NFMdebug ( (fname, "ENTER: \n"));
  status = NFMcheck_process_locks (user_id);
  if ((status != NFM_S_SUCCESS) && (status != NFM_I_PL_CO_SET))
  {
    ERRload_struct (NFM, NFM_E_BAD_COMMAND_ORDER, "", NULL);
    _NFMdebug ( (fname, "NFMcheck_process_locks 0x%.8x\n",
                 NFM_E_BAD_COMMAND_ORDER) );
  ...
  if (status == NFM_I_PL_CO_SET)
  {
    status = NFMreset_process_lock (user_id, NFM_I_PL_CO_SET);
    if (status != NFM_S_SUCCESS)
    {
      _NFMrollback_set ();
      NFMfree_linked_list ();
...
long NFMcomplete_set_checkout (user_id, storage_name,
...
MEMptr    attr_list, data_list;
{
  ...
  char   datetime [50];
  char   *fname="NFMcomplete_set_checkout";
  long   status, atol ();
  long   sa_no=0;
  MEMptr list = NULL;
...
/* PUT IN PASSWORD SO THAT TEMP SA HAS PASSWD MRG 07/18/91 */
  ...
  */
  ...
  status = NFMupd_co_set_cat_fcat (user_id, NFMglobal.NFMusername, datetime,
  ...
  status = NFMset_colevel (NFMchildren_list->cat_name, 
                           NFMchildren_list->citemno, "-1", attr_list);
  if (status != NFM_S_SUCCESS)
  {
    NFMundo_move_files_in (user_id, STO_info.sano, passwd, &list);
    MEMclose (&list);

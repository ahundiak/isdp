...
/* Jujare, 9th Nov 1995 - To avoid STARVATION
 
   The above NFMfile_copy fails when FTRRT product is not present on the
   system. When it fails, the first NFMfileserver holds the lock permanently
   thus creating starvation for all the later NFMfileserves since they also 
   try to lock the file 
*/
   _NFMdebug((fname,"FTR File Copy has failed!! \n\
      Seems FTR Runtime Product is missing in the System \n\
      So Executing FTR Unlock !! Lock has failed ! --- Jujare, 30th Nov 1995 \n"));
	   NFMftr_options_file_unlock ( ori_file,dst_file,NETid,0);

           /* OV -IGI-320. Disconnect before return. */
           NFMdisconnect ((int *)NETid);

        ...
        if (data.sts != FORM_CANCELED)
          {
            ...
            if (strcmp (data.data[2], "") == 0)
              ...
              {
                FORM_FILLED = FALSE;
                status = NFMvalidate_Y_or_N (data.data[5]);
                sprintf (msg, "NFM: Must Enter Y Or N For Tcp Addr.");
              }
...
/*
                ...
                SCdisplay_msg ("NFM:  Must Enter Y Or N For Tcp Address.");
                }
*/
/*
            status = NFMvalidate_string("char(1)", data.data[5]); 
...
*/
    ...
    status = ADMcreate_server(sername,tcp_net);
    
    if (status != NFM_S_SUCCESS)
    {
      _NFMdebug((fname, " this is wrong\n"));
      return (status);
    }

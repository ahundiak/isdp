...
#include "WF.h"
 ...
 long WFquery_workflow_states (user_id, workflow_name,
                               attr_list, data_list, value_list)
   long      user_id;
   char      *workflow_name;
   MEMptr    *attr_list, *data_list, *value_list;
   {
     static char *fname = "WFquery_workflow_states" ;
     long     status;
     long     a, b, c, d, x, y, count;
     long     wf_no;
     MEMptr   list = NULL;
     char     **column, **data;
	...
	ERRload_struct(NFM,status,"",NULL);
     ...
     status = NFMquery_table_attributes (user_id, "NFMSYSTEM",
              "NFMSTATES", attr_list, data_list, value_list);
     if (status != NFM_S_SUCCESS)
      {
        MEMclose (attr_list);
        MEMclose (data_list);
	...
	ERRload_struct(NFM,status,"",NULL);
     ...
     a = -1; b = -1; c = -1; d = -1; column_of_valueno = -1;
     ...
     for (y = 0; y < (*attr_list) -> columns; ++y)
      {
        ...
        else if (y == c) strcat (sql_str, "n_status");
	...
	ERRload_struct(NFM,status,"",NULL);
     ...
     if (status != NFM_S_SUCCESS)
       {
	 _NFMdebug ((fname, "Corrupted value list : status = <0x%.8x>\n",
		     status)) ;
	 return (status) ;
       }
 ...
 long WFupdate_workflow_states (user_id, attr_list, data_list)
   long      user_id;
   MEMptr    attr_list, data_list;
   {     
     char *fname = "WFupdate_workflow_states" ;
     long    status;
     long    WFadd_workflow_states ();
     long    WFdrop_workflow_states ();
	...
	ERRload_struct(NFM,status,"",NULL);
     ...
     status = WFadd_workflow_states (user_id, attr_list, data_list);
     if (status != NFM_S_SUCCESS)
      {
        NFMrollback_transaction (0);
        _NFMdebug ((fname, "Add Wf States : status = <0x%.8x>\n", 
        status));
        return (status);      
      }

     status = WFdrop_workflow_states (user_id, attr_list, data_list);
     if (status != NFM_S_SUCCESS)
      {
        NFMrollback_transaction (0);
        _NFMdebug ((fname, "Drop WF States : status = <0x%.8x>\n", 
        status));
        return (status);      
      }
...
*/
 ...
 long WFadd_workflow_states (user_id, attr_list, data_list)
   long    user_id;
   MEMptr  attr_list, data_list;
   {
     char *fname = "WFadd_workflow_states" ;
     long   status;
     ...
     for (y = 0; y < data_list -> rows; ++y)
      {
        ...
        if (! strcmp (data1 [count1 + i], "ADD"))
         {
           sprintf (sql_str,  "INSERT INTO %s (", "NFMSTATES");
           ...
           for (x = 0; x < attr_list -> rows; ++x)
            {
              ...
              if (! strcmp (data [count + a], "n_stateno"))
               {
                 status = NFMget_serial_slot (user_id, "nfmstates",
                          data [count + a], &state_no);
                 if (status != NFM_S_SUCCESS)
                  {
                    free (sql_str);
                    free (sql_str1);
           ...
           if (status != SQL_S_SUCCESS)
            {
              ...
              if (status != SQL_E_DUPLICATE_VALUE)
               {
                 status = NFM_E_STATE_EXISTS;
	ERRload_struct(NFM,status,"",NULL);
                 _NFMdebug ((fname,  "Dup State : status = <0x%.8x>\n",
                 status));
                 return (status);
               }
 ...
 long WFdrop_workflow_states (user_id, attr_list, data_list)
   long    user_id;
   MEMptr  attr_list, data_list;
   {
     char *fname = "WFdrop_workflow_states" ;
 ...
 long WFload_states_list (user_id, attr_list, data_list, inline)
   long     user_id;
   MEMptr   attr_list, data_list;
   char     *inline;
   {
     char *fname = "WFload_states_list" ;
     long    status;
     ...
     for (x = 0; x < attr_list -> rows; ++x)
      {
        ...
        if (! strcmp (data [count + c], "n_stateno"));

...
long NFMpccheckin_item (catalog_name, catalog_no, item_name, item_revision,
...
{
  char *fname="NFMpccheckin_item";
  char fcat_name[NFM_CATALOGNAME+3];
  char sql_str[2048], sub_str[1024], row_str[1024], w_clause[1024];
  ...
  char temp_datetime[NFM_DATE+1], itemdesc[NFM_ITEMDESC+1];
  char dumb_str[1024], error_msg[1024], type_m_file[NFM_COFILENAME+1];
  long  x, y, count, index, index1, first_time, first_move=0, move_move=0;
  long  nothing, sto_no;
  long i_co, i_cp, i_ref;
  int subsystem;
  long q_itemno=0, q_status=1, q_archivestate=2, q_lock=3, q_versionlimit=5,
       q_itemdesc=6;
  long type_m=0, type_acad=0, known=0, archive_no=0;
  long i_fileno, i_itemnum, i_filenum, i_cofilename, i_fileversion, i_filetype,
  ...
  if (status != NFM_S_SUCCESS)
  {
    ...
    if (DM_MANAGER)
      ERRload_struct (NFM, NFM_E_MESSAGE, "%s",
                      "Failed to retrieve document's information from catalog");
    else
      ERRload_struct (NFM, NFM_E_MESSAGE, "%s",
                      "Failed to retrieve item's information from catalog");
    _NFMcleanup (&degree, wf_info, "", "", "");
  ...
  strcpy (itemdesc, data[q_itemdesc]);
      ...
      (!first_time && strcmp (sto_list->saname, working_area)) ) 
  {
    ...
    _NFMcleanup (&degree, wf_info, catalog_name, item_no, file_status);
    MEMclose (&list);
    _NFMdebug ((fname, "the file(s) was/were checked out to a different PC\n"));
  ...
  { 
    ...
    if (type_m || type_acad)
    {
      ...
      status = NFMpcbuild_microstation_set (catalog_name, catalog_no,
      ...
      else if (status != NFM_S_SUCCESS)
      {
        ...
        /* replace the following NFM_E_MESSAGE after this delivery, i
           because there is no time to introduce new messages 
           ERRload_struct (NFM, NFM_E_OPEN_DRAWING, "", NULL);*/
        ERRload_struct (NFM, NFM_E_MESSAGE, "%s",
                        "Failed to open Microstation/AUTOCAD files");
        return (status);
      }
      SQLstmt ("COMMIT WORK");
    }
  ...
  if (operation.purge == NFM_PURGE)
  {
    ...
    if (status != NFM_S_SUCCESS)
    {
      _NFMdebug ((fname, "_NFMpcfs_purge 0x%.8x\n", status));
      ERRload_struct (NFM, NFM_S_SUCCESS, "", NULL);
      if (_NFMdebug_st.NFMdebug_on)
        MEMprint_buffer ("file_list", file_list, _NFMdebug_st.NFMdebug_file);
    }

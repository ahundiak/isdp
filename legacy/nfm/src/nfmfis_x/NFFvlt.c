...
long _NFFsetup_vlt ()
  {
    ...
    static  char     *fname = "_NFFsetup_vlt";
    ...
    NFFglobal.numslots = info[vs].vsi_VLT_size;

    /*  Find out what kind of vlt hardware is present  */

    switch (NFFglobal.numslots)
      {
        case VLT_TWO_SIZE:

            _NFIdebug ((fname, "Loading a vlt with 2 slots ...\n"));

            if (status = Loadvlt (vs, vlt_two, NFFglobal.numslots))
              {
        ...
        case VLT_FOUR_SIZE:
            ...
            if (status = Loadvlt (vs, vlt_four, NFFglobal.numslots))
              {
                _NFIdebug ((fname, "Loadvlt = <%d>\n", status));
                ERRload_struct (NFI, NFI_E_VLT, "%s%d", "Loadvlt", status);
                _NFIdebug ((fname, " returning FAILURE\n"));
    ...
    if ((vlt = (struct vlt_slot *) malloc
        (NFFglobal.numslots * sizeof (struct vlt_slot))) == NULL)
      {
        _NFIdebug ((fname, "malloc failed; size = <%d>\n",
            NFFglobal.numslots * sizeof (struct vlt_slot)));
        ERRload_struct (NFI, NFI_E_MALLOC,
            "%d", NFFglobal.numslots * sizeof (struct vlt_slot));
        _NFIdebug ((fname, " returning FAILURE\n"));
        return (NFI_E_MALLOC);
      }

    if (status = Readvlt (vs, vlt, NFFglobal.numslots, 0))
      {
    ...
    if (status = Loadvlt (vs, vlt, NFFglobal.numslots))
      {
        _NFIdebug ((fname, "Loadvlt = <%d>\n", status));
        free (vlt);
        ERRload_struct (NFI, NFI_E_VLT, "%s%d", "Loadvlt", status);
...
int _NFFtranslate_color (vs, color)
  int    vs;
  int    color;        
  {

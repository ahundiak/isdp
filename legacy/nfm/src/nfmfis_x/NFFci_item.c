...
long NFFcheckin_item (display_flag, catalog, item_name, revision)
  int     display_flag;
  char    *catalog;
  char    *item_name;
  char    *revision;
  {
    ...
    static  char    *fname = "NFFcheckin_item";
  ...
  /* prompt needs to change for confirm */
    if ((status = (long) UMSGetByNum (prompt1, NFT_P_CONFIRM_CHECKIN,
                                      NULL)) != UMS_S_SUCCESS)
      {
    ...
    NFFglobal.working_no = NFI_I_CHECKIN_ITEM;

    strcpy (command, CHECKIN_ITEM);

    /*  Set the NFMR function  */

    NFFglobal.NFMRfunction[0] = NFMRcheckin_item;

    if (strcmp (NFFglobal.set_indicator, "") == 0)
       status = _NFFfile_io (CHECKIN_CONFIRM_LABEL, display_flag, command,
                             catalog, item_name, revision, prompt1, prompt2);
    else
       status = _NFFfile_io (CI_SET_ITEM_LABEL, display_flag, command,
                             catalog, item_name, revision, prompt1, prompt2);

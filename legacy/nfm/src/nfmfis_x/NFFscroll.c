...
void _NFFscroll_message (form, label)
  Form   form;
  int    label;
  {
#ifdef XWINDOWS
    ...
    int     trans;
    static  char *fname = "_NFFscroll_message";

    _NFIdebug(( fname, "Entering ...\n" ));

    /*  Scroll the message one character  */

    _NFFscroll_message_one_word (form, label);

    /*  Check to see what the button transition was  */

    if (NFFglobal.button_transition == SINGLE_CLICK)
      {
...
/* Intelsolaris Port IGI -  12/07/95 */ 
/* For similar problem on Sparc Sun Solaris - IGI  */
	...
	if (event.type == Expose || event.type == ButtonRelease || 
        ...
        if (event & BUTTON_EVENT)
          {
            JNL_get_button_data (&area, &x, &y, &button, &trans, &timetag);
...
void _NFFscroll_message_one_word (form, label)
  Form   form;
  int    label;
  {
    static  char *fname = "_NFFscroll_message_one_word";

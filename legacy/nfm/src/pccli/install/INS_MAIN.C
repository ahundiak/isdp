...
*/
  fprintf(stderr,"*******************************************************\n");
  fprintf(stderr,"*                                                     *\n");
  fprintf(stderr,"*         <<<<<< BEFORE YOU BEGIN >>>>>>              *\n");
  fprintf(stderr,"*                                                     *\n");
  fprintf(stderr,"*%12.12s%-41.41s*\n"," Please add ",bpath);
  fprintf(stderr,"* to the PATH in your autoexec.bat file               *\n",bpath);
  fprintf(stderr,"*                                                     *\n");
  fprintf(stderr,"*******************************************************\n");
...
*/
  ...
  while ((c = getopt(argc,argv,"do:p:h:n:")) != EOF)
    {
      no_switches_flag = 0 ;
      switch(c)
        {
	...
	case 'h':
	  ...
	  break ;
	case 'n':
	  strncpy (netid, optarg, sizeof(netid)) ;
	  if(strcmp(netid,"FTP") == 0) nfs_package = FTP;
	  else nfs_package = PC_NFS ;
	  break ;
	...
	case 'd' :
	  ...
	  default :
	    error_flag = 1;
	  fprintf (stderr,"Usage: %s [-d [-o debug file] [-h hostname] [-p complete nfm_path] -n <Network (NFS/FTP)>\n", argv[0]);
	  break ;
  ...
  if (no_switches_flag)
    {
      error_flag = 1;
      fprintf (stderr,"Usage: %s [-d [-o debug file] [-h hostname] [-p absolute nfm_path] -n <Network (NFS/FTP)>\n", argv[0]);
    }
  ...
  if(!strlen(netid))
    {
      fprintf (stderr, "Please specify network type  [-n <network ID (NFS/FTP)>]\n") ;
      return (CLI_E_FAILURE) ;
    }
  if(strcmp(netid,"FTP") && strcmp(netid,"NFS"))
    {
      fprintf (stderr, "Please specify network type  [-n <network ID (NFS/FTP)>]\n") ;
      fprintf (stderr, "Valid network ID NFS or FTP\n") ;
      return (CLI_E_FAILURE) ;
    }

...
long NFMblocksac_list(attr_list,out_list,cat_name,cat_no,search_name , sort_name,query_flag,row_size)
	MEMptr	attr_list, *out_list;
	char *search_name , *sort_name;
	char *cat_name,*cat_no;
	long query_flag,row_size;
{
	long status = 0,i=0,m,n,size = 0;
	long length=0,count;
	char *qry_str,return_str[NFM_FILENAME],format_str[100];
	char **data,**format;
	char *fname="NFMblocksac_list";
	long SQLquery(),SQLquery_cursor2();
...
attr_list,out_list,cat_name, cat_no,search_name,sort_name,query_flag,row_size));
...
*/
 	if((status=SQLquery_cursor2 (qry_str, out_list, 32*MEM_SIZE,row_size))!=SQL_S_SUCCESS)
	{

...
long   catalog_no = -1;
int    confirm;
static char *fname = "NFAcheckin_item";

    SCdisplay_msg ("Working..");    

    NFAconfirm( "check in this item", &confirm );
    if( confirm == 0 )
        return( NFI_S_SUCCESS );

    SCdisplay_msg ("Working..");

    NFAset_global_config_info( NFM_WORKFLOW, CHECKIN_ITEM,
	   		       NFMworkflow, "", "NFM" );

    status = NFMRcheckin_item( NFMcatalog, catalog_no, NFMitem,
		NFMrev, NFMworking, NFAwf_info, NFAoperation );

    _NFIdebug(( fname, "Return from NFMRcheckin_item -> 0x%.8x\n", 
		    status));
...
static char *fname = "NFAcheckin_item_from_list";
...
"", "", title);
    ...
    NFAconfirm( "check in these item(s)", &confirm );
    if( confirm == 0 )
    {
	NFAfree_list_struct( &display_struct );
        return( NFI_S_SUCCESS );
    ...
    for (i=0,k=0; i<data.num_of_rows; ++i)
    {
	    ...
	    SCdisplay_msg( title );
 	    ...
 	    status = NFMRcheckin_item( NFMcatalog, catalog_no, 
	        data.data[j+NFMitem_loc], data.data[j+NFMrev_loc], NFMworking, 
 		NFAwf_info, NFAoperation );

            _NFIdebug(( fname, "NFMRcheckin_item -> 0x%.8x\n",status));

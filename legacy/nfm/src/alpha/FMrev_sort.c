...
struct SCfree_data_st fdata;
    ...
    status = NFMRretrieve_query_info( NFMsort_name, "O", "NFM", "NFM",
    ...
    order_rows = ( strlen( query_struct->order_by_clause )/50 ) + 1 ;
    _NFIdebug(( fname, "Where rows <%d>\n", order_rows ));
    fdata.num_of_rows = 2 + order_rows;
    fdata.num_of_cols = 2;
    ...
    sprintf (fdata.title, "Sort Information for <%s>", NFMsort_name);
    ...
    NFAput_string (&fdata.data[4], "Sort");
    for( i=0,j=0; i<order_rows; ++i,j+=fdata.num_of_cols )
    {
	strncpy( order_by_clause, (query_struct->order_by_clause+(i*50)), 50 );
        NFAput_string (&fdata.data[5+j], order_by_clause );
    }

...
#	   other than the already ems converted design files.
#
#       3. The variable EMS needs to be set to the right ems path.
#          (i.e. EMS=/usr/ip32/ems).
#
#	4. The shell script cvt_batch1.sh, cvt_new.e, cvt_batch1.e and
#          login.info should all reside in another separate directory.
#
...
if [ "$#" -lt 1 ]
then
	echo "Usage: cvt_batch1.sh <dirname> "
...
do
	echo $file
	$EMS/bin/ems -b -f $file -C' "ci=cvt_batch1" '
done

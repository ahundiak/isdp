/*
Abstract
		This COB file defines the command object which serves as
		the Command Launcher for PDU.  It executes PDU commands
		external to EMS by calling the function PDUexternal,
		and executes graphics related  PDU commands by calling
		the function PDUinternal.
History:
		file created - 08/14/87 - Bill Kirk
		file modified - 09/18/87 - Bill Kirk
		file modified - 12/10/87 - Bill Kirk
		file modified - 01/08/88 - Bill Kirk
		file modified - 02/11/88 - Bill Kirk
		file modified - 03/31/88 - Bill Kirk
		file modified - 11/01/88 - Lynn Mettlen 1.2 changes
                file modified - 10/30/90 - Jamal Shakra 141 changes
                file modified - 12/28/93 - Audra Romans 3.0 fixes
*/

product_name    "$PDU"
command_string	"pdu command launcher", 0
class		COpdu
...
/************************************************************************
 * This action invokes the function PDUexternal, which uses the command *
 * type from the refresh area to call the appropriate  function to      *
 * perform the required actions external to ems.  The resulting status  *
 * is returned in the refresh area.					*
 ************************************************************************/
...
action	get_prompt_type
  {
   *sts = PDU_prompt_type;
  }

action	pdu_chg_wf_state
  {
...
action	init
 ...
 /*************************************************************************
  This action gets invoked after the wakeup message is sent by the command
  server. This command object needs things done in addition to what the
  CEO wakeup does. Structures are initialized here because addresses can
  change when the command object is sleeping and require updating when the
  command object wakes up.
  *************************************************************************/

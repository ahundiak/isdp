...
/* gadgets from EXretrieve form */
#define EX_PROJECT_FLD           37
#define EX_PROJECT_BTN           49
#define EX_CATALOG_FLD           39
#define EX_CATALOG_BTN           50
#define EX_PARTNUM_FLD           40
#define EX_PARTNUM_BTN           51
#define EX_REVISION_FLD          42
#define EX_REVISION_BTN          52
#define EX_DESCRIPTION           47
#define EX_FILENAME_FLD          48
#define EX_FILENAME_BTN          54
#define ASSEMBLY_DISPLAY         30
#define DESIGN_OR_REVIEW         34
#define SEARCH                   31
...
int size;
{
 ...
 if( buf  ||  ( buf = (char *)malloc( size ) ) ) {
    ...
    if( pipe( fildes ) == 0 ) {
#if defined(CLIX) || defined(sparc) || defined(__Sol2__)
	if( child = vfork() ) {
#elif defined(IRIX)
	if( child = fork() ) {
#else
...
*/
    ...
    if ( pdir )
     {   
         msg_str = (char *)PDUtranslate_message(PDM_I_READING_DIRECTORY);
  ...
  Form		fp;		/* i - Form pointer		*/
{
  ...
  case ASSEMBLY_DISPLAY:
  case DESIGN_OR_REVIEW:
  case SEARCH:
       PDUprocess_ex_file_form(fp,g_label);
       break;
...
*/
	...
	FIg_set_text(fp,FORM_TITLE,msg_str);

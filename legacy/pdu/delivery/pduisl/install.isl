...
#
   ...
   echo WARNING:
   echo
   echo I/PDU version 3.0 is dependent on I/PDM version 3.0
   echo and NFM 3.4!!!!!!!
   echo
...
then
   ...
   echo
    ...
    apianswer=`getanswer "Install PDU/PDM api examples locally (y/n) ? " "n"`
    fi

    echo
   echo Solaris CPU API libraries require approximately ${PDUapi_sol2} blocks.
   apianswer=`getanswer "Install the Solaris CPU API libraries (y/n) ? " "y"`
...
then
  ...
  echo
  rm $UNIXDIR/config/dload/patch/*.so
fi
...
then
  ...
  echo
    echo Installing I/PDU in $UNIXDIR for INTEL Solaris CPU
    echo Installing I/PDU in $UNIXDIR for INTEL Solaris CPU >>$LOGFILE 2>&1
  echo
  date >>$LOGFILE 2>&1
...
#    getfile pdureadme.${SUFF} | compress -d | cpio -ivmud >>$LOGFILE 2>&1
#    AOK=$?
#    if [ "$AOK" != "0" ] ; then exit $AOK; fi
...
# Install MODEL executable
#
#if [ "$answer131" = "y" ]
#then
#  modelpath=`proddata +%p model`
#  if [ "${modelpath}" != "" ]
#  then
#       \cd ${modelpath}
#       getfile model.${SUFF} | compress -d | cpio -ivmud >>$LOGFILE 2>&1
#       AOK=$?
#       if [ "$AOK" != "0" ]; then exit $AOK; fi
#    fi
#fi
...
#ln -s $INGRHOME /usr/ip32
 if [ -d /usr/ip32 ]
 then
 ln -s $INGRHOME/pdu /usr/ip32/pdu
 else
 ln -s $INGRHOME /usr/ip32
 fi
else

...
%token _c_select _c_distinct _c_from _c_table_name
%%
...
condition 	:  expression oper expression
		  ...
		  |
		   expression_in_paranth
		   {
		     strcpy (col_nam, c_temp);
		   }
		    _c_select _c_distinct _c_column_name
		   {
		     strcpy (tblcol, yytext);
		     strcat (tblcol, "~");
		     tbl = strtok (tblcol, ".");
		     col = strtok (NULL, "~");
		     sprintf (SVSquery,
		   ...
		   _c_from _c_table_name _c_close
		  |
		   expression _c_not 
      		   { strcat (exp_cond, " not "); }
		   _c_in 

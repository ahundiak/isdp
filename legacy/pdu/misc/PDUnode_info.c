...
#if defined (sparc) || defined (__sgi) || defined (__Sol2)
    char                        *nodename = NULL;
#endif
   ...
   if (strcmp(user_info->pw_passwd,""))
     {
      ...
      form_data.attr_list_expected = 0;
...
/* 12/1/95 MJG - user getenv instead of PDUgetcwd
...
*/
   ...
   if (strlen(remote_path) > 0)
     {
      _pdm_debug("get_remote_path returns <%d>", (char *)remote_path);
...
/* Vasu 21.2.92 - Alternative mechanism to get the local node name */
  ...
  if (status != PDM_S_SUCCESS)
   ...
   _pdm_debug("PDUget_sun_nodename returns status = <%d>", (char *)status);
...
/* Vasu 21.02.92 - End addition */
   ...
   _pdm_status("clh_vbyop returns status = ", status);
...
void PDUprint_storage_area()
{
	_pdm_debug("STORAGE AREA", 0);
        _pdm_debug("storage->storage_no     = <%d>", (char *)storage->storage_no);

...
main()
{
  ...
  status = pdmoLoadFileStructure("file", "api_oct1", "api_oct1", "i", 1,
                                 &file_list);
  if (status != PDM_S_SUCCESS)
  {
  ...
  status = pdmoLoadFileStructure("file", "api_oct2", "api_oct2", "i", 1,
                                 &file_list);
  if (status != PDM_S_SUCCESS)
  {
    printf("\nReturn status from loading file structure <0x%.8x>",
  ...
  printf("\nAdding part to catalog\n");

  status = pdmoAddPart("apicat", "api_oct1", "a", "G", "api part", "PDM_PART_ACL", "NFMFILES", NULL, file_list);
  if (status != PDM_S_SUCCESS)
    printf("\nAdd part failed\n");

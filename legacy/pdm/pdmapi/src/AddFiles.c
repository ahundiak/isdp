...
main()
{
  ...
  printf("\n\nLoading file structure with first file\n");

  status = pdmoLoadFileStructure("file", "api_oct1.m01", "api_oct1.m01", "i", 1,
                                 &file_list);
  if (status != PDM_S_SUCCESS)
  {
  ...
  printf("\n\nLoading file structure with second file\n");

  status = pdmoLoadFileStructure("file", "api_oct2.m01", "api_oct2.m01", "i", 1,
                                 &file_list);
  if (status != PDM_S_SUCCESS)
  {
    printf("\nReturn status from loading file structure <0x%.8x>",
  ...
  status = pdmoAddFiles("apicat", "api_oct1", "a", "NFMFILES", file_list);
  if (status != PDM_S_SUCCESS)
  {
    printf("\nReturn status from pdmoAddFiles <0x%.8x>",
      status);
    printf("\nFreeing file structure");

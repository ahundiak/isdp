...
struct   file_info      *spr_files;
{
  ...
  _pdm_debug("in the function pdmoAddPart", 0);
  ...
  status = PDUget_col_number(data_bufr,"n_itemname",data2_col, &itemname_col);
  _pdm_status("PDUget_col_number", status);
  if (status != PDM_S_SUCCESS)
    return(status);

  status = PDUget_col_number(data_bufr,"n_itemrev",data2_col, &rev_col);
  _pdm_status("PDUget_col_number", status);
  if (status != PDM_S_SUCCESS)
    return(status);
  ...
  if (spr_attrs)
    {
    status = PDUverify_attr_list(spr_attrs,attr_bufr,list_bufr,&data_bufr,ADD);

...
# 02/02/93   dmb     Pilot attempt at unbundled packaging. We'll start
#                    with MIPS first. 
# 03/05/93   dmb     Replaced with package70.sh
#
# Notes:
...
# dmb:03/05/93:Changed to use PROC_TYPE_PACKAGE_LIST by default
  PROC_TYPE_PACKAGE_LIST="$PROC_TYPE_PACKAGE_LIST"
  ;;
esac
...
#      export PROD_NAME=$Product
      echo "   \$PRODINFO/package.sh $CURRENT_PROC_TYPE for $Product"
      PROD_NAME=$Product $SYSTEM_SOURCE/prodinfo/package.sh $CURRENT_PROC_TYPE
    )
  done
  echo ""

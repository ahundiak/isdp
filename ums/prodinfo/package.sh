...
#dmb:04/26/93:Like so much else, just guessing here...
    NewprodSuffix=sco
    NewprodPdefSuffix=sco
    ;;
  I386 )
    ArchType=sco
    ArchTypeKey=SCO
...
# dmb:03/25/93: Changing to unbundled model for ISS7.1
    ...
    in
      4.* )
        ...
        ;;
      5.* )
# dmb:03/25/93:I think this will be the defaulr for Solaris (SUNOS5)
        NewprodSuffix=ssl
        NewprodPdefSuffix=ssl
        ;;
    esac
...
#dmb:04/22/93:New location
#dmb:04/22/93:BOM Products have no internal components, so we never
# get into the component loop
  if [ ! -d "${PROD_DLVR}" ]
  then
...
#dmb:04/22/93:Original location
#  if [ ! -d "${PROD_DLVR}" ]
#  then
#    PAS_f_mkdir "${PROD_DLVR}"
#  fi
...
# sml:08-11-93:Removed -mud options, not compatible with -o
#              This is documented that way on sunos5, clix, and irix.
#              Irix is unforgiving on this point.
#  find * -depth -print | cpio -covmud | compress > $PROD_FILE_LOC  
  find * -depth -print | cpio -cov | compress > $PROD_FILE_LOC  
...
#dmb:04/22/93:Adding BOM check so we can use this for all products.
BOM_PROD=''
#if echo $DELattr | grep BOM >/dev/null
if grep 'DELattr=.*BOM.*' $P_DEF >/dev/null
then
  BOM_PROD=y
fi
...
do
  ...
  then
    echo 1>&2
#echo BOM_PROD is $BOM_PROD
    if [ "$BOM_PROD" ]
    then
      CompSpace=0
      echo "$0: INFO: CompSpace of $CompSpace is OK for BOM Products" 1>&2
      echo 1>&2
    else
...
in
  "SCO" )
#    DEL_SIZE_VAR="${ArchTypeKey}_SCO_DELsize"
...
#    DEL_SIZE_VAR="${ArchTypeKey}_DELsize"
  ...
  "INTEL_SOLARIS" )
#    DEL_SIZE_VAR="${ArchTypeKey}_DELsize"
    DEL_SIZE_VAR="DELsize"    # dmb:08/03/94:Now that we're unbundled ...
   ;;
  * )
   echo "package: unrecognized ArchTypeKey: " ${ArchTypeKey}
   ;;
esac
...
# dmb:04/28/93:Remove share.sh for ISS7.1
...
#  unbundling
# dmb:04/28/93:Just plan on using Suffixes for everyone except C[134]00's
# dmb:04/28/93:As a matter of fact, we can probably use $OS_KEY here.
# dmb:04/28/93:case $OS_KEY
#in
#  CLIX ) ;;
#  * ) ;;
#esac
...
#dmb:04/02/93:I understand that install.sh remove.sh are no longer needed
#   on other platforms, but are still used on CLIX.

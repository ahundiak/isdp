...
int	UMCReadTxt( char *textFile, UMCFileData *fileData )
{
...
char	line[UMC_BUFLEN];
	...
	{
		UMCErrorMsg( COMP_F_OPENTEXT, "%s", textFile );
		perror( "" );
...
int	UMCFileHdr( char line[], int lineNum, UMCFileData *fileData )
{
...
int	UMCKeyLine( char line[], int lineNum, UMCFileData *fileData )
{
...
int	UMCMsgLine( char line[], int lineNum, UMCFileData *fileData )
{
  ...
  else if ( strlen( p ) > (size_t) (UMS_FILE_EXT_LEN - 1 ))
    {
...
int	UMCGetLine( FILE *fp, char line[], int *lineNum )
{
  int	start_arg, end_arg;
  char	*ptr, tmpLine[UMC_BUFLEN];
	...
	while ( 1 )
	{
		...
		start_arg = strspn( tmpLine, " \t" );
		if ( tmpLine[start_arg] == '#' )
			continue;

		for ( end_arg = strlen( tmpLine ); end_arg > start_arg; end_arg-- )
		{
			ptr = &tmpLine[end_arg - 1];
			if ( *ptr != '\n' && *ptr != ' ' && *ptr != '\t' )
				break;
		}
		tmpLine[end_arg] = '\0';

		if ( strlen( &tmpLine[start_arg] ) )
		{
			strcpy( line, &tmpLine[start_arg] );
			return( COMP_S_SUCCESS );
		}
	}
}
...
int	UMCCheckMem( int msgLen, int extLen, int key2Len,
        UMCFileData *fileData )
{
	...
	if ( fileData->hdr.extBufLen + extLen > fileData->extAlloc )
	{
	...
	if ( fileData->hdr.key2BufLen + key2Len > fileData->key2Alloc )
	{

...
%HD%
     ...
     MODULE NAME GRelaxaxpt
 
     Abstract: This is the router called by command objects for
	      constructing an ellipse given three events which
	      specify the  endpoints of the axes and a point on
	      the ellipse.
-----
%SC%

     value = GRelaxaxpt(msg,events,num_events,el)
...
extern    IGRboolean    GRelaxaxpt_rtr();
extern    IGRboolean    GRprerout();
extern    IGRboolean    GRccschoice();

IGRboolean GRelaxaxpt(msg,events,num_events,el)
  ...
  *msg = MSSUCC;
  ...
  if( *num_events <= GRMAX_EL_EVENTS )		/* valid number of events?    */
      ...
      if( GRprerout(msg,events,num_events,&max_id,&max_val,rtevents) )
	{
	  ...
	  GRccschoice(msg,&level,rtevents,num_events,&view,&ccw,GRelaxaxpt_rtr,el);
        }
    }
  else
    {

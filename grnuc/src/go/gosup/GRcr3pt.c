...
%HD%
     ...
     Abstract: This is the router called by command objects for
	      constructing a circle given three events which define 
	      three points on the circle.
-----
%SC%

     value = GRcr3pt(msg,events,num_events,cr)
...
extern    IGRboolean    GRcr3pt_rtr();
extern    IGRboolean    GRprerout();
extern    IGRboolean    GRccschoice();

IGRboolean GRcr3pt(msg,events,num_events,cr)
  ...
  *msg = MSSUCC;
  ...
  if( *num_events <= GRMAX_CR_EVENTS )		/* valid number of events?    */
      ...
      if( GRprerout(msg,events,num_events,&max_id,&max_val,rtevents) )
	{
	  ...
	  GRccschoice(msg,&level,rtevents,num_events,&view,&ccw,GRcr3pt_rtr,cr);
        }
    }
  else
    {

...
GUotype24( msg, file_des, dgn_buf, igds, igds_size, class_name,
	   geometry, range, symbology, geometric_props, properties, bss_attr,
	   has_name, name )
...
{ /* begin GUotype24 */
	...
	IGRdouble		knots[1000];	/* array for u/v knots */
...
extern    void    BSfreesf();
	...
	*msg = GUsuccess;
	...
	if ( geometry->u_num_poles > 101 || geometry->v_num_poles > 101 )
	{
	    brc = FALSE;
	    *msg = MSFAIL;
	    goto wrap_up;

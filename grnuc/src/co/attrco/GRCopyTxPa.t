...
command_string  GRMCopyTxAttr,COPYTXSYMB,GRCpTxPr
class		COcpattr
super_class     COtools
super_class     COdpb
...
state *
     ...
     on EX_RESTART                                     state s1
     on EX_RJT_MOVEON                                  state .

state s1
     
     message_key         GRC_M_CopyTxAttr
...
on EX_BACK_UP                                                  state .
on EX_RJT_MOVEON                                               state s2
on EX_DATA          do COdpb.extract ( sts,
                                       me->mask,
                                       &me->located_chansel )

     on SUCCESS     do form_update ( me->mask, 0 )             state s2
...
on EX_BACK_UP or EX_RJT_MOVEON                                 state s1

...
/*
     ...
     CO_ni_place_vertical_line()

     Abstract:
        Non-interactive function for construction of a Line Segment which
        is VERTICAL in the view specified by the first data point event.
...
#define  VERTICAL ( 0 )  /* in GRhvls2pt(): HV_Flag == 0 means vertical */
 ...
 *----------------------------------------
 */
IGRlong CO_ni_place_vertical_line( ReturnMsg,
				     Events,
                                     DisplayMode,
                                     newflag,
                                     ModuleInfo,
...
/* BV - BEGINNING OF VARIABLE LIST */
    ...
    IGRboolean
	         HV_Flag = VERTICAL;   /* indicates vertical or vertical */
...
/* EV - END OF VARIABLE LIST */
    ...
    _m_debug_str( "CO_ni_place_vertical_line >" );
    ...
    /*
     * ------------------------------------------------------------------------
     *   Step 1:  Get b-sline geometry of vertical line
     * ------------------------------------------------------------------------
     */
    ...
    _m_check2p( ReturnStatus, *ReturnMsg,
       "CO_ni_place_vertical_line:  GRhvls2pt failed" );
     ...
     * -----------------------------------------------------------------------
     */
    _m_debug_str( "CO_ni_place_vertical_line:  calling co$construct" );
    if ( classname != NULL )
    {
       /*
    ...
    _m_check2p( ReturnStatus,
                *ReturnMsg,
                "CO_ni_place_vertical_line - failed in co$construct object" );

quit:
    _m_debug_str( "< CO_ni_place_vertical_line" );
    return ( ReturnStatus );

} /* end CO_ni_place_vertical_line() */

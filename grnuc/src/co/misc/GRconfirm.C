...
Notes
        This function has a twin in src/util/misc/GRconfirm.c.  Any changes
        made here need to be mirrored there.  The only real difference between
        the functions is that this version uses co$getevent so as to get along
        with RAP & journaling.  The other version handles its own input, and
        is intended for use outside of GRNUC (like in revasm).

        The other function is included in lib/utilmisc.a.  This one is
...
History
        ...
        mrm     10/26/93    change to use co$getevent so RAP & journaling
                            will work with GRconfirm
*/
...
int GRconfirm_callback(int f_label, int g_label, double value, Form form)
{
    ...
    /* push something on the queue to kick GRconfirm_box out of co$getevent */
    if (form_dismissed)
...
int GRconfirm_box(char *message)
{
    ...
    double response_data[1024/sizeof(double)];
    Form form;
    ...
    while (1)
    {
        /* call getevent - eat anything */
        co$getevent(msg = &msg,
                    event_mask = (long)-1, /* this gets 'em all */
                    event = &event,
                    response = &response,

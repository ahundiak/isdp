 ...
 * I/VDS
 *
 * File: vds/emsfixes/pdu/filemgr/PDUdsgn_frm.c
 *
 * Description:
 *
 * Dependencies:
 ...
 *       JTSMP CR 5924
 *
 *       Revision 1.1  2001/09/07 18:30:57  jdsauby
 *       Final Checkin for AIM / PDU integration  - jds
 *
 *       Revision 1.1  2001/01/19 23:51:52  ramarao
 *       Merged from SP.
 *
 * Revision 1.4  2000/10/25  19:03:24  pinnacle
 * Replaced: vds/emsfixes/pdu/filemgr/PDUlf_form.c for:  by impd252 for Service Pack
 *
...
extern void PDUvd_SetLFVDrefreshData();
extern void PDUvd_GetLFVDrefreshData();

//extern int      local_files_notification_routine ();
/********************************************************
*
  ...
  Form    fp;            /* Pointer to the form     */
{
  ...
  char catalog_val[25], partid_val[25], revision_val[25];
  ...
  switch ( g_label )
  {
    ...
    case PART_REV_FLD:         /* Field Type:    ASCII    */
	 ...
	 FIfld_get_text_length(fp, PART_REV_FLD, 0, 0, &length); 

	 text = (char *) malloc (sizeof (char) * length + 1);
	 memset(text, NULL, sizeof (char) * length + 1 );
	 FIfld_get_text(fp, PART_REV_FLD, 0, 0, length, 
	               (unsigned char *)text, &select, &pos);
	
	 strcpy(revision_val, text);

	 strcpy(catalog_val, refresh->rev_catalog);
	 strcpy(partid_val, refresh->rev_partid);

	 if(catalog_val != NULL && partid_val != NULL && revision_val != NULL)
	 {

	 	/*status = PDMQueryforMdcExists(catalog_val, partid_val,
		revision_val);*/
		 status = PDMInfoOnCommandbar(catalog_val, partid_val, revision_val); 
	}
...
*/
    ...
    PDU_active_button = CATALOG_LIST_BTN;
      ...
      // char *catalog_val, *partid_val, *revision_val;
  ...
  /* text verification for catalog */

         FIfld_get_text_length(fp, CATALOG_FLD, 0, 0, &length);
         text = (char *) malloc (sizeof (char) * length + 1);
	 memset(text, NULL, sizeof (char) * length + 1);
	 
         FIfld_get_text(fp, CATALOG_FLD, 0, 0, length, 
                      (unsigned char *)text, &select, &pos);
		  
	  	 
         strcpy(catalog_val, text);

   /* text verification for partid */

       FIfld_get_text_length(fp, PARTNUM_FLD, 0, 0, &length);
       text1 = (char *) malloc (sizeof (char) * length + 1);
       memset(text1, NULL, sizeof (char) * length + 1);
       
       FIfld_get_text(fp, PARTNUM_FLD, 0, 0, length, 
                       (unsigned char *)text1, &select, &pos);
	

	strcpy(partid_val, text1);

   /* text verification for revision */
   ...
   FIfld_get_text_length(fp, PART_REV_FLD, 0, 0, &length);
    ...
    FIfld_get_text(fp, PART_REV_FLD, 0, 0, length, 
       ...
       strcpy(revision_val, text2);
	 
	 if(catalog_val != NULL && partid_val != NULL && 
	 revision_val !=NULL )
	 {
	    /*status = PDMQueryforMdcExists(catalog_val, 
	    partid_val, revision_val);*/
		status = PDMInfoOnCommandbar(catalog_val, 
	    partid_val, revision_val);
	 }
         break;
  ...
  //  char *catalog_val, *partid_val, *revision_val;
	 ...
	 strcpy(revision_val, refresh->rev_revision);
	...
	//status = PDMQueryforMdcExists(catalog_val, partid_val, revision_val);
	status = PDMInfoOnCommandbar(catalog_val, partid_val, revision_val);
    ...
    case LIST_MCF:     /* Member of group MCF_LIST_GRP  */
	 ...
	 PDUset_edit_assy_toggle_after_select();
         ...
         /*status = PDMQueryforMdcExists(refresh->rev_catalog, 
	 			refresh->rev_partid, refresh->rev_revision);*/
		  status = PDMInfoOnCommandbar(refresh->rev_catalog, 
	 			refresh->rev_partid, refresh->rev_revision);
         break;
...
*/
    ...
    case LIST_PARTS_IN_CAT_BTN:
	 ...
	 {
	     // SAUBY, added for JTSMP CR 4008
	     // try getting it off the form.
	     if (1) {
		 IGRchar buf[80];
		 *buf = 0;
		 FIg_get_text(forms.local_files_form_id,CATALOG_FLD,buf);
		 if (*buf != 0) PDUfill_in_string(&refresh->rev_catalog,buf);
	     }
...
*/
    ...
    case DESIGN:        
	 ...
	 /* call function to loop through the rows */
	   ...
	   if(!strcasecmp(w_flow, "Y"))
	   {
           _pdm_debug("before 3rd PDUactivate.");
           status = SrValidateforRet(refresh->rev_catalog, refresh->rev_partid,
                               refresh->rev_revision);
           if(status)
             break;
...
int PDU_FIf_display_form(Form form)
{
  long  sts = 1;
...
int PDUlocal_files_form()
{
  ...
  sts = PDU_FIf_display_form(forms.local_files_form_id);
  PDU_local_files_displayed = TRUE;
  PDU_command = 0;
  ...
  // SAUBY, added for JTSMP CR 4008
  PDUvd_GetLFVDrefreshData(forms.local_files_form_id);
...
char          type[5];
  ...
  if ((PDU_parts_list_bufr == NULL) || (PDU_parts_list_bufr->rows == 0))
    ...
    if (display == FALSE )
      {
      PDU_FIf_display_form(forms.local_files_form_id);
      }
    return(status);
    }

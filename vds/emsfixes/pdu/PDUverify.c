/* $Id: PDUverify.c,v 1.2 2001/09/07 18:41:46 jdsauby Exp $ */
/* -------------------------------------------------------------------
 * I/VDS
 *
 * File: vds/emsfixes/pdu/design/PDUverify.c
 *
 * Description:
 *
 * Dependencies:
 * For some reason, this is needed even though know changes are made from
 * the original.
 *
 * Revision History:
 *       $Log: PDUverify.c,v $
 *       Revision 1.2  2001/09/07 18:41:46  jdsauby
 *       took out some darn print statements
 *
 *       Revision 1.1  2001/09/07 18:31:07  jdsauby
...
int	PDUverify_login ()
  ...
  if ( PDU_form_id == NULL ) {
      ...
      FI_get_form_by_name("Part_id", &PDU_form_id);
  }
  ...
  //if (!PDUnullstr(user->username) && (PDU_form_id != NULL)) 
     {
  ...
  //else if (!PDUnullstr(user->username) && (PDU_form_id == NULL))
     {

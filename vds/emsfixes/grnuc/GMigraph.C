/* $Id: GMigraph.C,v 1.3 2001/01/13 00:23:05 hans Exp $  */

/* --------------------------------------------------------------------
 * I/VDS
 *
 * File:	vds/emsfixes/grnuc / GMigraph.I
 *
 * Description:
 *
 * Dependencies:
 ...
 * Revision History:
 *	$Log: GMigraph.C,v $
 *	Revision 1.3  2001/01/13 00:23:05  hans
 *	SP16 modifications
 *	
 *	Revision 1.2  2001/01/13 00:05:17  hans
 *	SP16 modifications
 *
 * Revision 1.1  2000/05/24  14:23:50  pinnacle
 * Created: vds/emsfixes/grnuc/GMigraph.C by jwfrosch for Service Pack
 *
 *
 * History:
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
/*                          INTERNAL function GMdump_the_graph                */
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void GMdump_the_graph (p_graph, nb_ordered)
  struct GMgraph   *p_graph;  /* IN    graph to dump     */
  int    nb_ordered;          /* IN    nb of elements already ordered */
{
  long    index1, maxParent = -1;
  char    sts[80];
  struct GRid    *p_object;   /* GRid of a temporary graph element            */
  struct GMelmt  *p_content;  /* Content of a temporary graph element         */

  __printf ( "\n********** nb of elements in graph %d  with extern %d *********",
             `p_graph->nb_elmt, p_graph->p_hcode->cur_elmt` );

  // Then the array of elements and their status.
  for ( index1=0; index1<p_graph->p_hcode->cur_elmt; index1++ )
  {
    sts[0] = '\0';
    p_object = (struct GRid *) GMh_key(p_graph->p_hcode,index1);
    p_content = (struct GMelmt *) GMh_content(p_graph->p_hcode,index1);
    if (p_content->int_attr & GM_EXTERN) strcat ( sts, "\tEXTERN");
    if (p_content->int_attr & GM_ROOT)   strcat ( sts, "\tROOT");
    if (p_content->int_attr & GM_LOOP)   strcat ( sts, "\tLOOP");
    if (p_content->int_attr & GM_TIP)    strcat ( sts, "\tTIP");
    if (p_content->int_attr & GM_BODY)   strcat ( sts, "\tBODY");
    if (p_content->int_attr & GM_LEAF)   strcat ( sts, "\tLEAF");
    if (p_content->int_attr & GM_INIT)   strcat ( sts, "\tINIT");

    __printf ( "%4d\t[%d,%d]\t[%d,%d]\t%6d%s",
               `index1, p_object->osnum, p_object->objid,
               p_content->ind_parent, p_content->nb_parent,
               p_content->state, sts` );

    if (p_content->nb_parent)
      maxParent = p_content->ind_parent + p_content->nb_parent;
  }

  // Then the list of args.
  __printf ( "\n********** nb of args %d size of args %d *********", `maxParent, p_graph->s_lparent` );
  for ( index1=0; index1<maxParent; index1++ )
    __printf ( "%4d\t%d", `index1, p_graph->l_parent[index1]` );
  __printf ( "\n********** %d have been ordered  **********",`nb_ordered` );

  for ( index1=0; index1<nb_ordered; index1++ )
    __printf ( "%4d\t%d",` index1, p_graph->l_ord_elmt[index1]`);
  __printf ( "%s",`"**********************************************"`);
...
/*                              GLOBAL VARIABLE                               */
/*                                                                            */
/* This variable is defined in file: emsfixes/bs/VDdbg_glob.c in order        */
/* to prevent conflicts when dynamic loading (for debugging) is required      */
/*                   refer to TR 179901121 & 179901009                        */
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

extern int toggle_GMloop;    // Toggle can be set/unset by PPL: ci=toggle_loop
  ...
  /*.GMordinate */
{
  ...
  static int control_indx;

  p_hcode = p_graph->p_hcode;

  __enterFunction ( name = "GMordinate", argfmt = "control_indx = %d, p_hcode->cur_elmt = %d",
                    args = `control_indx, p_hcode->cur_elmt` );
  ...
  for(; index0 < p_hcode->cur_elmt;)
  {
    ...
    for(index1=0; index1 < p_graph->nb_elmt; index1++)
    {
      if(GMcompute_level((struct GMelmt *)GMh_content(p_hcode,index1),p_graph))
      {
        ...
        __printf("Add at index %d element[%d] = [%d,%d]",`index0, index1, ((struct GRid *)GMh_key(p_hcode,index1))->osnum, ((struct GRid *)GMh_key(p_hcode,index1))->objid`);
    ...
    __printf("loop for index0 = %d < p_hcode->cur_elmt = %d, control_indx = %d",`index0, p_hcode->cur_elmt, control_indx`);
    if ( ++control_indx == p_hcode->cur_elmt )
      break;

    if(toggle_GMloop == FALSE && index2 == index0)
    {
      // there is a loop in the graph. Break it where it has the chance to break the maximum number of loops
      // May be the loop has been found already and was made of 2 neested loop. This is to break the inner
      ...
      for(index1=0; index1 < p_graph->nb_elmt; index1++)
      {
        ...
        if((p_content->state == GM_UNKNOWN) && (p_content->int_attr & GM_LOOP) && (p_content->nb_parent > parent_count))
        {
          where_to_break = index1;
          parent_count = p_content->nb_parent;
        }
      }
      ...
      if(where_to_break < 0)
      {
        if (VDcomment)
        {
          GMdump_the_graph(p_graph,index0);
        }

        for(index1=0; index1 < p_graph->nb_elmt; index1++)
        {
          p_content = (struct GMelmt *)GMh_content(p_hcode,index1);
          if( p_content->state == GM_UNKNOWN )
          {
            // Since the element is not ordered, there MUST BE a loop in its parent
            p_object = (struct GRid *)GMh_key(p_hcode,index1);
            status = GMfind_loop(p_object, p_graph, &nb_elem_in_loop);
            break;
          }/* if(p_content->state == GM_UNKNOWN)) */

        }/* for(index1)*/

        // Ordering graph fails. But we can not find a loop : internal error !!!
        if (!nb_elem_in_loop) return GM_E_UNK_ERRCODE;

        // Break the loop somewhere
        for(index1=0; index1 < p_graph->nb_elmt; index1++)
        {
          p_content = (struct GMelmt *)GMh_content(p_hcode,index1);
          if((p_content->state == GM_UNKNOWN) && (p_content->int_attr & GM_LOOP) && (p_content->nb_parent > parent_count))
        ...
        // Ordering graph fails. But we can not find a loop : internal error !!!
        if (where_to_break <0 ) return GM_E_UNK_ERRCODE;
      }

      // That's where the loop is broken
      p_content = (struct GMelmt *) GMh_content(p_graph->p_hcode,where_to_break);
      p_object  = (struct GRid   *) GMh_key    (p_graph->p_hcode,where_to_break);
      p_content->state = GM_MIN_LEVEL;
      p_graph->l_ord_elmt[index0] = where_to_break;
      index0++;
      __printf("breaking the loop with elem[%d] = [%d , %d]",`where_to_break,p_object->osnum, p_object->objid`);

    }/* if(index2) */
  }/* for(;index0)*/

  __exitFunction ( name = "GMordinate", argfmt = "control_indx = %d, p_hcode->cur_elmt = %d, index0 = %d",
                    args = `control_indx, p_hcode->cur_elmt, index0` );
  control_indx = 0;    // Reset to zero
  return(status);
}/* GMordinate */

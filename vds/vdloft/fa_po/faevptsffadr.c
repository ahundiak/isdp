...
%SC%
     ...
     FAevPtSfFaDr( idim, sfUnw, pars, ptDr, vecDr, u, v, ptSol, &rc )
-----
%EN%
...
void FAevPtSfFaDr( idim, sfPols, sfUnw, pars, ptDr, vecDr, u, v, ptSol, rc )

IGRshort	idim;   /*I dimension of pars 2D,3D	*/
IGRdouble	*sfPols; /*I poles before unwrap         */
...
IGRdouble	*ptDr;  /*I point to project */
IGRdouble	*vecDr; /*I vector of projection */
...
cnt0:
	...
	jend = cv.num_knots - 1;
	for(j=1;j<jend;j++){
		if( cv.knots[j] > umax ){ i = j;  break; }
	}

        j = (i-2)*idim;

	/*
	 *  test if index belongs to the first row 
	 */
	...
	/*
	 * test if index belongs to the last colum 
	 */
	...
	/*
	 * test if index belongs to the last row 
	 */
...
eval0:
	...
	/*d
	printf("Boundary case  j = %d, quotient = %d, rest = %d\n",
		j,(j/offset),(j%offset));
	d*/
...
eval:
	...
	/*d	
	printf(" eval u = %16.14f, v  = %16.14f, fastat=%d\n",u,v,fastat);
	printf("     us = %16.14f, vs = %16.14f\n",us,vs);
	printf(" eval j = %d, quotient = %d, rest = %d\n",
		 j,(j/offset),(j%offset));
	d*/
	...
	FAintDrFa( P00, P01, P10, P11, ptDr, vecDr, ptSol, uv, rc );
	/*d
	printf(" uvSol = %lf,%lf,%lf\n",uv[0],uv[1],uv[2]);
	printf(" ptsol = %lf,%lf,%lf\n",ptSol[0],ptSol[1],ptSol[2]);
	d*/
	...
	FAev( idim, P00, P01, P10, P11, uv[0], uv[1], ptSol, rc );

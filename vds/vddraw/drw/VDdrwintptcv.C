...
extern long	VD_allocForBSmerge_cv() ;
...
/*----------------------------------------------------------------------------*/
#ifdef vdsDEBUG
static void VD_printMesh( comment, m )
...
#endif
/*----------------------------------------------------------------------------*/
static void VD_countNeighbor( m_in, m_nei )
...
/*----------------------------------------------------------------------------*/
static void VD_findInteriorPointUsingMesh( mesh, i_interior, j_interior )
...
int	*i_interior,
	*j_interior ; {
	...
	VD_countNeighbor( mesh, neighbor ) ;

#ifdef vdsDEBUG
	VD_printMesh( "Mesh of neighbor", neighbor ) ;
#endif
	...
	if( NB_PTS_ALONG_Y == i ) {
		...
		VD_findInteriorPointUsingMesh( neighbor,
					      i_interior,
					      j_interior ) ;
	}

} /* VD_findInteriorPointUsingMesh */
/*----------------------------------------------------------------------------*/
long VD_findTheMostInteriorPointToACurve( msg, cvGeom, point )
...
IGRpoint		point ; {
	...
	SetProc( VD_findTheMostInteriorPointToACurve ); Begin
	...
	if( vd_$is_curve_linear( msg = msg, curveGeom = cvGeom ) ) {
		...
		if( BSERROR( bsRC ) ) vd_$error( msgval = EMS_E_BSerror ) ;

		BScveval( cvGeom, 1., 0, (IGRpoint *) ptAt1, &bsRC ) ;
		if( BSERROR( bsRC ) ) vd_$error( msgval = EMS_E_BSerror ) ;

		VD_midpoint( ptAt0, ptAt1, point ) ;
	...
	if( !cvGeom->phy_closed ) {
		 ...
		 */
		sts = VD_allocForBSmerge_cv( msg, cvGeom, lnGeom, &wkGeom ) ;
...
#ifdef vdsDEBUG
	VD_printMesh( "Mesh of closed curve", mesh ) ;
#endif
	...
	VD_findInteriorPointUsingMesh( mesh, &i, &j ) ;

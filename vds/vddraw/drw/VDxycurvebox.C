...
#define _Z	2
/*----------------------------------------------------------------------------*/
VD_directionType VD_getDirType( dirVector ) IGRvector dirVector ; {
	/*
	...
	BSnorvec( &bsRC, VD_vccpy( dirUnit, dirVector ) ) ;

	return dirUnit[_X] >= COS45 || dirUnit[_X] <= COS135 ? VD_horizontal
							     : VD_vertical ;
} /* VD_getDirType */
/*----------------------------------------------------------------------------*/
long VD_findBoxOfCvInXYPlane( msg, cvGeom, direction, bx )
...
} /* VD_findBoxOfCvInXYPlane */
/*----------------------------------------------------------------------------*/
void VD_getJustPntsOfBlk( block, direction, justPnts )

IGRpoint	block[4] ;	/* IN : Box points of block	*/
IGRvector	direction ;	/* IN : Direction of block	*/
VD_blkJustPnts	*justPnts ; {	/* OUT: Justification points	*/
...
/*----------------------------------------------------------------------------*/
long VD_findEndPtsCvInXYPlane( msg, cvGeom, cvDir,
				endPt1, endPt2, endTn1, endTn2, swapped )

long			*msg ;		/* OUT: Completion code		*/
struct IGRbsp_curve	*cvGeom ;	/* IN : Geometry of curve	*/
VD_directionType		cvDir ;		/* IN : Type of curve direction	*/
...
#define _2ND	1
	...
	*swapped = FALSE ;
	...
	if(    vd_$is_curve_linear( msg = msg, curveGeom = cvGeom )
	    && VD_eqPoints( cvGeom->poles + 0, cvGeom->poles + 3 ) ) {
		/*
		 * It's a point: put two horizontal and opposite tangents.
		 */

		VD_vccpy( endPt2, VD_vccpy( endPt1, cvGeom->poles + 0 ) ) ;
		...
		VD_vccpy( endPt2, VD_vccpy( endPt1, evl[_1ST][PNT] ) ) ;
		BSnorvec( msg, VD_vccpy( endTn2, evl[_1ST][TAN] ) ) ;
		VD_negvc( endTn2, endTn1 ) ;
		...
		VD_vccpy( endPt1, evl[i1][PNT] ) ;
		VD_vccpy( endPt2, evl[i2][PNT] ) ;
		BSnorvec( msg, VD_vccpy( endTn1, evl[i1][TAN] ) ) ;
		BSnorvec( msg, VD_vccpy( endTn2, evl[i2][TAN] ) ) ;

		if( _1ST == i1 ) {
			VD_negvc( endTn1, endTn1 ) ;
		} else {
			VD_negvc( endTn2, endTn2 ) ;
		}
		*swapped = _2ND == i1 ;
	}
...
/*----------------------------------------------------------------------------*/
long VD_findMidPtCvInXYPlane( msg, cvGeom, midPt, midTn )
...
IGRvector		midTn ; {	/* OUT: Tangent at mid point	*/
	...
	if(    vd_$is_curve_linear( msg = msg, curveGeom = cvGeom )
		 ...
		 */
		VD_vccpy( midPt, cvGeom->poles + 0 ) ;
		midTn[_X] = 1 ; midTn[_Y] = midTn[_Z] = 0 ;
	} else {
		BScveval( cvGeom, 0.5, 1, (IGRpoint *) evl, msg ) ;

		VD_vccpy( midPt, evl[PNT] ) ;
		BSnorvec( msg, VD_vccpy( midTn, evl[TAN] ) ) ;

		if( evl[TAN][_Y] < 0 ) VD_negvc( midTn, midTn ) ;
	}

	*msg = MSSUCC ;
	return *msg & 1 ;

} /* VD_findMidPtCvInXYPlane */
/*----------------------------------------------------------------------------*/

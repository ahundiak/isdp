/* $Id: VDct1TreeF.c,v 1.2 2002/02/26 18:19:08 ahundiak Exp $  */
 ...
 * Revision History:
 *      $Log: VDct1TreeF.c,v $
 *      Revision 1.2  2002/02/26 18:19:08  ahundiak
 *      ah
 *
...
static IGRstat VDct1FormContractTreeRow  (Form form, IGRint gadget, IGRint row);
...
IGRstat VDct1FormUpdateTreeRow(Form form, IGRint gadget,TGRid *rowID)
{
  ...
  VDctxGetNthTreeChild(NULL,rowID,0,&childID); 
				    ...
				    IGRint  expFlag)
{
  ...
  VDctxGetBaseInfoID(nodeCP,nodeID,&baseInfo);
  strcpy(pnt,baseInfo.baseName);
  ...
  VDctxGetNthTreeChild(nodeCP,nodeID,0,&childID); 
  ...
  // Need current parent
  //VDctxGetTreeParent(NULL,nodeID,&parentID);
...
IGRstat VDct1FormFillTree(Form form, IGRint gadget, TGRid *setID)
{
  ...
  VDctxGetTree(NULL,setID,&treeID);
  ...
  // Put out next level
  for(i = 0; VDctxGetNthTreeChild(NULL,&treeID,i,&nodeID); i++) {
...
static IGRstat VDct1FormExpandTreeAttrRow(Form form, IGRint gadget, IGRint row)
{
  ...
  VDctxGetFldAtrsID(NULL,&rootID,&cnt,flds);
  for(i = 0; i < cnt; i++) {
...
static IGRstat VDct1FormExpandTreeRow(Form form, IGRint gadget, IGRint row)
{
  ...
  level++;
  row++;
  for(i = 0; VDctxGetNthTreeChild(rootCP,&rootID,i,&nodeID); i++) {
				       ...
				       TGRid *treeID)
{
  ...
  VDctxDisplayNode(NULL,treeID,mode,1);
 ...
 */
static IGRstat VDct1FormFillSetRow(Form form, IGRint gadget, TGRid *setID)
{
  ...
  // Set type and name
  VDctxGetBaseInfoID(NULL,setID,&baseInfo);
 ...
 */
static IGRstat VDct1FormFillSets(Form form, IGRint gadget, IGRint clearGadget)
{
  ...
  for(osnum = 0; VDosValidate(osnum); osnum++) {
    ...
    if (mgrID.objid != NULL_OBJID) {
      ...
      // Cycle through sets
      for(i = 0; VDctxGetNthMgrChild(NULL,&mgrID,i,&setID); i++) {
	  
	// Get the real tree
	VDctxGetNthMgrChild(NULL,&setID,0,&treeID);
  ...
  // Do the dom as well
  VDdomGetMgr(NULL_OSNUM,0,&mgrID);
...
IGRstat VDct1FormUpdateVRTree(Form form, IGRint gadget)
{
  ...
  for (row = 0; row < rows; row++) {
    ...
    // Get the calss pointer
    rowCP = VDctxGetNodeCP(&rowID);
    VDASSERTW(rowCP);
    
    // get the attributes for that field
    VDctxGetFldAtrID(rowCP,&rowID,"frozen",&tmpFld);
    strcpy(buf,"");
    if (*tmpFld.name != 0) {
      strcpy(buf1,tmpFld.val.txt);
      VDctxGetFldAtrID(rowCP,&rowID,"assy_rev",&tmpFld);
    ...
    VDctxGetFldAtrID(rowCP,&rowID,"node_count",&tmpFld);
    strcpy(buf,"");
    if (*tmpFld.name != 0) {

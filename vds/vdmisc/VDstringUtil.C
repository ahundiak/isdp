/* $Id: VDstringUtil.C,v 1.1.1.1 2001/01/04 21:08:57 cvs Exp $  */

/*************************************************************************
 * I/VDS
 *
 * File:	vdmisc / VDstringUtil.C
 *
 * Description:
 *	string utilities
 *
 ...
 *
 * Revision 1.2  1995/09/16  23:45:08  pinnacle
 * Replaced: vdmisc/VDstringUtil.C for:  by tlbriggs for vds.240
 *
 *
 * History:
 *	09/15/95	tlb	Added VD_strncat, use _MALLOC from VDmem.h
 *				Truncated fixed in VD_strncpy
 *
 *************************************************************************/
#include <stdio.h>
#include <string.h>
...
/*----------------------------------------------------------------------------*/
char *VD_strdup( string ) const char *string ; {
...
char *
VD_strncat( 	char 		*toStr, 
		const char 	*fromStr, 
		register int	len, 
		register int 	*truncated )
{
	int	rem, from_len, nchars;
	...
	/* find number of chars to copy (excluding null) */
	from_len = strlen(fromStr);
	rem = len - strlen(toStr) - 1 ;
	nchars = (rem < from_len) ? rem : from_len;
	*truncated = (nchars != from_len);

	if (nchars > 0)
		strncat( toStr, fromStr, nchars) ;
wrapup:
	return toStr ;

} /* VD_strncat */
/*----------------------------------------------------------------------------*/
void VD_dumpString( s ) const char *s ; {
	int i, j ;
...
/*----------------------------------------------------------------------------*/
char *VD_stripString( s ) register char *s ; {
...
/*----------------------------------------------------------------------------*/
char *VD_abbrvString( inString, nchars, outString )
...
/*----------------------------------------------------------------------------*/
char *VD_deleteWordAtIndex( string, index, word )
...
} /* VD_deleteWordAtIndex */
/*----------------------------------------------------------------------------*/
char *VD_toLOWER_Str( sto, sfrom ) char *sto ; const char *sfrom ; {
...
/*----------------------------------------------------------------------------*/
char *VD_toLowerStr( s ) const char *s ; {
...
/*----------------------------------------------------------------------------*/
int VD_strcmpLOWER( s1, s2 ) const char *s1, *s2 ; {
...
*/
	...
	rc = strcmp( S1 = VD_toLowerStr( s1 ), S2 = VD_toLowerStr( s2 ) ) ;
	_FREE( S1 ) ; _FREE( S2 ) ;
...
/*----------------------------------------------------------------------------*/
int VD_find_string_in_list( name, list, size, p_index )
...
} /* VD_find_string_in_list */
/*----------------------------------------------------------------------------*/
int VD_strcmpByPtrs( A, B )
...
const void	*A,
	...
	char	**a	= (char **) A,
		**b	= (char **) B ;
	/*
	 * Support comparison function for `VD_remove_dups_in_list'.
	 */
	return strcmp( *a, *b ) ;
}
/*----------------------------------------------------------------------------*/
int VD_remove_dups_in_list( msg, oldsize, oldlist, newsize, newlist )
...
*/
	...
	*newlist = NULL ; *newsize = 0 ;
	...
	if( ( sortedList = _MALLOC( oldsize, char * ) ) ) {
		...
		qsort( sortedList, oldsize, sizeof *sortedList, VD_strcmpByPtrs);
...
} /* VD_remove_dups_in_list */
/*----------------------------------------------------------------------------*/
int VD_find_item_in_array( item, array, size, p_index )
...
} /* VD_find_item_in_array */
/*----------------------------------------------------------------------------*/
char **VD_allocList( nbStrings, stringSize )
...
/*----------------------------------------------------------------------------*/
char **VD_copyList( msg, inList, size, outList )
...
*/
	*msg = MSSUCC ;
	if( *outList = _CALLOC( size, char * ) ) {
		int i ;
		for( i = 0 ; i < size ; i++ ) {
			if( !( (*outList)[i] = VD_strdup( inList[i] ) ) ) {
				*msg = EMS_E_NoDynamicMemory ;
				break ;
			}
		}
...
/*----------------------------------------------------------------------------*/
int VD_wordcount( string ) const char *string ; {
...
/*----------------------------------------------------------------------------*/
char *VD_getword( string, wordno, wordsize, word )
...
/*----------------------------------------------------------------------------*/
char *VD_substring( instr, from, to, outsize, outstr )
...
/*----------------------------------------------------------------------------*/
int VD_substituteString( instr, torep, bystr, times, outsz, outstr )
...
/*
	...
	1) Declare a static array of type "VD_str2index", either with a "static"
	...
	3) Invoke "VD_initStrLink" which will return a fast-search structure
	pointer pointing to memory allocated by "VD_initStrLink", NULL will
	be returned in case of memory allocation failure :

		VD_strLink **ptr ;

		ptr = VD_initStrLink( array ) ;
		if( !ptr ) {
			(* Allocation failed *)
		}

	4) Search strings with "VD_strLinkLookUp", which will return the
	associated index if string has been found and "-1" if the string
	was not found.

		if( VD_strLinkLookUp( "string2", ptr ) != -1 ) {
			(* Found string *)
		}

	5) When the fast-search structure is no longer needed, deallocate the
	memory with "VD_strLinkFree" :
	...
	int yourFunction( string ) char *string ; {
		...
		index = VD_strLinkLookUp( string, ptr ) ;
	...
	preferable.
*/
#define VD_K_MAX_ASCII_CHARS	128

VD_strLink **VD_initStrLink( list ) VD_str2index *list ; {

	register VD_strLink	**lkArray ;
	VD_strLink		*c,
				*next,
				*new ;
	register VD_str2index	*l ;
	register char		first ;

	if( !( lkArray = (VD_strLink **) calloc( VD_K_MAX_ASCII_CHARS,
						sizeof *lkArray ) ) )
			return NULL ;

	for( l = list ; ( first = *l->string ) ; l++ ) {
		if( !( new = (VD_strLink *) malloc( sizeof *new ) ) ) {
		}
		new->entry = l ;
		if( c = lkArray[first] ) {
			next		= c->next ;
...
/*----------------------------------------------------------------------------*/
int VD_strLinkLookUp( list, s ) register VD_strLink **list ; register char *s ; {

	register VD_strLink *c ;
...
/*----------------------------------------------------------------------------*/
void VD_strLinkFree( list ) register VD_strLink **list ; {

	register int		i ;
	register VD_strLink	*p,
				*n ;
	if( list ) {
		for( i = 0 ; i < VD_K_MAX_ASCII_CHARS ; i++ ) {
			p = list[i] ;
			while( p ) {
				n	= p->next ;
				free( p ) ;

...
#include "bsmkvec.h"
#include "bsrev_cv.h"
#include "v_sortsubsfs.h"
#include "v_spcvkp1res.h"
#include "v_vecmxproto.h"
/*----------------------------------------------------------------------------*/
int VD_splitCvKeepOneResult( msg, cvToSplit, splitPnt, whichToKeep, result )
...
} /* VD_splitCvKeepOneResult */
/*----------------------------------------------------------------------------*/
double VD_farthestSfPoleFromLnEndPt( surface, lineseg, whichEnd, farPolePrj )
...
} /* VD_farthestSfPoleFromLnEndPt */
/*----------------------------------------------------------------------------*/
static int VD_mk_portion_of_ln_through_sf( msg, lineseg, surface, whichEnd,
					  p_throughPortion )
long			*msg ;
struct IGRbsp_curve	*lineseg,
			**p_throughPortion ;
...
} /* VD_mk_portion_of_ln_through_sf */
/*----------------------------------------------------------------------------*/
static int VD_mk_portion_of_bc_through_sf( msg, curve, surface, whichEnd,
					  p_throughPortion )
long			*msg ;
...
struct IGRbsp_surface	*surface ; {
	...
	*p_throughPortion = NULL ;
	...
	vd_$extend_curve_through_surface(msg		= msg,
					curveGeom	= curve,
					surfaceGeom	= surface,
					whichEnd	= whichEnd,
...
} /* VD_mk_portion_of_bc_through_sf */
/*----------------------------------------------------------------------------*/
int VD_mk_portion_of_cv_through_sf( msg, curve, surface, whichEnd,
					p_throughPortion )
...
Abstract :
	...
	surface.
	This function interfaced by macro vd_$mk_portion_of_cv_through_sf
*/
...
struct IGRbsp_surface	*surface ; {
	...
	if( vd_$is_curve_linear( msg = msg, curveGeom = curve ) ) {
		VD_mk_portion_of_ln_through_sf(	msg,
						curve,
						surface,
						whichEnd,
						p_throughPortion ) ;
	} else {
		VD_mk_portion_of_bc_through_sf(	msg,
						curve,
						surface,
						whichEnd,
						p_throughPortion ) ;
...
} /* VD_mk_portion_of_cv_through_sf */
/*----------------------------------------------------------------------------*/

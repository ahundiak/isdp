/* $Id: SMCmdTrBk.sl,v 1.1.1.1 2001/01/04 21:07:38 cvs Exp $  */

/* --------------------------------------------------------------------
 * I/VDS
 *
 * File:	vds/smsurf/cmd / SMCmdTrBk.sl
 *
 * Description:
 *
 * Dependencies:
...
/*    File          SMCmdTrBk.sl
      Description   Command object to place a Transversal Bulkhead
      Auteur        Pascale Lacroix (ISDC) 
      Date          Oct-92     
*/

command_string       SMC_M_PlStTrBH,0,SMPlStTrBH
synonym		     SMC_M_MdStTrBH,1,SMMdStTrBH
command_table        "sman.cmd"
class                SMCmdTrBk
super_class          SMCmdSurf
product_name         "$VDS"

form "SMTransBk",0,0,"-1","-1"
...
state start
  ...
  on CMD_TYPE = PLACE	do message_key SM_M_PlStTrBH
                        do display_form  0
  			state modif_form 
  				
  on CMD_TYPE = MODIFY	do message_key SM_M_MdStTrBH
				state loc_macro
...
state loc_macro
  prompt_key         SM_P_IdTrBH
  accept_key        SM_P_PlAccRej
  locate_class	     "SMTransBk"
...
/*.init_all*/
{
 ...
 if(me->mytype == MODIFY) 
		     FIg_set_text(me->forms[0].form_ptr,F_TITLE,
					"Modify Transversal Bulkhead");

 else{
	(void)GetDefName("SMTransBk_0",me->def_name);

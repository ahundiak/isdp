...
int PDMrlist_local_files(buffer)
  MEMptr        *buffer;
  {
...
int PDMlist_checkedout_files(buffer)
  MEMptr        *buffer;
  {
  int  status = PDM_S_SUCCESS;
...
int PDMlist_checkedin_files(buffer)
  MEMptr        *buffer;
  {
  int  status = PDM_S_SUCCESS;
...
int PDMrcheck_part_status(catalog, part, revision)
  char          *catalog;
  char          *part;
  char          *revision;
  {
  _pdm_debug("In the function PDMrcheck_part_status",0);
  ...
  _pdm_debug("calling PDMcheck_part_status", 0);
  status = PDMcheck_part_status();
  _pdm_status("PDMcheck_part_status", status);

  /* Display Message */
  PDUmessage(PDM_E_CLEAR_FIELD, 's');
  
  _pdm_debug ("PDMrcheck_part_status: returning SUCCESS", 0);
  return (status);

...
/*
**	Purpose:
**		Dialog procedure for "Destination Directory" dialog.
**
**	Controls Recognized:
**		Pushbutton - IDC_B, IDC_B1, IDC_C, IDC_H, IDC_X
**
**	Initialization Symbols:
**		DirName : The name of the "default" destination directory.
**
**	Termination Symbols:
**		"DLGEVENT" - one of the following, according to control event:
**				event     value
...
**
*****************************************************************************/
BOOL FAR PASCAL FDirDlgProc(HWND hdlg, WORD wMsg, WORD wParam, LONG lParam)
{
	WORD cb;
	CHAR szSymBuf[cbSymBuf];
	...
	case STF_REINITDIALOG:
	case WM_INITDIALOG:
		cb = CbGetSymbolValue("DirName", szSymBuf, cbSymBuf);
		Assert(cb < cbSymBuf);
		SetDlgItemText(hdlg, IDC_TEXT1, szSymBuf);
		return(fTrue);
	...
	case WM_COMMAND:
		...
		case IDC_B1:
			if (!FSetSymbolValue("DLGEVENT", "CHANGE"))
				{
				DestroyWindow(GetParent(hdlg));
				return(fTrue);
				}
...
/*
**	Purpose:
**		Information Dialog procedure, with one
**		text item which can be set from the Setup program.
**
**	Controls Recognized:
**		Pushbutton - IDC_B, IDC_C, IDC_H, IDC_X
**
**	Initialization Symbols:
**		"Text1In" - String to be placed into IDC_TEXT1
**
**	Termination Symbols:
**		"DLGEVENT" - one of the following, according to control event:
**				event     value
...
**
*****************************************************************************/
BOOL FAR PASCAL FInfo1DlgProc(HWND hdlg, WORD wMsg, WORD wParam, LONG lParam)
{
	WORD	cb;
	CHAR	szSymBuf[cbSymBuf];
	...
	switch (wMsg)
		{
	case WM_INITDIALOG:
		cb = CbGetSymbolValue("Text1In", szSymBuf, cbSymBuf);
		Assert(cb < cbSymBuf);
		SetDlgItemText(hdlg, IDC_TEXT1, szSymBuf);
		return(fTrue);
...
**	Purpose:
**		Radio Button Group Dialog procedure for templates with one group
**		of one to ten radio button controls, and two text fields.
**
**	Controls Recognized:
**		Radio      - IDC_B1 to IDC_B10 (sequential)
**		Pushbutton - IDC_B, IDC_C, IDC_H, IDC_X
...
**			to be initialized as disabled.  Indexes not in the list will
**			be left enabled.
**		"Text1In" - String to be displayed in text field 1
**		"Text2In" - String to be displayed in text field 2
**
**	Termination Symbols:
**		"ButtonChecked" - index of currently selected radio button.
**		"DLGEVENT"      - one of the following, depending on event:
...
BOOL FAR PASCAL FRadioDlgProc(HWND hdlg, WORD wMsg, WORD wParam, LONG lParam)
{
	...
	case WM_INITDIALOG:
		...
		cb = CbGetSymbolValue("Text2In", szSymBuf, cbSymBuf);
		Assert(cb < cbSymBuf);
		SetDlgItemText(hdlg, IDC_TEXT2, szSymBuf);

		cb = CbGetSymbolValue("Text3In", szSymBuf, cbSymBuf);
		Assert(cb < cbSymBuf);
		SetDlgItemText(hdlg, IDC_TEXT3, szSymBuf);

		cb = CbGetSymbolValue("Text4In", szSymBuf, cbSymBuf);
		Assert(cb < cbSymBuf);
		SetDlgItemText(hdlg, IDC_TEXT4, szSymBuf);
	...
	case WM_COMMAND:
		...
		case IDC_B9:
		case IDC_B10:
			CheckRadioButton(hdlg, IDC_B1, IDC_B10, LOWORD(wParam));
			if (HIWORD(wParam) != BN_DOUBLECLICKED)
				break;
			else
				LOWORD(wParam) = IDC_C;
...
**		(Limits the input string length to cbNameBuf characters.)
**
**	Controls Recognized:
...
BOOL FAR PASCAL FNameOrgDlgProc(HWND hdlg, WORD wMsg, WORD wParam, LONG lParam)
{
	...
	char  rgchText[cbNameBuf + 1];

	switch (wMsg)
		{
	case WM_INITDIALOG:
		SendDlgItemMessage(hdlg, IDC_EDIT, EM_LIMITTEXT, cbNameBuf, 0L);
		SetDlgItemText(hdlg, IDC_EDIT, (LPSTR)"");

		SendDlgItemMessage(hdlg, IDC_EDIT2, EM_LIMITTEXT, cbNameBuf, 0L);
	...
	case WM_COMMAND:
		...
		case IDCANCEL:
			...
			SendDlgItemMessage(hdlg, IDC_EDIT, (WORD)WM_GETTEXT,
					cbNameBuf + 1, (LONG)((LPSTR)rgchText));
...
/*
**	Purpose:
**		Initialization/termination routine for DLL.
**	Arguments:
**		hDLL	     handle of the DLL
**		dwReason     why the Entry point is being called
**		lpReserved   reserved parameter
**	Returns:
**		FALSE if CRT initialization/termination failed
**		TRUE oherwize
**
***************************************************************************/
_dt_hidden BOOL APIENTRY LibMain(HINSTANCE hDLL, DWORD dwReason, LPVOID lpReserved)
{
...
**
*****************************************************************************/
BOOL FAR PASCAL FPrereqDlgProc(HWND hdlg, WORD wMsg, WORD wParam, LONG lParam)
{
    WORD cb, i, cItems;
    CHAR szMissing[256];
    CHAR szAllMissing[1024];

    switch (wMsg)
      {
        case WM_INITDIALOG:
        strcpy(szAllMissing, "");
        cItems = UsGetListLength("PreReqText");
        for (i = 1; i <= cItems; ++i)
          {
            cb = CbGetListItem("PreReqText",i,szMissing,256);
            Assert(cb < 256);
            strcat(szAllMissing,szMissing);
            if ( i < cItems ) strcat(szAllMissing,"; ");
        }
        SetDlgItemText(hdlg, IDC_PRE, szAllMissing);
        return(fTrue);
...
**
*****************************************************************************/
BOOL FAR PASCAL FInfoDlgProc2(HWND hdlg, WORD wMsg, WORD wParam, LONG lParam)
{
	char  szSymBuf[cbSymBuf];
	...
	switch (wMsg)
		{
	case WM_INITDIALOG:
		CbGetListItem("OldVersion", 1, szSymBuf, cbSymBuf);
		SetDlgItemText(hdlg, IDC_OLDVER, szSymBuf);
		return(fTrue);
...
/*
**	Purpose:
**		Information Dialog with dynamic text procedure.
**
**	Controls Recognized:
**		Text       - IDC_OLDVER 
**		Pushbutton - IDC_B, IDC_C, IDC_H, IDC_X
...
**
*****************************************************************************/
BOOL FAR PASCAL FConfirmRemVerProc(HWND hdlg, WORD wMsg, WORD wParam, LONG lParam)
{
	char  szSymBuf[cbSymBuf];
	...
	switch (wMsg)
	{
		case WM_INITDIALOG:
			CbGetListItem("OldVersion", 1, szSymBuf, cbSymBuf);
			SetDlgItemText(hdlg, IDC_OLDVER, szSymBuf);
			CbGetListItem("OldVersion", 2, szSymBuf, cbSymBuf);
			SetDlgItemText(hdlg, IDC_OLDPATH, szSymBuf);
		return(fTrue);

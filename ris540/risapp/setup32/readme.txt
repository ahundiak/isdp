...
Version Information
-------------------
Product:        RISDP - RIS Development Platform for Windows NT and Windows 95
...
Description:    RIS (Relational Interface System) is a generic relational
                ...
                The development platform contains an embedded SQL
...
System Requirements
-------------------
 Following are the minimum system requirements for RISDP:
 ...
 * At least 13 megabytes of free disk space
...
-----------------
*** IMPORTANT NOTICE ***
    Installation on Windows 95 may fail if the Microsoft C Runtime DLL
    (msvcrt20.dll) is older than version 2.10. The error reported is:
        "The MSDETSTF.DLL file is linked to missing
         export MSVCRT20.DLL:_adjust_fdiv."
    Work-around: copy the 'msvcrt20.dll' from the RIS product
    delivery directory to the Windows 95 SYSTEM directory, or
    remove the 'msvcrt20' entry from the registry key
    "HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control\SessionManger\KnownDlls".
    This problem is under investigation.

To install the RIS Development Platform (RISDP) on a machine, run 'setup.exe'
through either File Manager, Explorer, or the MSDOS command prompt. When
running SETUP through the MSDOS command prompt, change the drive (for floppy
installation) or directory (for network installation) to where the 'setup.exe'
program is located.

The RIS Development Platform installation procedure is quite interactive.
Different menus will be displayed for various input and output information. 
Alternative choices will be prompted for whenever appropriate.

At any stage of installation, chose the CONTINUE button to proceed further
or CANCEL SETUP to quit the RISDP installation procedure.

If the setup procedure detects a previous version of RISDP installed in
the default directory "C:\Program Files\RISDP", or in the destination
directory specified, you will be prompted to remove the existing version.
Alternatively, RISDP can be installed under a different directory.

The installation will prompt for Registration Information. RISDP Setup 
procedure requires the following:
        Name of the User
        Name of the Company
        Serial Number

The RISDP is partitioned into five sections, namely:

1. Minimum section consists of basic RIS development platform to create
   applications using only RIS SQL statements or functions.

2. Forms section consists of RIS forms development platform to create
   applications using RIS forms functions.

3. Loader/Unloader section consists of RIS loader/unloader development 
   platform to create applications using RIS loader/unloader functions.

4. Sample programs for all the above sections. These samples were designed
   to be compiled using the 'nmake' utility.

5. Shared component packaging files for redelivery with a RIS based product.

Install any of the above sections or combination of these sections depending
upon your requirements. 

The Compact Installation consists of the Minimum section only. This section
is MANDATORY and therefore will always be installed on the system.

The Typical Installation consists of all sections.

The Custom installation allows particular sections to be selected and loaded.
...
SPECIAL NOTES: 
	...
	NT:	C:\WINNT\SYSTEM32
 	WIN95:	C:\WIN95\SYSTEM
      ...
      The RISDP installation will load the RIS Shared Component (RIS Client
      ...
      RISDP can now be installed using SMS.
------------------------
...
Getting Started
---------------
Assuming that RISDP was installed under "C:\Program Files\RISDP", it
may be helpful to update the following environment variables as shown below:

     INCLUDE=%INCLUDE%;C:\PROGRAM FILES\RISDP\INCLUDE
     LIB=%LIB%;C:\PROGRAM FILES\RISDP\LIB
     PATH=%PATH%;C:\PROGRAM FILES\RISDP\BIN

The directory for the RIS utilities is added to the PATH environment variable
during installation. If the PATH contains spaces, it is enclosed in quotes so
that it can be used by the command interpreter (CMD.EXE).

The RIS Preprocessor(riscpp) delivered with the RIS Version 5 Development
Platform REQUIRES the Microsoft Visual C++ Compiler 2.x.
...
will be stored under "<SystemDrive>:\win32app\ingr\share\" or
"<SystemDrive>:\Program Files\Common Files\Intergraph\", depending upon
the existing configuration.

NOTE: If this utility is executed to remove a RIS V4 Shared Component, the
      RIS V4 Client will be inactivated and a local RIS V4 Data Server can
      only be accessed by a REMOTE RIS V4 Client.
...
The "Configure RIS Version" utility should be executed by a member of the
...
During RIS Setup and various other utilities, RIS tries to remove existing
files and/or directories. If a file or directory is open, the RIS setup or
utility may fail. A process cannot delete a file/directory while another
process has it open.

If a remote schema file on an NT system will be used, that system must have
the FTP server installed and functioning. By default, the FTP server is not
loaded during normal Operating System setup. Refer to the OS documentation
or your system manager for installation details.
For more information on the configuration of the FTP Server available with
NTW or NTS 4.0, refer to the RIS Shared Component README file (located in
\Program Files\Common Files\Intergraph\ris05.04 or \win32app\ingr\ris05.04).
...
The following networking software constraints apply.

   PC-NFS networking software
      Version 5.1a is required.

   PC_TCP networking software
      Version 2.3: Need to up the MinimumCopySpace to at least 28
          in the SYSTEM.INI file.
      Version 3.0: Need to up the MinimumCopySpace to at least 28
          in the PCTCP.INI file.
---------------------
End of Win32s section
---------------------
...
Security Info
-------------
RISDP will run under a User account created with default privileges.
--------------------
...
o  RIS Orphan Processes
   ...
   The default wait time for an orphan process before it begins testing for
   whether the other end of the TCP/IP connection is dead is 2 hours.
   That means that after the RIS application or RIS client machine is
   re-booted, the orphan process waits for more than 2 hours before it goes
   away. The wait time can be changed and the procedure for this change is
   platform specific. 

   WARNING: The side effect of changing the wait time is that it affects ALL
   applications that use the TCP/IP KeepAlive feature. It is recommended
   that you NOT set the KeepAlive wait time to less than 30 minutes.
   ...
   the wait time for the Windows NT platform:
     ...
     The entry for the TCP/IP parameter appears under the following Registry
     path:
     ...
     To add a value, highlight the "Parameters" key, select "Add Value"
     from the "Edit" pulldown menu. Set the 'Value Name' and 'Data Type'
     accordingly.  Select the 'OK' button.  Set the 'Data' value.

     If the KeepAliveTime key already exists, the time-out value can be
     changed by double-clicking on the key and setting a new value in the
     dialog box.
...
o  Statements at application startup:
...
Open Problems
-------------
Following are known problems with the RISDP.

[TR #439600971]
    Installation on Windows 95 will fail if the MS C runtime DLL
    (msvcrt20.dll) is older than version 2.10.
    The error is: "The MSDETSTF.DLL file is linked to missing
     export MSVCRT20.DLL:_adjust_fdiv".
    Work-around: Remove the 'msvcrt20' entry from the registry key
    HKey_Local_Machine\System\CurrentControlSet\Control\SessionManger\KnownDlls
    or copy the DLL from the RIS product delivery directory to the Windows 95
    SYSTEM directory.


Following are known problems with the RIS Shared Component (Client 
and Utilities).

[TR # 439601440]
    RISMGR and RISGUI forms fields are not sized properly on NTJ 3.51.  
    Possible OS problem, pending external resolution.

[TR #439601492]
   Cannot create a schema or access the schema file after locating the schema 
   file to a remote NT system from a UNIX platform.

   Workaround: a schema file must exist, if not create a empty file.  On UNIX
   use DOS 8.3 notation for directory names containing spaces. 
   EX:
      c:/progra~1/common~1/interg~1/ris05.04/schemas 
    for the default location of
      c:\program files\common files\intergraph\ris05.04\schemas.
--------------------

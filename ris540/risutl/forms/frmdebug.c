...
{
	FRM_DBG(("clear_text_field()\n"));

	col = 0;
	row = 0;
	FIfld_set_num_rows(db_form, TEXT_FIELD, 0);

	FRM_DBG(("clear_text_field:returning\n"));
}
	...
	...)
{
	...
	FRM_DBG(("append_text_field:returning sts:%d\n", sts));
	return sts;
}
	...
	void)
{
	...
	FRM_DBG(("update_toggles()\n"));

	FIg_set_state(db_form, FRM_TOGGLE, RISfrm_debug);
...
{
	FRM_DBG(("process_all_off_button()\n"));
	...
	int g_label)
{
	...
	FRM_DBG(("process_debug_toggle(g_label:%d)\n", g_label));

	switch(g_label)
	{
		case FRM_TOGGLE:
			FIg_get_state(db_form, FRM_TOGGLE, &RISfrm_debug);
			break;
...
{
	FRM_DBG(("process_show_servers_button()\n"));
	clear_text_field();
...
{
	FRM_DBG(("process_show_hash_button()\n"));
	clear_text_field();
...
{
	FRM_DBG(("process_show_cli_memory_button()\n"));
	clear_text_field();
...
{
	FRM_DBG(("process_show_cli_stmts_button()\n"));
	clear_text_field();
...
{
	FRM_DBG(("process_show_app_memory_button()\n"));
	clear_text_field();
...
{
	FRM_DBG(("process_show_app_stmts_button()\n"));
	clear_text_field();
...
{
	FRM_DBG(("process_show_schema_file_button()\n"));
	clear_text_field();
...
{
	FRM_DBG(("process_show_struct_mgrs_button()\n"));
	clear_text_field();
...
static int process_db_form(
	...
	Form    fp)
{
	FRM_DBG(("process_db_form(f_label:%d g_label:%d value:%f fp:0x%x)\n",
...
extern int RISfrm_db_form_displayed()
{
	int	form_displayed = 0;

	FRM_DBG(("RISfrm_db_form_displayed()\n"));
	...
	FRM_DBG(("RISfrm_db_form_displayed: returning form_displayed:%d\n",
		form_displayed));
...
extern int RISfrm_display_db_form()
{
	int	sts;

	FRM_DBG(("RISfrm_display_db_form()\n"));
	...
	if (!db_form)
	{
		...
		if (sts)
		{
			RISfrm_form_errhandle(RISUTL_E_CREATE_DB_FORM, sts);
			FRM_DBG(("RISfrm_display_db_form:returning\n"));
			return RIS_forms_error.error;
		}
	}

	if (!RISfrm_db_form_displayed())
	{
		sts = FIf_display (db_form);
		if (sts)
		{
			RISfrm_form_errhandle(RISUTL_E_DISPLAY_DB_FORM, sts);
			FRM_DBG(("RISfrm_display_db_form:returning\n"));
			return RIS_forms_error.error;
		}
	}
...
extern int RISfrm_erase_db_form()
{
	int	sts;

	FRM_DBG(("RISfrm_erase_db_form()\n"));
	...
	if (RISfrm_db_form_displayed())
	{
		...
		if (sts)
		{
			RISfrm_form_errhandle(RISUTL_E_ERASE_DB_FORM, sts);
			FRM_DBG(("RISfrm_erase_db_form:returning\n"));
			return RIS_forms_error.error;
		}

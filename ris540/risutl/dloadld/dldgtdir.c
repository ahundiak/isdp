...
extern int RISdldLDULD_get_risdir(
	...
	rissqlca	*dbcap)			/*  For errors */
{
	...
	if (RISdldLDULD_debug)
	{
		RISdldLDULD_output_debug("RISdldLDULD_get_risdir()\n");
		RISdldLDULD_output_debug("\tprodname:<%s>\n", STRING(prodname));
		RISdldLDULD_output_debug("\tvers_maj:%d vers_min:%d vers_rel:%d mode:%d\n",
			version_maj, version_min, version_rel, mode);
		RISdldLDULD_output_debug("risdir:0x%x development:0x%x dbcap:0x%x\n",
	...
	*/
    if ((cp = getenv("RISDEVDIR")) != NULL)
	{
		...
		DLD_DBG(("RISdldLDULD_get_risdir:complete\n\trisdir:<%s>\n",risdir));
		return(0);
	}
	/*
	...
	*/
	else if (risdir_file && ((fd = fopen(risdir_file,"r")) != NULL))
	{
		...
		if (!cp)
		{
			DLD_DBG(("RISdldLDULD_get_risdir:exiting fgets error reading %s\n",
					 risdir_file));
	...
	*/
	else
	{
		...
		{
			syserr(errno, dbcap);
			DLD_DBG(("RISdldLDULD_get_risdir:exiting CANT OPEN INGRCONFIG\n"));
		...
		{
			DLD_DBG(("RISdldLDULD_get_risdir:exiting CANT FIND PRODUCT\n"));
			return(RIS_E_CANT_FIND_PRODUCT);
		}
	}	/* else config file */
#endif  /* unix */
#if defined(WIN32)
	if (dld_get_platform() != IS_WIN32S)
	{
	...
	{
		DLD_DBG(("RISdldLDULD_get_risdir: RISDEVDIR PathName:<%s>\n", prod_dir));
	...
	*/
   	else
	{
		...
		DLD_DBG(("RISdldLDULD_get_risdir:Opening Reg. key <%s>\n", szKey));
		if ((lResult = RegOpenKeyEx(HKEY_LOCAL_MACHINE, szKey, 0, KEY_READ,
									&hKey2)) != ERROR_SUCCESS)
		{
			DLD_DBG(("RISdldLDULD_get_risdir:exiting CANT FIND REG PRODUCT\n"));
		...
		if (dwNumSubKeys)
		{
			DLD_DBG(("RISdldLDULD_get_risdir:found %d Subkeys\n", dwNumSubKeys));
...
extern void RISdldLDULD_generate_path(
	char *ris_objects,
	char *ris_dir,
	int	 development)
{
	if( development )
	{
		...
		if (dld_get_platform() == IS_WIN32S)
			sprintf(ris_objects, "%s%c%s%c%s%c%s%c%s", ris_dir, PATH_CHAR, "risutl", 
				PATH_CHAR, "lib", PATH_CHAR, PLATFORM, PATH_CHAR, RIS_OBJECT_NAME_32S);
#endif
	}
	else
	{
